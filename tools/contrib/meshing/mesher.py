...
def CreatePrismaticMesh(data,max_depth,ncells,verbose=False):
    """
...
def WriteJou(outfile,data,max_depth,solid,verbose):
    ...
    if verbose: print "Creating journal file..."
    f = open(outfile,'w')
    for r in range(x.shape[0]):
        crv = 'create curve spline'
        for c in range(x.shape[1]):
            crv += ' location %.16e %.16e %.16e' % (x[r,c],y[r,c],z[r,c])
        f.write('%s\n' % crv)
    for c in range(x.shape[1]):
        crv = 'create curve spline'
        for r in range(x.shape[0]):
...
def WriteHDF5(outfile,data,ncells):
    """
...
parser.add_option("-n",
                  action="store", type="int", dest="ncells", default=10,
                  help="number of cells in the depth direction")
parser.add_option("-d",
                  action="store", type="float", dest="depth", default=40,
...
if (options.vtk or options.h5):
    xyz = CreatePrismaticMesh(data,options.depth,options.ncells,options.verbose)
    if options.vtk: WriteVTK("%s.vtk" % fhead,data)
    if options.h5:  WriteHDF5("%s.h5" % fhead,data,options.ncells)
if options.jou or options.solid:
    WriteJou("%s.jou" % fhead,data,options.depth,options.solid,options.verbose)
