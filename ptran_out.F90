...
public ptran_out, ptran_psai_out
contains
...
!---mineral reaction rates
 ...
 !********************************************************************************** 
  subroutine ptran_psai_out (kplt,da,da_1dof,da_kin)
...
!   	  print *,'ptran_out: ',n,kplt,jph,zk,ph,log(10.d0),c_p(jph+(n-1)*ncomp)
  ...
  call VecDestroy(c_nat,ierr)
   ...
   ! write psi in aqeous phase
	...
	fid = 68
    if (kplt < 10) then
      write(fname,'(a4,i1,a4)') 'psai', kplt, '.dat'
    else
      write(fname,'(a4,i2,a4)') 'psai', kplt, '.dat'
    endif
