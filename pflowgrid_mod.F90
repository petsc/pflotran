...
! public pflowGrid_output
! public porperm_out
  public pflowGrid_setvel
  public pflowGrid_destroy
  public pflowGrid_parse_cmdline
    ! pflowGrid_output() should probably be a private method, actually.
...
!          grid%da_3_dof,ierr)
      ...
      call DACreate3D(PETSC_COMM_WORLD,DA_NONPERIODIC,DA_STENCIL_STAR, &
       ...
       if(grid%use_2ph == PETSC_TRUE)then
          ...
          call DACreate3D(PETSC_COMM_WORLD,DA_NONPERIODIC,DA_STENCIL_STAR, &
               nx,ny,nz,npx,npy,npz,grid%nphase*grid%nspec*grid%npricomp,1, &
               PETSC_NULL_INTEGER,PETSC_NULL_INTEGER,PETSC_NULL_INTEGER, &
...
!     allocate(grid%vvl_loc(grid%nconn*grid%nphase))
	  ...
	  if(grid%use_mph == PETSC_TRUE .or. grid%use_owg == PETSC_TRUE)then
...
! check number of dofs and phases
  if (grid%use_cond == PETSC_TRUE) then
    ...
    if(grid%ndof .ne. 1 .or. grid%nphase .ne. 1) then
      write(*,*) 'Specified number of dofs or phases not correct-stop: LIQUID ', &
      'ndof= ',grid%ndof,' nph= ',grid%nphase
      stop
    endif
  endif
...
!        SNESSetSolutionBC
!    call SNESSetSolutionBC( grid%snes,TTPHASESolutionBC,ierr)
      ...
      do i = grid%istart, grid%iend
        do k = 1, grid%ngz - 1
          ...
          grid%area(nc) = dx_loc_p(mg1) * dy_loc_p(mg1)
            ...
            grid%area(nc) =  Pi * (grid%rd(i+grid%nxs)+ grid%rd(i-1+grid%nxs))* &
...
!   allocate(grid%velocitybc(grid%nphase,grid%nconnbc))
   ...
   if(grid%use_mph == PETSC_TRUE .or. grid%use_owg == PETSC_TRUE)then
    allocate(grid%varbc(1:(grid%ndof+1)*(2+7*grid%nphase + 2 * grid%nphase*grid%nspec)))
   else  
...
!       print *,'pflowgrid_mod: ',nc,ibc,ir,m,ng,ii1,ii2,kk1,kk2,grid%nblkbc,grid%igeom
...
!       print *,'pflowgrid_mod: bc- ',nc,ibc,grid%pressurebc(1,nc),grid%pressurebc(2,nc), &
...
! call VecView(grid%conc,PETSC_VIEWER_STDOUT_WORLD,ierr)
 ...
 if (myrank == 0) &
  ...
  case(3)
    if(grid%use_mph /= PETSC_TRUE .and. grid%use_owg /= PETSC_TRUE)then   
...
! update solution vector and physical properties (VecCopy(x,y): y=x)
  if (grid%ndof == 1) then
    ...
    if(grid%ndof <= 3 .and. grid%use_mph/=PETSC_TRUE .and. grid%use_owg/=PETSC_TRUE)then
      call VecCopy(grid%xx, grid%yy, ierr)
...
!....................
    ...
    case ('PHAR')
      ...
      if (grid%myrank==0)&
           write(IUNIT2,'(/," *PHAR ",1pe12.4)')grid%qu_kin
...
!-----------------------------------------------------------------------
   ...
   !       do j=1, grid%nphase
	     ...
	     if(grid%use_mph /= PETSC_TRUE .and. grid%use_owg /= PETSC_TRUE)then	  
			 ...
			 !enddo
			  if(grid%nphase>1) grid%pressurebc0(1,ibc)=grid%pressurebc0(2,ibc)
			   ! For simple input
          ...
          do ireg = grid%iregbc1(ibc), grid%iregbc2(ibc)
            if(grid%use_mph== PETSC_TRUE .or. grid%use_owg== PETSC_TRUE)then
              ...
              if (grid%ibndtyp(ibc) == 1 .or. grid%ibndtyp(ibc) == 3) then
                ...
                grid%j1bc(ireg),grid%j2bc(ireg), &
                grid%k1bc(ireg),grid%k2bc(ireg), &
                (grid%pressurebc0(j,ireg),j=1,grid%nphase),grid%tempbc0(ireg), &
                grid%concbc0(ireg)
              else if (grid%ibndtyp(ibc) == 2) then
                write(IUNIT2,'(6i4,1p10e12.4)') &
                grid%i1bc(ireg),grid%i2bc(ireg), &
                grid%j1bc(ireg),grid%j2bc(ireg), &
                grid%k1bc(ireg),grid%k2bc(ireg), &
                (grid%velocitybc0(j,ireg),j=1,grid%nphase),grid%tempbc0(ireg), &
                grid%concbc0(ireg)
              endif
		    endif
          enddo
