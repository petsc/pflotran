...
#include "definitions.h"
  ...
  PetscScalar, pointer :: perm_xx_p(:),perm_yy_p(:),perm_zz_p(:),por_p(:),tor_p(:), volume_p(:)
  integer iln,na,nx,ny,nz,ir,ierr
  real*8 ::  xc,yc,zc,vc
  real*8 ::  px,py,pz, por, tor
     ...
     call fiReadInt(string,ncna,ierr) 
	   ...
	   if(na1 ==ir) then
		  ...
		  ! z+1
		   ...
		   if(na2 ==ir)then
		    grid%nd2(nnc) = nx- grid%ngxs + (ny-1-grid%ngys)*grid%ngx +(nz-1-grid%ngzs)*grid%ngxy
		    ina2=1; grid%iperm1(nnc) = 3; grid%iperm2(nnc) =3
		   endif
		   ...
		   if(na2 ==ir)then
		    ...
		    nx=nx1+1; ny=ny1; nz=nz1
		   ir = nxyz2na(grid,nx,ny,nz)
		   if(na2 ==ir)then
		    grid%nd2(nnc) = nx- grid%ngxs + (ny-1-grid%ngys)*grid%ngx +(nz-1-grid%ngzs)*grid%ngxy
			ina2=1; grid%iperm1(nnc) = 1; grid%iperm2(nnc) =1
		   endif
		  ...
		  !  nx=nx1; ny=ny1+1; nz=nz1
		  ! ir = nxyz2na(grid,nx,ny,nz)
		  ! if(na2 ==ir)then
		  ! grid%nd2(nnc) = nx- grid%ngxs + (ny-1-grid%ngys)*grid%ngx +(nz-1-grid%ngzs)*grid%ngxy
		  !  ina2=1; grid%iperm1(nnc) = 2; grid%iperm2(nnc) =2
		  ! endif
	    ...
	    elseif(na2==ir)then			
		  ...
		  ! z+1
		   ...
		   nx=nx2; ny=ny2; nz=nz2+1
		   ir = nxyz2na(grid,nx,ny,nz)
		   if(na1 ==ir)then
		    grid%nd1(nnc) = nx- grid%ngxs + (ny-1-grid%ngys)*grid%ngx +(nz-1-grid%ngzs)*grid%ngxy
		    ina1=1; grid%iperm1(nnc) = 3; grid%iperm2(nnc) =3
		   endif
		   ...
		   if(na1 ==ir)then
		    ...
		    nx=nx2+1; ny=ny2; nz=nz2
		   ir = nxyz2na(grid,nx,ny,nz)
		   if(na2 ==ir)then
		    grid%nd1(nnc) = nx- grid%ngxs + (ny-1-grid%ngys)*grid%ngx +(nz-1-grid%ngzs)*grid%ngxy
			ina1=1; grid%iperm1(nnc) = 1; grid%iperm2(nnc) =1
		   endif
		...
		!    nx=nx2; ny=ny2+1; nz=nz2
		!   ir = nxyz2na(grid,nx,ny,nz)
		!   if(na1 ==ir)then
		!   grid%nd1(nnc) = nx- grid%ngxs + (ny-1-grid%ngys)*grid%ngx +(nz-1-grid%ngzs)*grid%ngxy
		!    ina1=1; grid%iperm1(nnc) = 2; grid%iperm2(nnc) =2
		!   endif
