...
module mixture_module
 ...
 real*8, private, parameter::eps=1D-6
 contains
...
!        apply mixing rules
  ...
  subroutine mixture_eos_noderiv (p,t,xga,sg, energyscale,num_phase,num_spec,&
                    ...
                    pckr_m,pckr_pcmax,sat_pressure,den,avgmw,h, &
                    u,diff,hen,pc,kvr,ierr)
...
! P/Pa, t/(Degree Centigreed), Pc/Pa, Hen(xla=Hen*xga, dimensionless)
    ...
    integer ::  ipckrtype, ithrmtype,num_phase,num_spec,num_pricomp
    ...
    ierr=0
    den=0.D0; h=0.D0; u=0.D0; avgmw=0.D0; pc=0.D0; kvr=0.D0
    diff=0.D0; hen=0.D0
...
!***************  Liquid phase properties **************************
   ...
   ! call VISW_noderiv(t,pw,sat_pressure,tmp,ierr)
  !  print *,'visw  ',visl,tmp
    dif= 1.D-7 !effective diffusion coeff in liquid phase
...
!********************************************************************* 
    ...
    pa=p*xga
    if(pa>=5D4)then
      call co2_span_wagner(p*xga*1D-6,t+273.15D0,dg,dddt,dddp,fg,&
      dfgdp,dfgdt,eng,hg,dhdt,dhdp,visg,dvdt,dvdp)
      dg= dg / fmwco2
      fg= fg * 1D6
      hg= hg * fmwco2
    else      
...
!    hg=hg*1.D-3
    ...
    call Henry_CO2_noderiv(xla,tmp,t,p*xga, fg/p/xga, henry,co2_poyn)
    henry=henry
...
!*************************************************************
  ...
  subroutine mixture_eos(p,t,xga,sg, energyscale,num_phase,num_spec, &
              ...
              pckr_pcmax, sat_pressure,den,den_p,den_t,den_c,den_s, avgmw,avgmw_c, &
               h,h_p,h_t,h_c,h_s, u,u_p,u_t,u_c,u_s, diff, diff_p,&
               diff_t, diff_c, diff_s, hen,hen_p, hen_t, hen_c,&
               hen_s,pc, pc_p, pc_t, pc_c, pc_s,kvr,kvr_p,kvr_t,kvr_c,kvr_s,ierr)
...
!  since the conservation equations do not refer to kr or vis  individually
    ...
    real*8  :: pc(num_phase),pc_p(num_phase),pc_t(num_phase), &
                             pc_s(num_phase),pc_c(num_phase*num_pricomp)
    real*8  :: kvr(num_phase),kvr_p(num_phase),kvr_t(num_phase), &
                              kvr_s(num_phase),kvr_c(num_phase*num_pricomp)
    ...
    real*8  :: hw_p,hw_t,dg,dg_p,dg_t,hg,hg_p,hg_t,ug,ug_p,ug_t
    real*8  :: dstea,dsteamol,dstea_p,dstea_t, hstea,hstea_p,hstea_t
    real*8  :: kr(num_phase),kr_s(num_phase),visg,visw_p,visw_t,visg_t,&
               visg_p ,visg_c,visg_s 
    ...
    ierr=0 
    den=0.D0; den_p=0.D0; den_t=0.D0; den_c=0.D0; den_s=0.D0
...
!***************  Liquid phase properties **************************
    ...
    if(num_phase>=2)then
       call pflow_pckr(ipckrtype,pckr_swir,pckr_lambda, pckr_alpha,&
                        pckr_m,pckr_pcmax,sg,pc,pc_s,kr,kr_s) 
    !   print *,'kr', kr,kr_s
       pw=p!-pc(1)
       if((pw<0.D0).and.(sg>(1.D0-eps)))then
          pw=p
 ...
 ! once called, gives pc, kr for both phases 
    ...
    xlw=1.D0
    avgmw(1)=xlw*18.0153d0+(1.D0-xlw)*28.96d0
    avgmw_c(1)=0.D0 
    ...
    diff_t(1:num_spec)=0.
    diff_s(1:num_spec)=0.
    diff_c(1:num_spec*num_pricomp)=0.
...
!********************************************************************* 
    ...
    if(pa>=5D4)then
      ...
      hg= hg * fmwco2
      dddt = dddt/ fmwco2
      dddp = dddp/ fmwco2 * 1D-6
      dfgdp=dfgdp
      dfgdt=dfgdt* 1D6
      eng=eng*fmwco2
      dhdt=dhdt* fmwco2
      dhdp=dhdp* fmwco2*1D-6
      dvdp=dvdp*1D-6
!      print *,'Ana: ', dg,dddt/ fmwco2,dddp/ fmwco2 *1D-6,fg,&
!      dfgdp,dfgdt* 1D6,eng*fmwco2,hg,dhdt* fmwco2,dhdp* fmwco2*1D-6,visg,dvdt,dvdp*1D-6
    else      
      call ideal_gaseos_noderiv(pa,t,energyscale,dg,hg,ug)
      call visco2(t,dg*fmwco2,visg)
      fg=pa
...
!****************End Checking *****************
 ...
 if(xgw>eps)then
  ...
  ! conduct numerical evaluation of vapor properties ********************************
    ...
    den_p(2) = dddp *xga + dstea_p *xgw
    h_p(2) =  xga* dhdp*xga  + xgw * hstea_p *xgw
    u_p(2) =  h_p(2) - (1.D0/den(2) - p/den(2)*den_p(2)/den(2)) * energyscale
    visg_p =  dvdp 
   
    den_t(2) = dddt + dstea_t
    h_t(2) =  xga* dhdt  + xgw * hstea_t
    u_t(2) = h_t(2) + p * den_t(2)/ den(2) /den(2) * energyscale
    visg_t = dvdt
    
    den_c(2) = p * (  dddp - dstea_p)
    h_c(2) = p * ( hg-hstea + xga* p * dhdp - xgw * p *   )
    u_c(2) = h_c(2) + p *  den_c(2)/ den(2) /den(2)  * energyscale
 ...
 !dp 
    ...
    call Henry_CO2_noderiv(xlco2,xmlco2,t,(p+tmp)*xga, fg/(p+tmp)/xga, henry,co2_poyn)
  ...
  ! call Henry_CO2_noderiv(xlco2,xmlco2,t,p*(xga+tmp), co2_phi, henry,co2_poyn)
    ...
    call Henry_CO2_noderiv(xlco2,xmlco2,t,p*(tmp+xga), fg/p/(xga+tmp), henry,co2_poyn)  
