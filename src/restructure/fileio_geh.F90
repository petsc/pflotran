...
! ************************************************************************** !
!
! fiInitializeParallelInput: Initializes parallel input
! author: Glenn Hammond
! date: 06/15/07
!
! ************************************************************************** !
subroutine fiInitializeParallelInput()
...
end subroutine fiInitializeParallelInput
...
subroutine fiReadMultDouble(fid, string, doubles, n, variable_name, keyword, &
  ...
  do
    ...
    if (ierr /= 0) then
      ...
      ierr = 0
      call fiReadFlotranStringLocal(fid,string,ierr)
...
!
! ************************************************************************** !
subroutine fiReadString(fid, string, read_err)

  implicit none

  character(len=*) :: string
  integer :: fid, irecv, read_err, ierr
  ...
  if (myrank > -1) then
    if (myrank == 0) then
      read(fid,'(a128)',iostat=read_err) string
    endif
    call mpi_bcast(string,MAXSTRINGLENGTH,MPI_CHARACTER,0, &
                   PETSC_COMM_WORLD,ierr)
    call mpi_bcast(read_err,1,MPI_INTEGER,0, &
                   PETSC_COMM_WORLD,ierr)
  else
    read(fid,'(a128)',iostat=read_err) string
  endif
...
!
! ************************************************************************** !
subroutine fiReadFlotranString(fid, string, read_err)

  implicit none

  character(len=*) :: string
  integer :: fid, irecv, ierr, read_err
  ...
  if (myrank > -1) then
    if (myrank == 0) then
      call fiReadFlotranStringLocal(fid,string,read_err)
    endif
    call mpi_bcast(string,MAXSTRINGLENGTH,MPI_CHARACTER,0, &
                   PETSC_COMM_WORLD,ierr)
    call mpi_bcast(read_err,1,MPI_INTEGER,0, &
...
!geh - debug
!  if (myrank == 0) print *, 'p0: (',ierr,') ', trim(string)
!  if (myrank == 1) print *, 'p1: (',ierr,') ', trim(string)
...
! ************************************************************************** !
!
! fiReadFlotranStringLocal: Reads a string (strlen characters long) from a 
!                           file while avoiding commented or skipped lines.
! author: Glenn Hammond
! date: 11/02/00
!
! ************************************************************************** !
subroutine fiReadFlotranStringLocal(fid, string, ierr)
...
end subroutine fiReadFlotranStringLocal
...
!
! ************************************************************************** !
subroutine fiReadDBaseString(fid, string, read_err)

  implicit none

  character(len=*) :: string
  character(len=dbaselen) :: tempstring, word
  integer :: i, fid, irecv, ierr, read_err
  ...
  if (myrank > -1) then
    if (myrank == 0) then
      call fiReadDBaseStringLocal(fid,string,read_err)
    endif
    call mpi_bcast(string,MAXSTRINGLENGTH,MPI_CHARACTER,0, &
                   PETSC_COMM_WORLD,ierr)
    call mpi_bcast(read_err,1,MPI_INTEGER,0, &
...
! ************************************************************************** !
!
! fiReadDBaseStringLocal: Reads a long string (dbaselen characters long) 
!                         from the database file while avoiding commented 
!                         or skipped lines.
! author: Glenn Hammond
! date: 06/15/07
!
! ************************************************************************** !
subroutine fiReadDBaseStringLocal(fid, string, ierr)
...
end subroutine fiReadDBaseStringLocal
...
!
! ************************************************************************** !
  subroutine fiFindStringInFile(fid, string, read_err)

  implicit none

  character(len=*) :: string
  character(len=strlen) :: string2
  integer :: fid, ierr, irecv, length, read_err
  ...
  if (myrank > -1) then
    if (myrank == 0) then
      ...
      length = len_trim(string)
      do 
        call fiReadFlotranStringLocal(fid,string2,read_err)
