...
!
! ************************************************************************** !
subroutine RealizationMapSurfSubsurfaceGrids(realization)
...
#include "finclude/petscmat.h90"
  ...
  Mat :: Mat_vert_to_face_subsurf_trans
  Mat :: Mat_vert_to_face_surf
  Mat :: Mat_vert_to_face_surf_trans
  Mat :: prod
  ...
  surf_grid => realization%discretization%surfgrid%unstructured_grid
  ...
  call MatTranspose(Mat_vert_to_face_subsurf,MAT_INITIAL_MATRIX, &
                    Mat_vert_to_face_subsurf_trans,ierr)

#if UGRID_DEBUG
  string = 'Mat_vert_to_face_subsurf_trans.out'
  call PetscViewerASCIIOpen(option%mycomm,string,viewer,ierr)
  call MatView(Mat_vert_to_face_subsurf_trans,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
  ...
  call MatTranspose(Mat_vert_to_face_surf,MAT_INITIAL_MATRIX, &
                    Mat_vert_to_face_surf_trans,ierr)

#if UGRID_DEBUG
  string = 'Mat_vert_to_face_surf_trans.out'
  call PetscViewerASCIIOpen(option%mycomm,string,viewer,ierr)
  call MatView(Mat_vert_to_face_surf_trans,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
#endif  
  
  call MatMatMult(Mat_vert_to_face_subsurf,Mat_vert_to_face_surf_trans, &
  ...
  call RealizationMapSurfSubsurfaceGrid(realization, prod, TWO_DIM_GRID, &
                                        surf_petsc_ids)
  call MatDestroy(prod,ierr)

  call MatMatMult(Mat_vert_to_face_surf,Mat_vert_to_face_subsurf_trans, &
                  MAT_INITIAL_MATRIX,PETSC_DEFAULT_DOUBLE_PRECISION,prod,ierr)
  ...
  call PetscViewerDestroy(viewer,ierr)
#endif  
  call RealizationMapSurfSubsurfaceGrid(realization, prod, THREE_DIM_GRID, &
                                        subsurf_petsc_ids)

  call MatDestroy(prod,ierr)

  call MatDestroy(Mat_vert_to_face_subsurf,ierr)
  call MatDestroy(Mat_vert_to_face_subsurf_trans,ierr)
  call MatDestroy(Mat_vert_to_face_surf,ierr)
  call MatDestroy(Mat_vert_to_face_surf_trans,ierr)

  call VecDestroy(subsurf_petsc_ids,ierr)
  call VecDestroy(surf_petsc_ids,ierr)

end subroutine RealizationMapSurfSubsurfaceGrids
...
!! date: 01/18/12
! ************************************************************************** !
subroutine RealizationMapSurfSubsurfaceGrid(realization, &       !<
...
#include "finclude/petscmat.h90"
  ...
  select case(source_grid_flag)
    case(TWO_DIM_GRID)
      dm_ptr => DiscretizationGetDMPtrFromIndex(realization%discretization,SURF_ONEDOF)
