...
!                    cell_distribution(option%myrank+2)
  ...
  call VecDuplicate(elements_local,elements_local2,ierr)
  call VecCopy(elements_local,elements_local2,ierr)
  allocate(unstructured_grid%vertices_with_border &
            (unstructured_grid%num_vertices_local))
  do ivertex = 1, unstructured_grid%num_vertices_local
    unstructured_grid%vertices_with_border(ivertex)%id = &
      unstructured_grid%vertices(ivertex)%id
    unstructured_grid%vertices_with_border(ivertex)%x = &
      unstructured_grid%vertices(ivertex)%x
    unstructured_grid%vertices_with_border(ivertex)%y = &
      unstructured_grid%vertices(ivertex)%y
    unstructured_grid%vertices_with_border(ivertex)%z = &
      unstructured_grid%vertices(ivertex)%z
  enddo  
...
! load up the coordinates
...
#ifdef GEOMECH
  ! Following needed for geomechanics, SK 05/30/2013
  ! make a list of local vertices
  max_int_count = 2*unstructured_grid%nlmax
  allocate(int_array_pointer(max_int_count))
  int_array_pointer = 0
  vertex_count = 0
  call VecGetArrayF90(elements_local2,vec_ptr,ierr)
  do local_id = 1, unstructured_grid%nlmax ! Note that ghosted cells not considered
    do ivertex = 1, unstructured_grid%max_nvert_per_cell
      vertex_id = int(vec_ptr(ivertex + vertex_ids_offset + (local_id-1)*stride))
      if (vertex_id < 1) exit
      vertex_count = vertex_count + 1
    ...
    enddo
  enddo
  call VecRestoreArrayF90(elements_local2,vec_ptr,ierr)
  ...
  allocate(unstructured_grid%vertex_ids_natural_without_ghosted(vertex_count))
  unstructured_grid%vertex_ids_natural_without_ghosted = &
                                                int_array3(1:vertex_count)
                                                
  ! now load all the vertices needed to define all the local cells without 
  ! without ghosted cells on the processor
  deallocate(needed_vertices_petsc)
  allocate(needed_vertices_petsc(vertex_count))
  needed_vertices_petsc(1:vertex_count) = int_array3(1:vertex_count)
  
  ! Store the number of vertices on the local (unghosted) cells
  unstructured_grid%num_vertices_local_with_border = vertex_count

  ! allocate the array that will store the vertex ids for each cell.
  ! remember that max_nvert_per_cell is the max # of vertices in a cell
  ! currently hardwired to 8.
  allocate(unstructured_grid%cell_vertices_without_ghosted( &
             0:unstructured_grid%max_nvert_per_cell,unstructured_grid%nlmax))
  unstructured_grid%cell_vertices_without_ghosted = 0
  ...
  do local_id = 1, unstructured_grid%nlmax
    do ivertex = 1, unstructured_grid%max_nvert_per_cell
      ...
      vertex_id = int(vec_ptr(ivertex + vertex_ids_offset + (local_id-1)*stride))
      if (vertex_id < 1) exit
      count = unstructured_grid%cell_vertices_without_ghosted(0,local_id)+1
      unstructured_grid%cell_vertices_without_ghosted(count,local_id) = &
        int_array4(vertex_id)
      unstructured_grid%cell_vertices_without_ghosted(0,local_id) = count
      ! load the permuted value back into the petsc vector
  ...
  ! The elem_local vectors at this point have information beyond nlmax
  ! which can be ignored.
#if UGRID_DEBUG
  write(string,*) option%myrank
  if (unstructured_grid%grid_type == THREE_DIM_GRID) then
    string = 'elements_vert_local_unghosted_elem' // trim(adjustl(string)) &
      // '_subsurf.out'
  else
    string = 'elements_vert_local_unghosted_elem' // trim(adjustl(string)) // &
      '_surf.out'
  ...
  call PetscViewerASCIIOpen(PETSC_COMM_SELF,trim(string),viewer,ierr)
  call VecView(elements_local2,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
#endif  
  call VecDestroy(elements_local2,ierr)
  ...
  ! IS for gather operation - need local numbering
  allocate(int_array(vertex_count))
  ! vertex_count = # of local vertices (no ghosted element vertices)
  do ivertex = 1, vertex_count
    int_array(ivertex) = ivertex-1
  enddo
  ...
  call VecCreate(option%mycomm,vertices_old,ierr)
  call VecSetSizes(vertices_old, &
                  size(unstructured_grid%vertices_with_border)*3, &
                  PETSC_DECIDE,ierr)  
  call VecSetBlockSize(vertices_old,3,ierr)
  call VecSetFromOptions(vertices_old,ierr)
  ...
  call VecGetArrayF90(vertices_old,vec_ptr,ierr)
  do ivertex = 1, size(unstructured_grid%vertices_with_border)
    vec_ptr((ivertex-1)*3+1) = unstructured_grid%vertices_with_border(ivertex)%x
    vec_ptr((ivertex-1)*3+2) = unstructured_grid%vertices_with_border(ivertex)%y
    vec_ptr((ivertex-1)*3+3) = unstructured_grid%vertices_with_border(ivertex)%z
  enddo
  call VecRestoreArrayF90(vertices_old,vec_ptr,ierr)
  ...
  ! resize vertex array to new size
  unstructured_grid%num_vertices_local_with_border = vertex_count
  allocate(unstructured_grid%vertices_with_border(vertex_count))
  do ivertex = 1, vertex_count
    unstructured_grid%vertices_with_border(ivertex)%x = 0.d0
    unstructured_grid%vertices_with_border(ivertex)%y = 0.d0
    unstructured_grid%vertices_with_border(ivertex)%z = 0.d0
  enddo

#if UGRID_DEBUG
  if (unstructured_grid%grid_type == THREE_DIM_GRID) then
    call PetscViewerASCIIOpen(option%mycomm, &
      'is_scatter_vert_old_to_new_subsurf_border.out',viewer,ierr)
  else
    call PetscViewerASCIIOpen(option%mycomm, &
      'is_scatter_vert_old_to_new_surf_border.out',viewer,ierr)
  endif
  call ISView(is_scatter,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
  if (unstructured_grid%grid_type == THREE_DIM_GRID) then
    call PetscViewerASCIIOpen(option%mycomm, &
      'is_gather_vert_old_to_new_subsurf_border.out',viewer,ierr)
  else
    call PetscViewerASCIIOpen(option%mycomm, &
      'is_gather_vert_old_to_new_surf_border.out',viewer,ierr)
  endif
  call ISView(is_gather,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
#endif
  ...
  if (unstructured_grid%grid_type == THREE_DIM_GRID) then
    call PetscViewerASCIIOpen(option%mycomm, &
      'vertex_coord_old_subsurf_border.out',viewer,ierr)
  else
    call PetscViewerASCIIOpen(option%mycomm, &
      'vertex_coord_old_surf_border.out',viewer,ierr)
  endif
  call VecView(vertices_old,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
#endif
  ...
  call VecGetArrayF90(vertices_new,vec_ptr,ierr)
  do ivertex = 1, unstructured_grid%num_vertices_local_with_border
    unstructured_grid%vertices_with_border(ivertex)%id = &
      needed_vertices_petsc(ivertex)
    unstructured_grid%vertices_with_border(ivertex)%x = vec_ptr((ivertex-1)*3+1)
    unstructured_grid%vertices_with_border(ivertex)%y = vec_ptr((ivertex-1)*3+2)
    unstructured_grid%vertices_with_border(ivertex)%z = vec_ptr((ivertex-1)*3+3)
  enddo
  call VecRestoreArrayF90(vertices_new,vec_ptr,ierr)
  ...
  write(string,*) option%myrank
  if (unstructured_grid%grid_type == THREE_DIM_GRID) then
    string = 'vertex_coord_new' // trim(adjustl(string)) // &
      '_subsurf_border.out'
  else
    string = 'vertex_coord_new' // trim(adjustl(string)) // '_surf_border.out'
  endif
  call PetscViewerASCIIOpen(PETSC_COMM_SELF,trim(string),viewer,ierr)
  call VecView(vertices_new,viewer,ierr)
  call PetscViewerDestroy(viewer,ierr)
...
#if UGRID_DEBUG
  call printMsg(option,'Setting cell types for geomechanics grid')
#endif

  allocate(unstructured_grid%cell_type_without_ghosted(unstructured_grid%nlmax))

  select case(unstructured_grid%grid_type)
    case(THREE_DIM_GRID)
      do local_id = 1, unstructured_grid%nlmax
        ! Determine number of faces and cell-type of the current cell
        select case(unstructured_grid%cell_vertices_without_ghosted(0,local_id))
          case(8)
            unstructured_grid%cell_type_without_ghosted(local_id) = HEX_TYPE
          case(6)
            unstructured_grid%cell_type_without_ghosted(local_id) = WEDGE_TYPE
          case(5)
            unstructured_grid%cell_type_without_ghosted(local_id) = PYR_TYPE
          case(4)
            unstructured_grid%cell_type_without_ghosted(local_id) = TET_TYPE
          case default
            option%io_buffer = 'Cell type not recognized: '
            call printErrMsg(option)
        end select      
      enddo
    case(TWO_DIM_GRID)
      option%io_buffer = '2D Grid not supported with GEOMECHANICS'
      call printErrMsg(option)
    case default
      option%io_buffer = 'Grid type not recognized: '
      call printErrMsg(option)
