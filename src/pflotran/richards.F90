            ...
            if (global_aux_vars(ghosted_id)%pres(1) > pressure_min) then
              ...
              if (dphi >= 0.D0) then ! outflow only
                ...
                if (ukvr*Dq > floweps) then
                  v_darcy = Dq * ukvr * dphi
                  ! store volumetric rate for ss_fluid_fluxes()
                  Jup(1,1) = -Dq*rich_aux_vars(ghosted_id)%dkvr_dp*dphi*global_aux_vars(ghosted_id)%den(1) &
                             -Dq*ukvr*1.d0*global_aux_vars(ghosted_id)%den(1) &
                             -Dq*ukvr*dphi*rich_aux_vars(ghosted_id)%dden_dp
                endif
              endif
            endif
          endif 
