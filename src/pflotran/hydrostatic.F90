...
! ************************************************************************** !
!
! HydrostaticUpdateCouplerRotate: Computes the hydrostatic initial/boundary 
!                           condition 
! author: Glenn Hammond
! date: 11/28/07
!
! ************************************************************************** !
subroutine HydrostaticUpdateCouplerRotate(coupler,option,grid)
  ...
  PetscInt :: local_id, ghosted_id, iconn
  PetscInt :: num_iteration, ipressure, idatum, num_pressures
  PetscReal :: dist_x, dist_y, dist_z, dist_depth, delta_depth
  PetscReal :: dx_conn, dy_conn, dz_conn
  PetscReal :: rho, rho1, rho0, pressure, pressure0, pressure_at_datum 
  PetscReal :: temperature_at_datum, temperature
  PetscReal :: concentration_at_datum
  PetscReal :: xm_nacl, dw_kg
  PetscReal :: max_depth, min_depth
  PetscReal, pointer :: pressure_array(:), density_array(:), depth(:)
  PetscReal :: pressure_gradient(3), piezometric_head_gradient(3), datum(3)
  PetscReal :: temperature_gradient(3), concentration_gradient(3)
  
  PetscReal :: unit_vector(3), length_vector(3), vector(3)
  PetscReal :: magnitude_gravity, max_distance
  PetscInt :: i, j, k
  ...
  max_distance = sqrt(dot_product(unit_vector,length_vector))
  
  
  datum(1:3) = condition%pressure%datum%cur_value(1:3)
  vector = grid%x_max_global - datum(X_DIRECTION)
  vector = ProjectAOntoUnitB(vector,unit_vector)
  max_distance = max(
  ...
  rho = dw_kg * 1.d3
  pressure_gradient(1:3) = piezometric_head_gradient(1:3)* &
                           rho*magnitude_gravity ! gravity is negative, but we just want magnitude

  if (dabs(pressure_gradient(Z_DIRECTION)) < 1.d-40) then
    ! compute the vertical gradient based on a 1 meter vertical spacing and
    ! interpolate the values from that array
    max_depth = max(grid%z_max_global,datum(Z_DIRECTION))+1.d0 ! add 1m buffer
    min_depth = min(grid%z_min_global,datum(Z_DIRECTION))-1.d0
    
    num_pressures = int((max_depth-min_depth)/delta_depth) + 1
    allocate(pressure_array(num_pressures))
    allocate(density_array(num_pressures))
    allocate(depth(num_pressures))
    pressure_array = 0.d0
    density_array = 0.d0
    depth = 0.d0
    ! place this pressure in the array
    idatum = int((datum(Z_DIRECTION)-min_depth)/(max_depth-min_depth) * &
                 dble(num_pressures))+1
    pressure_array(idatum) = pressure_at_datum
    call nacl_den(temperature_at_datum,pressure_at_datum*1.d-6,xm_nacl,dw_kg) 
    temperature = temperature_at_datum
    ...
    rho = dw_kg * 1.d3
    density_array(idatum) = rho
    depth(idatum) = datum(Z_DIRECTION)
    ! compute pressures above datum, if any
    dist_depth = 0.d0
    rho0 = rho
    do ipressure=idatum+1,num_pressures
      dist_depth = dist_depth + delta_depth
!      if (option%iflowmode /= RICHARDS_MODE) &
!        temperature = temperature + temperature_gradient(Z_DIRECTION)*delta_depth
      call nacl_den(temperature,pressure0*1.d-6,xm_nacl,dw_kg) 
      rho = dw_kg * 1.d3
      ...
      pressure_array(ipressure) = pressure
      density_array(ipressure) = rho
      depth(ipressure) = depth(idatum)+dist_depth
      pressure0 = pressure
    enddo
...
end subroutine HydrostaticUpdateCouplerRotate
#endif
! ************************************************************************** !
!
