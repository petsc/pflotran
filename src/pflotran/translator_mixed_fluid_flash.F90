...
! within each phase component index : 1. H2O; 2. CO2; 3. Air
 ...
 PetscReal, private, parameter :: fmwh2o = 18.0153D0, fmwa = 28.96D0, &
                               fmwco2 = 44.0098D0
 PetscReal, private, parameter :: eps=5D-7, formeps=1D-5
...
! print *,nzc,c0
 ...
 !  call PETSCBarrier(PETSC_NULL_OBJECT,ierr)
  ...
  if(grid%commsize >1)then
    call MPI_REDUCE(n2p, n2p0,ONE_INTEGER,&
        MPI_INTEGER,MPI_SUM,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)
    call MPI_REDUCE(c0, c00,ONE_INTEGER,&
        MPI_DOUBLE_PRECISION,MPI_SUM,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)
    call MPI_REDUCE(nxc, nxc0,ONE_INTEGER,&
        MPI_DOUBLE_PRECISION,MPI_SUM,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)
   call MPI_REDUCE(nyc, nyc0,ONE_INTEGER,&
        MPI_DOUBLE_PRECISION,MPI_SUM,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)

    call MPI_REDUCE(nzc, nzc0,ONE_INTEGER,&
        MPI_DOUBLE_PRECISION,MPI_SUM,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)
  call MPI_REDUCE(nzm, nzm0,ONE_INTEGER,&
        MPI_DOUBLE_PRECISION,MPI_MIN,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)
    call MPI_REDUCE(nsm, nsm0,ONE_INTEGER,&
        MPI_DOUBLE_PRECISION,MPI_MIN,ZERO_INTEGER, PETSC_COMM_WORLD,ierr)
...
! & grid%t/grid%tconv,tot(2,1),tot(2,2),tot(2,0),tot(2,1)+tot(2,2)
 ...
 PetscInt function flash_check_phase_cond(iphase, var_node,num_phase,num_spec)
   implicit none
...
! P/Pa, t/(Degree Centigreed), Pc/Pa, Hen(xla=Hen*xga, dimensionless)
 ...
 !  PetscInt :: size_var_node = (grid%ndof+1)*size_var_use
  ...
  PetscReal p2,tmp,temp1
  PetscReal pw,dw_kg,dw_mol,hw,sat_pressure,visl,xphi, dco2
  PetscReal dg,fg,eng,hg,visg
  PetscReal ug,dddt,dddp,dfgdp,dfgdt,dhdt,dhdp, dvdt,dvdp
  PetscReal henry
  PetscReal dsteamol,hstea
  PetscReal kr(num_phase)
  PetscReal xla,vphi

! PetscReal :: p1,co2_phi,co2_poyn,stea,dstea_p,dstea_t,hstea_p,hstea_t,dstea
! PetscReal :: err,pckr_swir
