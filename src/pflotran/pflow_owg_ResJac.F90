...
! Cutoff parameters
  ...
  PetscReal, parameter :: floweps   = 1.D-20
...
! Contributions to residual from accumlation/source/Reaction, flux(include diffusion)
 ...
 subroutine pflow_OWG_timecut(grid)
 ...
 ! call VecGetArrayF90(grid%iphas, iphase_p, ierr); 
  ...
  end subroutine pflow_OWG_timecut
  

 subroutine pflow_OWG_setupini(grid)
  implicit none
  type(pflowGrid), intent(inout) :: grid
...
!   print *,'pflow_OWG_resjac: ',na,nx,ny,nz
 ...
 enddo
  ...
  end  subroutine pflow_OWG_setupini
  

 subroutine OWG_Update_Reason(reason,grid)
...
!   enddo
 ...
 ! print *,' update reason', grid%myrank, re,n,grid%nlmax
  ...
  end subroutine OWG_Update_Reason
...
!should include related energy change here
 ...
 subroutine OWGRes_FLCont(nconn_no,area, &
  ...
  PetscReal uxmol(1:grid%nspec),ukvr,difff,diffdp,Dq
  PetscReal upweight,density_ave,gravity, dphi
  
! PetscReal :: uh, DK,cond, 
...
! conduction term
 ...
 end subroutine OWGRes_FLCont

 subroutine OWGRes_FLBCCont(nbc_no,area, &
 ...
 ! Notice : index 1 stands for BC node
  ...
  PetscReal fluxm(grid%nspec),fluxe,v_darcy,q
  PetscReal uh,uxmol(1:grid%nspec),ukvr,diff,diffdp,Dq
  PetscReal upweight,density_ave,gravity, dphi
...
!      grid%vvbc(j+(nc-2)*grid%nphase)= grid%velocitybc(j,nc)
  ...
  Res_FL(1:grid%nspec)=fluxm(:)* grid%dt
   ...
   nullify(temp1, pre_ref1, sat1, density1, amw1, h1,u1, pc1,kvr1,xmol1,diff1)       
  nullify(temp2, pre_ref2, sat2, density2, amw2, h2,u2, pc2,kvr2,xmol2,diff2)       
 end  subroutine OWGRes_FLBCCont 


  subroutine OWGResidual(snes,xx,r,grid,ierr)
...
! PetscReal :: cond, den, por1, por2, density_ave,
  ...
  call VecGetArrayF90(xx, xx_p, ierr); CHKERRQ(ierr)
     do n = 1, grid%nlmax
      do i=2,3
       if(xx_p((n-1)*grid%ndof+i) <0.D0)xx_p((n-1)*grid%ndof+i) =1.D-12
       if(xx_p((n-1)*grid%ndof+i) >1.D0)xx_p((n-1)*grid%ndof+i) =1.D0 -1D-12
        enddo
     enddo
...
!-----  phase properities ---- last time step---
  ...
  call pri_var_trans_owg_ninc(xx_p((n-1)*grid%ndof+1:n*grid%ndof),iiphase,&
    ...
    grid%itable,ierr,grid%xxphi_co2(n), grid%tref)
...
!---------------------------------------------------------------------------
  ...
  do nc = 1, grid%nconn  ! For each interior connection...
    ...
    call OWGRes_FLCont(nc ,grid%area(nc), &
...
!   grid%pressurebc(2,ibc),grid%tempbc(ibc),grid%concbc(ibc),grid%sgbc(ibc)
        ...
        !*****************
  dif(1)= grid%difaq; dif(3)= grid%difaq
    ...
    call OWGRes_FLBCCont(nc,grid%areabc(nc), &
...
! print *,'finished OWGResidual'
 end subroutine OWGResidual
...
! PetscInt :: ibc_hencoeff
  ...
  PetscReal :: vv_darcy(grid%nphase),voldt,pvoldt
...
!print *,'accum r',ra(1:5,1:8)   
 !print *,'devq:',nc,q,dphi,devq(3,:)
  ...
  !print *,' OWG Jaco Finished Two node terms'
...
! stop
 ...
 subroutine pflow_OWG_initaccum(grid)
...
! PetscReal :: sat_pressure, pvol, satw,   ! Saturation pressure of water.
  ...
  call VecGetArrayF90(grid%ithrm, ithrm_p, ierr)
  call VecGetArrayF90(grid%icap, icap_p, ierr)
  !print *,'OWG initaccum  Gotten pointers'
...
! print *,n,accum_p(p1),accum_p(t1),accum_p(c1),accum_p(s1)
 ...
 end subroutine pflow_OWG_initaccum


  subroutine pflow_update_OWG(grid)
    use translator_owg_module  
   ! use water_eos_module
    implicit none
    type(pflowGrid) :: grid 
...
!    if(xx_p(n0+3)<0.D0) xx_p(n0+3)=0.D0
  ...
  dif(1)= grid%difaq; dif(3)=dif(1)
    ...
    !*******************************************
     call pri_var_trans_owg_ninc(xx_p((n-1)*grid%ndof+1:n*grid%ndof),iiphase,&
        ...
        grid%pcbetac(iicap),grid%pwrprm(iicap),dif,&
    var_p((n-1)*size_var_node+1:(n-1)*size_var_node+size_var_use),&
    grid%itable,ierr, tmp, grid%tref)
   ...
   if(grid%nphase>1) call translator_OWG_massbal(grid)
 ! endif 
  ...
  call  pflow_OWG_initaccum(grid)
    !print *,'pflow_OWG_initaccum done'
 ...
 end subroutine pflow_update_OWG





  subroutine pflow_OWG_initadj(grid)
...
!  PetscReal, parameter :: Rg=8.31415D0
 ...
 do n = 1, grid%nlmax
     ...
     dif(3)=grid%difaq
    !*******************************************
  !print *,' owg_initadj:',grid%ndof, xx_p((n-1)*grid%ndof+1:n*grid%ndof)
  ...
  call pri_var_trans_owg_ninc(xx_p((n-1)*grid%ndof+1:n*grid%ndof),iiphase,&
    ...
    var_p((n-1)*size_var_node+1: (n-1)*size_var_node+size_var_use),&
    grid%itable,ierr, tmp, grid%tref)
   ! print *,' owg_initadj:', n,iicap, iiphase, xx_p((n-1)*grid%ndof+1:n*grid%ndof), &
   !             var_p((n-1)*size_var_node+1: (n-1)*size_var_node+size_var_use)

   if(translator_owg_check_phase_cond(iiphase, &
           var_p((n-1)*size_var_node+1: (n-1)*size_var_node+size_var_use),&
...
!      print *,'initadj_bc',nc,ibc,grid%ibndtyp(ibc),grid%nconnbc
      ...
      grid%varbc(1:size_var_use), grid%itable,ierr, tmp, grid%tref)
       ...
       if(translator_owg_check_phase_cond(grid%iphasebc(nc), &
 ...
 end subroutine pflow_OWG_initadj


end module OWG_module
