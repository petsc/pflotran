...
!....................
      ...
      case ('UNIFORM_VELOCITY')
        ...
        if (input%ierr == 0) then
          units_conversion = UnitsConvertToInternal(trim(word),'length/time',option) 
        ...
        if (input%ierr == 0) then
          ...
          if (input%ierr == 0) then
            option%restart_time = option%restart_time* &
                                  UnitsConvertToInternal(trim(word),'time',option)
          else
                ...
                select case(trim(word))
                  case('TIME')
                    ...
                    call InputErrorMsg(input,option,'time increment units', &
                                       'CHECKPOINT,PERIODIC,TIME')
                    units_conversion = UnitsConvertToInternal(trim(word),'time',option)
        ...
        do
          ...
          select case(trim(word))
            ...
            case('FINAL_TIME')
              ...
              temp_real2 = UnitsConvertToInternal(trim(word),'time',option)
            ...
            case('INITIAL_TIMESTEP_SIZE')
              ...
              dt_init = temp_real*UnitsConvertToInternal(trim(word),'time',option)
            case('MINIMUM_TIMESTEP_SIZE')
              ...
              dt_min = temp_real*UnitsConvertToInternal(trim(word),'time',option)
            case('MAXIMUM_TIMESTEP_SIZE')
              ...
              waypoint => WaypointCreate()
              waypoint%dt_max = temp_real*UnitsConvertToInternal(trim(word),'time',option)
              call InputReadWord(input,option,word,PETSC_TRUE)
              if (input%ierr == 0) then
                call StringToUpper(word)
                if (StringCompare(word,'AT',TWO_INTEGER)) then
                  ...
                  waypoint%time = temp_real*UnitsConvertToInternal(trim(word),'time',option)
                else
