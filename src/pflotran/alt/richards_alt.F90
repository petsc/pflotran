...
subroutine RichardsSetupPatch(realization)
  ...
  allocate(aux_vars)
  call RichardsAuxVarInit(aux_vars,grid%ngmax,option)
  patch%aux%Richards%aux_vars => aux_vars
  patch%aux%Richards%num_aux = 1
  ...
  do 
    ...
    sum_connection = sum_connection + &
                     boundary_condition%connection_set%num_connections
    if (boundary_condition%connection_set%num_connections > max_bc_conn) &
      max_bc_conn = boundary_condition%connection_set%num_connections
    call RichardsAuxVarInit(aux_vars_bc(bc_count),boundary_condition%connection_set%num_connections,option)
    boundary_condition => boundary_condition%next
  enddo

  patch%aux%Richards%aux_vars_bc => aux_vars_bc
  patch%aux%Richards%num_aux_bc = bc_count
...
subroutine RichardsUpdateAuxVarsPatch(realization)
  ...
  PetscInt :: iactive(max_bc_conn)
  PetscInt :: iactive2(realization%patch%grid%ngmax)
  PetscReal :: xxbc(max_bc_conn), icap(max_bc_conn), por(max_bc_conn), perm(max_bc_conn), iphasebc(max_bc_conn)
  PetscErrorCode :: ierr
  PetscInt :: ibc
  ...
  call RichardsAuxVarCompute(xx_loc_p,aux_vars,iactive2, &
                             iphase_loc_p,icap_loc_p,realization%saturation_function_array, &
                             porosity_loc_p,perm_xx_loc_p, &                       
                             option)
  ...
  do 
    ...
    do iconn = 1, cur_connection_set%num_connections
      ...
      if (associated(patch%imat)) then
        iactive(iconn) = patch%imat(ghosted_id)
      endif

      select case(boundary_condition%flow_condition%itype(RICHARDS_PRESSURE_DOF))
        case(DIRICHLET_BC,HYDROSTATIC_BC,SEEPAGE_BC)
          xxbc(iconn) = boundary_condition%flow_aux_real_var(RICHARDS_PRESSURE_DOF,iconn)
        case(NEUMANN_BC,ZERO_GRADIENT_BC)
          xxbc(iconn) = xx_loc_p(ghosted_id)
      end select
      
      select case(boundary_condition%flow_condition%itype(RICHARDS_PRESSURE_DOF))
        case(DIRICHLET_BC,HYDROSTATIC_BC,SEEPAGE_BC)
          iphasebc(iconn) = boundary_condition%flow_aux_int_var(RICHARDS_PRESSURE_DOF,iconn)
        case(NEUMANN_BC,ZERO_GRADIENT_BC)
          iphasebc(iconn)=int(iphase_loc_p(ghosted_id))                               
      end select
      icap(iconn) = icap_loc_p(ghosted_id)
      por(iconn) = porosity_loc_p(ghosted_id)
      perm(iconn) = perm_xx_loc_p(ghosted_id)

    enddo
    call RichardsAuxVarCompute(xxbc,aux_vars_bc(ibc),iactive,  &
                       iphasebc,icap, &
...
subroutine RichardsLiUpdateFixedAccumPatch(realization)
  ...
  PetscInt :: iactive(realization%patch%grid%ngmax)

  PetscInt :: ghosted_id, local_id, iphase
  PetscReal, pointer :: xx_p(:), xx_loc_p(:), icap_loc_p(:), iphase_loc_p(:)
  ...
  if (associated(patch%imat)) then
    ...
    call RichardsAuxVarCompute(xx_loc_p,aux_vars,iactive, &
                       iphase_loc_p,icap_loc_p, &
                       realization%saturation_function_array, &
                       porosity_loc_p,perm_xx_loc_p, &                        
                       option)
 ...
 do local_id = 1, grid%nlmax
    ...
    accum_p(local_id) = aux_vars%sat(ghosted_id) * aux_vars%den(ghosted_id) * porosity_loc_p(ghosted_id) * volume_p(local_id)
  enddo  

#if 0
    call RichardsAccumulation(grid,patch%imat,aux_vars,porosity_loc_p, &
                              volume_p, &
                                  option,accum_p) 
#endif                                  
...
!
! ************************************************************************** !
subroutine RichardsAccumDerivative(grid,imat,aux_var,por,vol,option,J)
  ...
  do local_id = 1, grid%nlmax
    ghosted_id = grid%nL2G(local_id)
    !geh - Ignore inactive cells with inactive materials
    if (associated(imat)) then
      if (imat(ghosted_id) <= 0) cycle
    endif
    porXvol = por(ghosted_id)*vol(local_id)
      
    J(1,1) = (aux_var%sat(ghosted_id)*aux_var%dden_dp(ghosted_id)+aux_var%dsat_dp(ghosted_id)*aux_var%den(ghosted_id))*porXvol
  enddo
...
!
! ************************************************************************** !  
subroutine RichardsAccumulation(grid,imat,aux_var,por,vol,option,Res)
 ...
 do local_id = 1, grid%nlmax
    ...
    Res(1) = aux_var%sat(ghosted_id) * aux_var%den(ghosted_id) * por(ghosted_id) * vol(local_id)
  enddo       
...
subroutine RichardsResidualPatch(snes,xx,r,realization,ierr)
  ...
  PetscReal :: Dq, fluxm, density_ave, gravity, dphi, ukvr, q
  PetscInt :: ibc
...
! now assign access pointer to local variables
  ...
  do local_id = 1, grid%nlmax  ! For each local node do...
    ...
    Res(1) = aux_vars%sat(ghosted_id) * aux_vars%den(ghosted_id) * porosity_loc_p(ghosted_id) * volume_p(local_id)
#if 0
    ...
    do iconn = 1, cur_connection_set%num_connections      
      ...
      select case(source_sink%flow_condition%pressure%itype)
        ...
        case(VOLUMETRIC_RATE_SS)  ! assume local density for now
          r_p(local_id) = r_p(local_id) - qsrc1*aux_vars%den_kg(ghosted_id)* &
                                          option%flow_dt
      end select
...
! Flow term
  if (aux_vars%sat(ghosted_id_up) > option%sir(1,icap_up) .or. aux_vars%sat(ghosted_id_dn) > option%sir(1,icap_dn)) then
    if (aux_vars%sat(ghosted_id_up) <eps) then 
      upweight=0.d0
    else if (aux_vars%sat(ghosted_id_dn) <eps) then 
      upweight=1.d0
    endif
    density_ave = upweight*aux_vars%den(ghosted_id_up)+(1.D0-upweight)*aux_vars%den(ghosted_id_dn) 

    gravity = (upweight*aux_vars%den(ghosted_id_up)*aux_vars%avgmw(ghosted_id_up) + &
              (1.D0-upweight)*aux_vars%den(ghosted_id_dn)*aux_vars%avgmw(ghosted_id_dn)) &
              * distance_gravity

    dphi = aux_vars%pres(ghosted_id_up) - aux_vars%pres(ghosted_id_dn)  + gravity

! note uxmol only contains one phase xmol
    if (dphi>=0.D0) then
      ukvr = aux_vars%kvr(ghosted_id_up)
    else
      ukvr = aux_vars%kvr(ghosted_id_dn)
    endif      
    ...
    if (ukvr>floweps) then
      ...
      q = v_darcy * cur_connection_set%area(iconn)
 ...
 !                                              2 R = R - Res_FL  
  ...
  select case(boundary_condition%flow_condition%itype(RICHARDS_PRESSURE_DOF))
    ! figure out the direction of flow
    case(DIRICHLET_BC,HYDROSTATIC_BC,SEEPAGE_BC)
      Dq = perm_dn / cur_connection_set%dist(0,iconn)
      ! Flow term
      if (aux_vars_bc(ibc)%sat(iconn) > option%sir(1,icap_dn) .or. aux_vars%sat(ghosted_id) > option%sir(1,icap_dn)) then
        upweight=1.D0
        if (aux_vars_bc(ibc)%sat(iconn) < eps) then 
          upweight=0.d0
        else if (aux_vars%sat(ghosted_id) < eps) then 
          upweight=1.d0
        endif
        density_ave = upweight*aux_vars_bc(ibc)%den(iconn)+(1.D0-upweight)*aux_vars%den(ghosted_id)
   
        gravity = (upweight*aux_vars_bc(ibc)%den(iconn)*aux_vars_bc(ibc)%avgmw(iconn) + &
                  (1.D0-upweight)*aux_vars%den(ghosted_id)*aux_vars%avgmw(ghosted_id)) &
                  * distance_gravity
       
        dphi = aux_vars_bc(ibc)%pres(iconn) - aux_vars%pres(ghosted_id) + gravity

        if (boundary_condition%flow_condition%itype(RICHARDS_PRESSURE_DOF) == SEEPAGE_BC) then
              ! flow in         ! boundary cell is <= pref
          if (dphi > 0.d0 .and. aux_vars_bc(ibc)%pres(iconn)-option%pref < eps) then
            dphi = 0.d0
          endif
        endif
   
        if (dphi>=0.D0) then
          ukvr = aux_vars_bc(ibc)%kvr(iconn)
        else
          ukvr = aux_vars%kvr(ghosted_id)
        endif      
    ...
    case(NEUMANN_BC)
      if (dabs(boundary_condition%flow_aux_real_var(RICHARDS_PRESSURE_DOF,iconn)) > floweps) then
        v_darcy = boundary_condition%flow_aux_real_var(RICHARDS_PRESSURE_DOF,iconn)
        if (v_darcy > 0.d0) then 
          density_ave = aux_vars_bc(ibc)%den(iconn)
        else 
          density_ave = aux_vars%den(ghosted_id)
        endif 
      endif
...
subroutine RichardsJacobianPatch(snes,xx,A,B,flag,realization,ierr)
  ...
  PetscReal :: v_darcy = 0.D0 

  
  PetscReal :: dden_ave_dp_up = 0.d0
  PetscReal :: dden_ave_dp_dn = 0.d0
  PetscReal :: dgravity_dden_up = 0.d0
  PetscReal :: dgravity_dden_dn = 0.d0
  PetscReal :: dphi_dp_up = 0.d0
  PetscReal :: dphi_dp_dn = 0.d0
  PetscReal :: dukvr_dp_up = 0.d0
  PetscReal :: dukvr_dp_dn = 0.d0
  PetscReal :: dq_dp_up = 0.d0
 PetscReal ::  dq_dp_dn = 0.d0
  PetscReal :: density_ave, gravity, dphi, ukvr, kvr, q
  PetscInt :: ibc
  ...
  do local_id = 1, grid%nlmax  ! For each local node do...
    ...
    call RichardsAccumDerivative(aux_vars(ghosted_id), &
                              ...
                              Jup) 
#endif       
    porXvol = porosity_loc_p(ghosted_id)*volume_p(local_id)
      
    Jup(1,1) = (aux_vars%sat(ghosted_id)*aux_vars%dden_dp(ghosted_id)+aux_vars%dsat_dp(ghosted_id)*aux_vars%den(ghosted_id))*porXvol
  ...
  do 
    ...
    do iconn = 1, cur_connection_set%num_connections      
      ...
      select case(source_sink%flow_condition%pressure%itype)
        case(MASS_RATE_SS)
        case(VOLUMETRIC_RATE_SS)  ! assume local density for now
          Jup(1,1) = -qsrc1*aux_vars%dden_dp(ghosted_id)*aux_vars%avgmw(ghosted_id)* &
                     option%flow_dt
...
! Flow term
  if (aux_vars%sat(ghosted_id_up) > option%sir(1,icap_up) .or. aux_vars%sat(ghosted_id_dn) > option%sir(1,icap_dn)) then
    ...
    density_ave = upweight*aux_vars%den(ghosted_id_up)+(1.D0-upweight)*aux_vars%den(ghosted_id_dn)
    dden_ave_dp_up = upweight*aux_vars%dden_dp(ghosted_id_up)
    dden_ave_dp_dn = (1.D0-upweight)*aux_vars%dden_dp(ghosted_id_dn)

    gravity = (upweight*aux_vars%den(ghosted_id_up)*aux_vars%avgmw(ghosted_id_up) + &
              (1.D0-upweight)*aux_vars%den(ghosted_id_dn)*aux_vars%avgmw(ghosted_id_dn)) &
              * distance_gravity
    dgravity_dden_up = upweight*aux_vars%avgmw(ghosted_id_up)*distance_gravity
    dgravity_dden_dn = (1.d0-upweight)*aux_vars%avgmw(ghosted_id_dn)*distance_gravity

    dphi = aux_vars%pres(ghosted_id_up) - aux_vars%pres(ghosted_id_dn)  + gravity
    dphi_dp_up = 1.d0 + dgravity_dden_up*aux_vars%dden_dp(ghosted_id_up)
    dphi_dp_dn = -1.d0 + dgravity_dden_dn*aux_vars%dden_dp(ghosted_id_dn)

! note uxmol only contains one phase xmol
    if (dphi>=0.D0) then
      ukvr = aux_vars%kvr(ghosted_id_up)
      dukvr_dp_up = aux_vars%dkvr_dp(ghosted_id_up)
!      dukvr_dp_dn = 0.d0
    else
      ukvr = aux_vars%kvr(ghosted_id_dn)
!      dukvr_dp_up = 0.d0
      dukvr_dp_dn = aux_vars%dkvr_dp(ghosted_id_dn)
    endif      
    ...
    if (ukvr>floweps) then
      ...
      q = v_darcy * cur_connection_set%area(iconn)
      dq_dp_up = Dq*(dukvr_dp_up*dphi+ukvr*dphi_dp_up)*cur_connection_set%area(iconn)
      dq_dp_dn = Dq*(dukvr_dp_dn*dphi+ukvr*dphi_dp_dn)*cur_connection_set%area(iconn)
  ...
  select case(boundary_condition%flow_condition%itype(RICHARDS_PRESSURE_DOF))
    ...
    case(DIRICHLET_BC,HYDROSTATIC_BC,SEEPAGE_BC)
      ...
      if (aux_vars_bc(ibc)%sat(iconn) > option%sir(1,icap_dn) .or. aux_vars%sat(ghosted_id) > option%sir(1,icap_dn)) then
        ...
        density_ave = upweight*aux_vars_bc(ibc)%den(iconn)+(1.D0-upweight)*aux_vars%den(ghosted_id)
        dden_ave_dp_dn = (1.D0-upweight)*aux_vars%dden_dp(ghosted_id)

        gravity = (upweight*aux_vars_bc(ibc)%den(iconn)*aux_vars_bc(ibc)%avgmw(iconn) + &
                  (1.D0-upweight)*aux_vars%den(ghosted_id)*aux_vars%avgmw(ghosted_id)) &
                  * distance_gravity
        dgravity_dden_dn = (1.d0-upweight)*aux_vars%avgmw(ghosted_id)*distance_gravity

        dphi = aux_vars_bc(ibc)%pres(iconn) - aux_vars%pres(ghosted_id) + gravity
        dphi_dp_dn = -1.d0 + dgravity_dden_dn*aux_vars%dden_dp(ghosted_id)
        ...
        if (dphi>=0.D0) then
          ...
          ukvr = aux_vars%kvr(ghosted_id)
          dukvr_dp_dn = aux_vars%dkvr_dp(ghosted_id)
        endif      
    ...
    case(NEUMANN_BC)
      if (dabs(boundary_condition%flow_aux_real_var(RICHARDS_PRESSURE_DOF,iconn)) > floweps) then
        ...
        if (v_darcy > 0.d0) then 
          ...
          density_ave = aux_vars%den(ghosted_id)
          dden_ave_dp_dn = aux_vars%dden_dp(ghosted_id)
        endif 
        q = v_darcy * cur_connection_set%area(iconn)
      endif
