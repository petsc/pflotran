      ...
      if (associated(coupler%flow_condition)) then
        ...
        if (associated(coupler%flow_condition%pressure)) then
          ...
          select case(option%iflowmode)
            ...
            case(TH_MODE,THC_MODE,RICHARDS_MODE,MIS_MODE)
...
subroutine PatchUpdateCouplerAuxVars(patch,coupler_list,force_update_flag, &
  ...
  do
    ...
    if (associated(coupler%flow_aux_real_var)) then
      ...
      update = PETSC_FALSE
      select case(option%iflowmode)
        case(TH_MODE,THC_MODE,MPH_MODE, MIS_MODE)
...
subroutine PatchBridgeFlowAndTransport(patch,option)
  ...
  select case(option%iflowmode)
    case(RICHARDS_MODE)
      ...
      enddo
      do iaux = 1, patch%aux%RT%num_aux
        patch%aux%RT%aux_vars_bc(count)%den(1) = &
          patch%aux%Richards%aux_vars_bc(iaux)%den_kg
        patch%aux%RT%aux_vars_bc(count)%sat = &
          patch%aux%Richards%aux_vars_bc(iaux)%sat
      enddo
#else    
      do iaux = 1, patch%aux%RT%num_aux
        patch%aux%RT%aux_vars(iaux)%den(1) = &
          patch%aux%Richards%aux_vars%den_kg(iaux)
        patch%aux%RT%aux_vars(iaux)%sat = &
          patch%aux%Richards%aux_vars%sat(iaux)
      enddo
      count = 0
!      do iaux = 1, patch%aux%RT%num_aux_bc
      do iaux = 1, patch%aux%Richards%num_aux_bc
        do i=1, patch%aux%Richards%aux_vars_bc(iaux)%n
          count = count + 1
          patch%aux%RT%aux_vars_bc(count)%den(1) = &
            patch%aux%Richards%aux_vars_bc(iaux)%den_kg(i)
          patch%aux%RT%aux_vars_bc(count)%sat = &
            patch%aux%Richards%aux_vars_bc(iaux)%sat(i)
        enddo
      enddo
#endif      
    case(TH_MODE,THC_MODE,MPH_MODE,MIS_MODE)
    ...
    case(TEMPERATURE,PRESSURE,LIQUID_SATURATION,GAS_SATURATION, &
      ...
      if (associated(patch%aux%THC)) then
        ...
        select case(ivar)
          case(TEMPERATURE)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%TH%aux_vars(grid%nL2G(local_id))%temp
            enddo
          case(PRESSURE)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%TH%aux_vars(grid%nL2G(local_id))%pres
            enddo
          case(LIQUID_SATURATION)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%TH%aux_vars(grid%nL2G(local_id))%sat
            enddo
          case(LIQUID_DENSITY)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%TH%aux_vars(grid%nL2G(local_id))%den_kg
            enddo
          case(GAS_MOLE_FRACTION,GAS_ENERGY,GAS_DENSITY) ! still need implementation
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = 0.d0
        ...
        select case(ivar)
          ...
          case(PRESSURE)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%Richards%aux_vars%pres(grid%nL2G(local_id))
            enddo
          case(LIQUID_SATURATION)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%Richards%aux_vars%sat(grid%nL2G(local_id))
            enddo
          case(LIQUID_DENSITY)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = patch%aux%Richards%aux_vars%den_kg(grid%nL2G(local_id))
            enddo
        end select
      else if (associated(patch%aux%Mphase)) then
        select case(ivar)
  ...
  select case(ivar)
    case(TEMPERATURE,PRESSURE,LIQUID_SATURATION,GAS_SATURATION, &
      ...
      if (associated(patch%aux%THC)) then
        ...
        select case(ivar)
          case(TEMPERATURE)
            value = patch%aux%TH%aux_vars(ghosted_id)%temp
          case(PRESSURE)
            value = patch%aux%TH%aux_vars(ghosted_id)%pres
          case(LIQUID_SATURATION)
            value = patch%aux%TH%aux_vars(ghosted_id)%sat
          case(LIQUID_DENSITY)
            value = patch%aux%TH%aux_vars(ghosted_id)%den_kg
          case(GAS_MOLE_FRACTION,GAS_ENERGY,GAS_DENSITY) ! still need implementation
            value = 0.d0
          case(GAS_SATURATION)
#ifdef ICE
    ...
    case(TEMPERATURE,PRESSURE,LIQUID_SATURATION,GAS_SATURATION, &
      ...
      if (associated(patch%aux%THC)) then
        ...
        select case(ivar)
          case(TEMPERATURE)
            do local_id=1,grid%nlmax
              patch%aux%TH%aux_vars(grid%nL2G(local_id))%temp = vec_ptr(local_id)
            enddo
          case(PRESSURE)
            do local_id=1,grid%nlmax
              patch%aux%TH%aux_vars(grid%nL2G(local_id))%pres = vec_ptr(local_id)
            enddo
          case(LIQUID_SATURATION)
            do local_id=1,grid%nlmax
              patch%aux%TH%aux_vars(grid%nL2G(local_id))%sat = vec_ptr(local_id)
            enddo
          case(LIQUID_DENSITY)
            do local_id=1,grid%nlmax
              patch%aux%TH%aux_vars(grid%nL2G(local_id))%den_kg = vec_ptr(local_id)
            enddo
          case(GAS_MOLE_FRACTION,GAS_ENERGY,GAS_DENSITY) ! still need implementation
          case(GAS_SATURATION)
            do local_id=1,grid%nlmax
        ...
        select case(ivar)
          ...
          case(PRESSURE)
            do local_id=1,grid%nlmax
              patch%aux%Richards%aux_vars%pres(grid%nL2G(local_id)) = vec_ptr(local_id)
            enddo
          case(LIQUID_SATURATION)
            do local_id=1,grid%nlmax
              patch%aux%Richards%aux_vars%sat(grid%nL2G(local_id)) = vec_ptr(local_id)
            enddo
          case(LIQUID_DENSITY)
            do local_id=1,grid%nlmax
              patch%aux%Richards%aux_vars%den_kg(grid%nL2G(local_id)) = vec_ptr(local_id)
            enddo
        end select
      else if (associated(patch%aux%Mphase)) then
        select case(ivar)
