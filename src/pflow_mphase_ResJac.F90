...
!   enddo
 ...
 ! print *,' update reason', grid%myrank, re,n,grid%nlmax
  ...
  subroutine MPhase_Update(xx,grid,icri,ichange)
  ...
  PetscScalar, pointer :: xx_p(:), yy_p(:),iphase_p(:),var_p(:),ithrm_p(:),icap_p(:)
  PetscScalar, pointer :: accum_p(:),volume_p(:),porosity_p(:)
  integer*4 :: n,n0,index,ipr
  integer :: ierr,iipha,iicap,i,index_var_begin,index_var_end,p1
  real*8 :: pco2,p,tmp,xw, dif(2),res(1:grid%ndof)
  real*8, pointer :: xmol(:),sat(:),den(:) 
...
! mphase code need assemble 
 ...
 select case(icri)
  case(0)
    if ((1.D0-xx_p(n0+3)) > grid%yh2o_in_co2*1.025 .and. iipha==2) then
      write(*,'('' Gas -> 2ph '',i8,1p10e12.4)') grid%nL2A(n),xx_p(n0+1:n0+3),grid%yh2o_in_co2*1.025
...
!       xx_p(n0+1)= yy_p(n0+1);xx_p(n0+2)= yy_p(n0+2)
    ...
    !if (xx_p(n0+3) > 1.025D0  .and. iipha==1) then
     if (xx_p(n0+3) > 1.025D0  .and. iipha==1) then
	     write(*,'('' Liq -> 2ph '',i8,1p10e12.4)') grid%nL2A(n),xx_p(n0+1:n0+3),xmol(3)+xmol(4),xmol(3),xmol(4)
...
!	   xx_p(n0+1)= yy_p(n0+1);xx_p(n0+2)= yy_p(n0+2)
    ...
    if(xx_p(n0+3) > 1.0D0 .and. iipha==3) then
	!if(xx_p(n0+3)> 1.D0 .and. iipha==3 )then
        write(*,'('' 2ph -> Gas '',i8,1p10e12.4)') grid%nL2A(n),xx_p(n0+1:n0+3)
...
!		xx_p(n0+1)= yy_p(n0+1);xx_p(n0+2)= yy_p(n0+2)  
   ...
   ! if(sat(2)<= -formeps .and. iipha==3 )then
     if(xx_p(n0+3) <= 0.0D0 .and. iipha==3 )then
	  write(*,'('' 2ph -> Liq '',i8,1p10e12.4)') grid%nL2A(n),xx_p(n0+1:n0+3),sat(1),sat(2)
...
!	  call 
  case(1)
      ...
      if ((1D0-xx_p(n0+3)) > grid%yh2o_in_co2 .and. iipha==2) then
      write(*,'('' Gas -> 2ph '',i8,1p10e12.4)') grid%nL2A(n),xx_p(n0+1:n0+3),grid%yh2o_in_co2
      !write(IUNIT2,'('' Gas -> 2ph '',i8,1p10e12.4)') n,xx_p(n0+1:n0+3)
      iphase_p(n) = 3
      xx_p(n0+3)=1D0-eps
	  ichange = 1 ;ipr=1
    ...
    if (xx_p(n0+3) > 1.0D0  .and. iipha==1) then
      write(*,'('' Liq -> 2ph '',i8,1p10e12.4)') grid%nL2A(n),xx_p(n0+1:n0+3),xmol(3)+xmol(4),xmol(3),xmol(4)
      !write(IUNIT2,'('' Liq -> 2ph '',i8,1p10e12.4)') n,xx_p(n0+1:n0+3)
      iphase_p(n) = 3
...
!	call pri_var_trans_mph_ninc(xx_p((n-1)*grid%ndof+1:n*grid%ndof),iipha,&
