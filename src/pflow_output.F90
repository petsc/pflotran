...
! primary variables
  if(grid%use_2ph == PETSC_TRUE .or. grid%use_mph == PETSC_TRUE) then
...
!	call VecScatterCreateToAll(t_nat, scat_1dof, t_all, ierr)
    ...
    if(grid%use_mph == PETSC_TRUE) call VecDuplicate(c_all,iphase_all,ierr)
...
!   write out initial conditions
 ...
 &      "       p      ","      T      ","     sl(g)      ","      C      ")')
  ...
  if( grid%use_mph == PETSC_TRUE)call VecDestroy(iphase_all, ierr)
        ...
        do j = 1, grid%ny
          do i = 1, grid%nx
            ...
            if (j>1 .and. j<grid%ny) then
              ...
              vaveg(ip+1) = (vl_p(2+ip*grid%nphase+(nym1-1)*3*grid%nphase))*grid%tconv
            endif
            ...
            if (k>1 .and. k<grid%nz) then
              ...
              vaveg(ip+1) = (vl_p(2+ip*grid%nphase+(nzm1-1)*3*grid%nphase))*grid%tconv
            endif
