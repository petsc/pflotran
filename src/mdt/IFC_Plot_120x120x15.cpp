...
IFC_Plot_120x120x15::IFC_Plot_120x120x15(Grid **grid_) {
...
//      if (ilayer == 0) printf("%.8f %.8f %.8f\n",x,y,zlayer);
    ...
    if (boundary_polygon) {
      if (!boundary_polygon->pointInPolygon(x,y)) {
  ...
  for (PetscInt j=0; j<8; j++) {
    ...
    for (PetscInt i=0; i<8; i++) {
      PetscReal x = 594240. + i*2.;
      PetscReal xx = x;
      PetscReal yy = y;
      grid->computeCoordinate(&xx,&yy);
      xx -= 594239.4155;
      yy -= 115982.5663;
      printf("%.8f %.8f %.8f %.8f\n",x,y,xx,yy);
    }
...
void IFC_Plot_120x120x15::computeWestBoundary(Grid *grid, PetscInt complete) {
...
void IFC_Plot_120x120x15::computeEastBoundary(Grid *grid, PetscInt complete) {
...
void IFC_Plot_120x120x15::computeSouthBoundary(Grid *grid, PetscInt complete) {
...
void IFC_Plot_120x120x15::computeNorthBoundary(Grid *grid, PetscInt complete) {
...
void IFC_Plot_120x120x15::computeBottomBoundary(Grid *grid, PetscInt complete) {
...
void IFC_Plot_120x120x15::computeTopBoundary(Grid *grid, PetscInt complete) {
...
void IFC_Plot_120x120x15::computeIFCBoundary(Grid *grid, Polygon *p) {
...
void IFC_Plot_120x120x15::flagGridCells(Grid *grid) {
...
void IFC_Plot_120x120x15::setMaterialIdBasedOnNaturalId(PetscInt natural_id, PetscInt material_id,
                                             Grid *grid) {
  for (PetscInt i=0; i<grid->getNumberOfCellsGhosted(); i++) 
...
void IFC_Plot_120x120x15::setActiveBasedOnNaturalId(PetscInt natural_id, PetscInt active,
                                         Grid *grid) {
  for (PetscInt i=0; i<grid->getNumberOfCellsGhosted(); i++) 
    if (grid->cells[i].getIdNatural() == natural_id) 
...
IFC_Plot_120x120x15::~IFC_Plot_120x120x15() {
  if (ascii_grids) {
    for (PetscInt i=0; i<AsciiGrid::nasciigrids; i++)
