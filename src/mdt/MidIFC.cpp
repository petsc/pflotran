...
//  grid->computeConnectivity();
  ...
  boundary_polygon->createMidIFCPolygon();
...
void MidIFC::computeWestBoundary(Grid *grid, PetscInt complete) {
...
void MidIFC::computeEastBoundary(Grid *grid, PetscInt complete) {
...
void MidIFC::computeSouthBoundary(Grid *grid, PetscInt complete) {
...
void MidIFC::computeNorthBoundary(Grid *grid, PetscInt complete) {
...
void MidIFC::computeBottomBoundary(Grid *grid, PetscInt complete) {
...
void MidIFC::computeTopBoundary(Grid *grid, PetscInt complete) {
...
void MidIFC::computeIFCBoundary(Grid *grid, Polygon *p) {
...
void MidIFC::flagGridCells(Grid *grid) {
...
void MidIFC::setMaterialIdBasedOnNaturalId(PetscInt natural_id, PetscInt material_id,
                                             Grid *grid) {
  for (PetscInt i=0; i<grid->getNumberOfCellsGhosted(); i++) 
...
void MidIFC::setActiveBasedOnNaturalId(PetscInt natural_id, PetscInt active,
                                         Grid *grid) {
  for (PetscInt i=0; i<grid->getNumberOfCellsGhosted(); i++) 
    if (grid->cells[i].getIdNatural() == natural_id) 
