...
+ \frac{\Delta t_k}{\varphi_\b + \Delta t_k\Gamma_{\a\b}}\Gamma_{\a\b}\Psi_{jn}^{\a(k+1)}.
\EA
This equation expresses $\Psi_{jn}^{\b(k+1)}$ in terms of $\Psi_{jn}^{\a(k+1)}$, however, the right-hand side is still a function of $C_{jn}^{\b(k+1)}$ and therefore complete elimination of the $\b$-continuum variables from the primary continuum is not possible in the nonlinear case.
