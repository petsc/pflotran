...
\EQ
R_{jn} \eq \varphi \frac{\Psi_{jn}^{t+\Delta t}-\Psi_{jn}^t}{\Delta t}V_n + \sum_\a \frac{q_{j\a n}^{t+\Delta t}-q_{j\a n}^t}{\Delta t}V_n + \sum_{n'} \Omega_{jnn'} A_{nn'} + \sum_m\nu_{jm}I_{mn}V_n,
\EN
with control volume $V_n$ and interfacial area $A_nn'$, becomes
\EQ
R_{jn} \eq \varphi \frac{\Psi_{jn}^{t+\Delta t}-\Psi_{jn}^t}{\Delta t}V_n + \sum_\a \frac{k_\a}{1+k_\a\Delta t}\big(q_{j\a n}^{{\rm eq},\,t+\Delta t}-q_{j\a n}^{t}\big)V_n + \sum_{n'} \Omega_{jnn'} A_{nn'} + \sum_m\nu_{jm}I_{mn}V_n,
\EN
