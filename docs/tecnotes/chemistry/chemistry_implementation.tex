...
This formulation is considered only for surface complexation reactions, but ion exchange could also be implemented if desired. Consider surface complexation reactions of the form
\EQ
>\!\chi_\a + \sum_j\nu_{ji} \A_j \arrows >\! S_{i\a},
\EN
with the rate of reaction described through a kinetic relation given by
\EQ\label{sorbed}
\frac{\p S_{i\a}}{\p t} \eq k_\a^{} \big(S_{i\a}^{\rm eq}-S_{i\a}\big),
\EN
for sorbed concentrations, and
\EQ\label{fsite}
...
\EN
where $\omega_\a$ denotes the total site concentration of type $\a$ given by
\EQ\label{totsite}
\omega_\a \eq S_\a + \sum_i S_{i\a}.
\EN
The site concentration $\omega_\a$ is constant as follows by differentiating Eqn.\eqref{totsite} using Eqns.\eqref{sorbed} and \eqref{fsite} to give
\EQ
\frac{\p\omega_\a}{\p t} \eq 0.
\EN
...
\EQ
\frac{\p}{\p t}\varphi \Psi_j + \bnabla\cdot\bOmega_j \eq -\sum_{i\a}\nu_{ji}\frac{\p S_{i\a}}{\p t},
\EN
or
\EQ
\frac{\p}{\p t}\left(\varphi \Psi_j + \sum_{i\a}\nu_{ji} S_{i\a}\right) + \bnabla\cdot\bOmega_j \eq 0.
\EN
...
\EQ
\frac{\p}{\p t}\left(\varphi \Psi_j + \sum_{\a}q_{j\a}\right) + \bnabla\cdot\bOmega_j \eq 0.
\EN
