...
\EQ
\Omega_{jn}^{\a\b} \eq -\varphi_\b D_\b \left.\frac{\p\Psi_j^\b}{\p y}\right|_{y=0} \eq -\varphi_\b D_\b \left(\frac{\Psi_j^\a-\Psi_j^\b}{d_{\a\b}}\right),
\EN
...
\EQ
\sum_\b a_{\a\b} \Omega_j^{\a\b} \eq \sum_\b \Gamma_{\a\b} \big(\Psi_j^\a-\Psi_j^\b\big),
\EN
where
\EQ
...
The sub-grid model involves solving a block-tridiagonal system of nonlinear algebraic equations for each primary continuum node $n$.
The residual function for the subgrid domain for the $M$th node neighboring the interface to the primary continuum fluid has the form (see Appendix)
\begin{subequations}
\BA
\bR_{nM}^{\b}&\eq\frac{\varphi V_M}{\Delta t}\big(\bPsi_M^{\b,t+\Delta t}-\bPsi_M^{\b t}\big)+\bOmega_M^{\b +} A_M^+ - \bOmega_M^{\b -} A_M^- + V_M \sum_s\bnu_{s}^{} I_{sM}^\b,\\
&\eq\widetilde\bR_{nM}^{\b}-\frac{\varphi DA_M^+}{d_M}\bPsi_n^\a,
\EA
\end{subequations}
where $\widetilde\bR_{nM}^{\b}$ denotes the contribution to the residual function which does not contain the boundary concentration equal to the primary continuum concentration $\bPsi_n^\a$ at the $n$th node. 

The quantity $\bz_M$ defined in the Appendix in Eqn.~\eqref{zn} then becomes
\begin{subequations}
\BA
\bz_M&\eq -\bD_M^{-1}\big(\widetilde\bR_M + \bbeta_M\bz_{M-1}\big) + \frac{\varphi D A_M^+}{d_M} \bD_M^{-1}\bPsi_n^\a,\\
&\eq \ba_n+\bb_n\bPsi_n^\a,
\EA
\end{subequations}
with known quantities $\ba_n$ and $\bb_n$ defined as
\begin{subequations}
\BA
\ba_n&\eq -\bD_M^{-1}\big(\widetilde\bR_M + \bbeta_M\bz_{M-1}\big),\\
\bb_n&\eq -\frac{\varphi DA_M^+}{d_M}\bD_M^{-1}.
\EA
\end{subequations}

The three possible cases for the choice of $\d\bx$ are considered in turn.

\subsubsection{Case I: $\d x_j = \d C_j$}

For the situation where $\d x_j \!=\! \d C_j$, $C_{jM}^{(k+1)}$ is found from the relation
\EQ
C_{jM}^{(k+1)}\eq C_{jM}^{(k)} + a_{jn} + \sum_l b_{jln} \Psi_{ln}^\a.
\EN
The residual function and Jacobian for the primary continuum involves the total concentration
\EQ
\Psi_{jM}^{(k+1)}\eq C_{jM}^{(k+1)}(\Psi_{jn}^\a) + \sum_i\nu_{ji} \C_{iM} [C_{jM}^{(k+1)}(\Psi_{jn}^\a)],
\EN
with derivative
\EQ
\frac{\p\Psi_{jM}^{(k+1)}}{\p C_{kn}^\a}\eq \frac{\p C_{jM}^{(k+1)}}{\p C_{kn}^\a}+ \sum_i\nu_{ji} \frac{\p\C_{iM}^{(k+1)}}{\p C_{kn}^{b}}.
\EN

\subsubsection{Case II: $\d x_j = \d{\rm ln} C_j$}
...
\EQ
\ln C_{jM}^{(k+1)}\eq\ln C_{jM}^{(k)} + a_{jn} + \sum_l b_{jln} \Psi_{ln}^\a,
\EN
or alternatively
\EQ
C_{jM}^{(k+1)}\eq C_{jM}^{(k)} \e^{a_{jn} + \sum_l b_{jln} \Psi_{ln}^\a}.
\EN
The total matrix concentration can now be written as a function of the total bulk fluid concentration as
\EQ
\Psi_{jM}^{(k+1)}\eq C_{jM}^{(k+1)}(\Psi_{jn}^\a) + \sum_i\nu_{ji} \C_{iM} \big[C_{jM}^{(k+1)}(\Psi_{jn}^\a)\big].
\EN
...
\BA
\frac{\p C_{jM}^{(k+1)}}{\p\ln C_{kn}^\a}&\eq C_{jM}^{(k+1)} \sum_l b_{jln}\frac{\p\Psi_{jn}^\a}{\p\ln C_{kn}^\a},\\
&\eq C_{jM}^{(k+1)} \sum_l b_{jln}\left(C_{jn}^\a\delta_{jk}+\sum_i\nu_{ji}\nu_{ki}\C_{in}^\a\right),
\EA
making use of the relation
\EQ
\frac{\p\Psi_{jn}^\a}{\p\ln C_{kn}^\a}\eq C_{jn}^\a\delta_{jk}+\sum_i\nu_{ji}\nu_{ki}\C_{in}^\a.
\EN
Further one has
\EQ
\frac{\p\Psi_{jM}^{(k+1)}}{\p\ln C_{kn}^\a}\eq \frac{\p\ln C_{jM}^{(k+1)}}{\p C_{kn}^\a}+ \sum_i\nu_{ji} \frac{\p\C_{iM}^{(k+1)}}{\p\ln C_{kn}^\a},
\EN
with
\BA
\frac{\p\C_{iM}^{(k+1)}}{\p\ln C_{kn}^{b}}&\eq\C_{iM}^{(k+1)}\sum_j\nu_{ji}\frac{\p C_{jM}^{(k+1)}}{\p\ln C_{kn}^\a},\\
&\eq\C_{iM}^{(k+1)}\sum_j\nu_{ji} C_{jM}^{(k+1)} \sum_l b_{jln}\left(C_{jn}^\a\delta_{jk}+\sum_i\nu_{ji}\nu_{ki}\C_{in}^\a\right).
\EA

For the backward solve
\EQ
\d\ln C_{jM}\eq z_{jM}\eq a_{jn}+\sum_lb_{jln}\Psi_{ln}^\a,
\EN
\EQ
\d\ln C_{jn} \eq z_{jn}-\sum_lU_{jln}\d\ln C_{l,n+1}, \ (n=M-1, \, \ldots, \, 1).
\EN

\subsubsection{Case III: $\d x_j = \d\Psi_j$}

For the case $\d x_j = \d\Psi_j$
\EQ
\Psi_{jM}^{(k+1)}\eq \Psi_{jM}^{(k)} + a_{jn} + \sum_l b_{jln} \Psi_{ln}^\a.
\EN
The derivative is given by
\EQ
\frac{\p\Psi_{jM}^{(k+1)}}{\p C_k^\a}\eq \sum_l b_{jln} \frac{\p\Psi_{ln}^\a}{\p C_k^\a}.
\EN
...
For an effective one-dimensional, multicomponent system the finite volume discretized equations have the form of a block-tridiagonal system of equations. The residual function has the form
\EQ
...
\EA
\end{subequations}
The area $A_n^\pm$ and volume $V_n$ determines the geometry of the system. 
...
\EN
The molality is related to the mole fraction by the equation
\begin{subequations}
\EQ
...
\BA
C_\c^g &\eq \frac{n_i^g}{V_g} \eq \frac{n_i^g}{N_g}\frac{N_g}{V_g} \eq \rho_\c y_i^g,\\
&\eq \rho_\c \frac{f_\c}{\phi_\c P_\c},\\
