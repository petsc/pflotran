...
\left(\frac{S_{il}^{\a(k+1)}-S_{il}^{\a(k)}}{\Delta t_k}\right) \nonumber \\
...
\left(\frac{S_{il}^{\b(k+1)}-S_{il}^{\b(k)}}{\Delta t_k}\right) \nonumber \\
...
- \sum_{il} \nu_{ji} \big(S_{il}^{\b(k+1)} - S_{il}^{\b(k)}\big)\right] 
\nonumber\\
& - \frac{\Delta t_k}{\varphi_\b + \Delta t_k\Gamma_{\a\b}} \sum_m\nu_{jm} I_{mn}^{\b,k+1} 
+ \frac{\Delta t_k \Gamma_{\a\b}}{\varphi_\b + \Delta t_k\Gamma_{\a\b}}\Psi_{jn}^{\a(k+1)}.
\EA
This equation expresses $\Psi_{jn}^{\b(k+1)}$ in terms of $\Psi_{jn}^{\a(k+1)}$, however, the right-hand side is still a function of $C_{jn}^{\b(k+1)}$ and therefore complete elimination of the $\b$-continuum variables from the primary continuum in the nonlinear case is not possible.

Writing
\EQ
\Psi_{jn}^{\b(k+1)} \eq f_{jn}^\b(C^{\b(k+1)}) + g_{\a\b}\Psi_{jn}^{\a(k+1)},
\EN
the residual for the primary continuum equations become
\EQ
R_{jn}^\a (C^\a,\,C^\b) \eq \widetilde R_{jn}^\a + \Gamma_{\a\b} (1-g_{\a\b}) \Psi_{jn}^{\a(k+1)} - \Gamma_{\a\b} f_{jn}^\b,
\EN
...
+ \frac{\p R_{jn}^\b}{\p C_{lm}^\a}\, \delta\ln C_{lm}^\b\right] \eq -R_{jn}^\b.
\EA
