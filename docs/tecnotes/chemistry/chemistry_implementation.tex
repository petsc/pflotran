...
\EN
where $S_{\i\a}$ denotes the sorbed concentration for the $i$th surface complex on site $\a$. For reversible reaction the forward and backward rate constants, $k_{i\a}^f$, $k_{i\a}^b$ satisfy the relation
%\footnote{Should the selectivity coefficienty $K_i$ also depend on the site index $\a$?}
\EQ
K_{i\a} \eq \frac{k_{i\a}^f}{k_{i\a}^b},
\EN
...
\EQ\label{sfree1}
S_\a^{k+1} \eq \sum_l S_{l\a}^{k+1} - \omega_\a^{k}.
\EN
\end{subequations}
...
S_\a^{k+1} \eq \frac{
\omega_\a - 
\displaystyle\sum_l \dfrac{S_{l\a}^{k}}{1+k_{l\a}^b\Delta t}
}
...
\frac{
\omega_\a^k - \displaystyle\sum_l S_{l\a}^{k}
}
{1+\displaystyle\sum_l k_{l\a}^f\Delta t Q_l^{k+1}}
\right] Q_i^{k+1}.
\EN
...
In this expression
\EQ
N_\a \eq\omega_\a^k - \displaystyle\sum_l \dfrac{S_{l\a}^{k}}{1+k_{l\a}^b\Delta t},
\EN
and
\EQ
D \eq 1+\displaystyle\sum_l\dfrac{k_{l\a}^f\Delta t}{1+k_{l\a}^b\Delta t} Q_l^{k+1}.
\EN
The contribution to the Jacobian is given by
\BA
\frac{\p R_{jn}}{\p \ln C_l^{k+1}} &\eq \frac{1}{D} \sum_{i\a}
\frac{\nu_{ji} k_{i\a}^f N_\a}{1+k_{i\a}^b \Delta t} \left\{
\frac{\p Q_i^{k+1}}{\p\ln C_l^{k+1}}-\frac{Q_i^{k+1}}{D}
\displaystyle\sum_{l'}\dfrac{k_{l'\a}^f\Delta t}{1+k_{l'\a}^b\Delta t} \frac{\p Q_{l'}^{k+1}}{\p\ln C_l^{k+1}}
\right\},
\EA
with
\EQ
