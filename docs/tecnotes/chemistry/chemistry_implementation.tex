...
\EN
where the distribution coefficients, $K_j^s$ for the stationary solid phase including contributions from minerals and colloids, and $K_j^l$ for colloids present in the aqueous phase are defined by
\EQ
...
>\!X_c^{l,s} + \displaystyle\sum_i\nu_{ji}^c\A_j^{} &\!\!\!\!\!\arrows\! >\!A_{ic}^{l,s}, & \text{(Sorption: $I_{ic}^{l,s}$)},
\end{array}
\EN
for mobile $(l)$ and immobile $(s)$ colloids where $l$ and $s$ refer to liquid and solid phases, respectively.
...
&\eq \varphi s_l^{} \Big(S_c^l+ \sum_{i=1}^{N_c^{\rm srf}} S_{ic}^{l}\Big) + S_c^s + \sum_{i=1}^{N_c^{\rm srf}} S_{ic}^{s},\\
&\eq \varphi s_l^{} S_c^l + S_c^s 
+ \sum_{i=1}^{N_c^{\rm srf}} \big(\varphi s_l^{} S_{ic}^{l}+S_{ic}^{s}\big),\\
...
\EQ
\frac{\p \omega_c^s}{\p t} \eq \Gamma_c I_c.
\EN
...
\begin{subequations}
\BA
S_{ic}^{\m,\,\im} &\eq K_{ic}^{} Q_{ic}^{} S_c^{l,s},\\
