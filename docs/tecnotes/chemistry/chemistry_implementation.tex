...
\EN
\EQ
\frac{\p C_c^s}{\p t} \eq k_c^f C_c^s - k_c^b C_c^s,
\EN
$l=$liq, $s=$solid.
...
\EN
\EQ
I_{ic}^{l,\,s} \eq k_{ic}^f K_{ic} Q_i S_{c}^{l,\,s} - k_{ic}^b S_{ic}^{l,\,s}.
\EN
\noindent Alternatively:
\BA
\frac{\p}{\p t} \left[\varphi s_l\Psi_j + \varphi\sum_{ic} \nu_{ji} S_{ic}^l + \sum_{ic} \nu_{ji} S_{ic}^s + \sum_{m} \nu_{jm} S_{m}^{} \right] +\bnabla\cdot\left[\bOmega_j + \sum_{ic}\bq_c\nu_{ji} S_{ic}^l \right] \eq \nonumber\\
-\sum_m\nu_{jm}I_m
\EA
...
\bJ \delta\bx \eq -\bR
\EN
\noindent Equilibrium Sorption Reactions:
\EQ
...
\noindent Kinetic Sorption Reactions:
\EQ
\left(\begin{array}{cc} \dfrac{\p R_{j}}{\p\ln C_{l}} & \dfrac{\p R_{j}}{\p\ln S_{i'c'}} \\
\dfrac{\p R_{ic}}{\p\ln C_{l}} & \dfrac{\p R_{ic}}{\p\ln S_{i'c'}} \\
\end{array}\right)
\left[
\begin{array}{c}
\delta\ln C_j\\
...
\item srf\_name
\begin{deflist}{spacingspacingspacingspacingspace}
...
\item[MOBILE] coll\_name \ conc [mol/m$^3$]
\item[IMMOBILE] coll\_name \ conc [mol/m$^3$]
\end{deflist}
\item[END] ~
\end{deflist}
