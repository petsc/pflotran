...
\EN
for a fully backward implicit time differencing. Solving for $q_{j\a}$ at the new time $t+\Delta t$ gives
\EQ\label{qkin}
...
\EN
where $q_{j\a}^{\rm eq}$ is given explicitly as a function of the primary species concentrations in Eqn.\eqref{qeq}. Once the primary species transport equations are solved, the sorbed concentrations can be updated from Eqn.\eqref{qkin}.
