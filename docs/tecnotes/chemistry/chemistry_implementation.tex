...
- \sum_{il} \nu_{ji} \big(S_{il}^{\b(k+1)} \!\!\!\!\!\!- S_{il}^{\b(k)}\big) - \Delta t_k \sum_m\nu_{jm} I_{mn}^{\b,k+1} \right] 
\nonumber\\
...
Writing
\EQ
\Psi_{jn}^{\b(k+1)} \eq e_{jn}^\a + f_{jn}^\b(C^{\b(k+1)}) + g_{\a\b}\Psi_{jn}^{\a(k+1)},
\EN
where
\EQ
e_{jn}^\a \eq \frac{(\varphi_\b\Psi_j^\b)_n^k}{\varphi_\b + \Delta t_k\Gamma_{\a\b}},
\EN
\EQ
...
\EQ
R_{jn}^\a (C^\a,\,C^\b) \eq \widetilde R_{jn}^\a + \Gamma_{\a\b} (1-g_{\a\b}) \Psi_{jn}^{\a(k+1)} - \Gamma_{\a\b} \big(e_{jn}^\a + f_{jn}^\b\big),
\EN
