...
\EN
\EQ
\frac{\p}{\p t} \varphi s_l C_c^l + \bnabla\cdot\bq C_c^l \eq -\frac{\p C_c^s}{\p t},
\EN
\EQ
\frac{\p C_c^s}{\p t} \eq k_c^f C_c^l - k_c^b C_c^s,
\EN
$l=$ liq, $s=$ solid. Alternatively, the mobile colloid equation can be written as
\EQ
\frac{\p}{\p t} \Big[\varphi s_l C_c^l + C_c^s\Big] + \bnabla\cdot\bq C_c^l \eq 0.
\EN

\noindent
Primary Species Concentrations:
\EQ
\frac{\p}{\p t} \varphi s_l\Psi_j +\bnabla\cdot\bOmega_j \eq - \sum_{ic} \nu_{ji} \frac{\p\varphi s_l S_{ic}^l}{\p t} - \sum_{ic} \nu_{ji} \frac{\p S_{ic}^s}{\p t} - \sum_{m} \nu_{jm} \frac{\p S_{m}^{}}{\p t} - \sum_m\nu_{jm}I_m
\EN
\noindent Kinetic Sorption Equations---Colloids:\\
\noindent
mobile:
\EQ
\frac{\p}{\p t} \varphi s_l S_{ic}^l + \bnabla\cdot\bq_c S_{ic}^l \eq I_{ic}^l,
\EN
immobile:
\EQ
\frac{\p}{\p t} S_{ic}^s \eq I_{ic}^s,
\EN
kinetic rate law:
\BA
...
\BA
R_{jn} &\eq \left[\varphi s_l \big(\Psi_{jn}^{k+1}-\Psi_{jn}^k\big) 
+ \varphi s_l \sum_{ic}\nu_{ji} \big(S_{ic}^{l,k+1}-S_{ic}^{l,k}\big)\right. \nonumber\\
&\qquad\left. + \sum_{ic} \nu_{ji}\big(S_{ic}^{s,k+1}-S_{ic}^{s,k}\big) 
+ \sum_{i} \nu_{ji}\big(S_i^{m,k+1}-S_i^{m,k}\big) \right]\frac{V_n}{\Delta t}\nonumber \\
&\qquad\qquad + \sum_{n'} \Omega_{jnn'} A_{nn'} + V_n\sum_m\nu_{jm}I_m,\\
...
\Big]\frac{V_n}{\Delta t} + \sum_{n'} q_{cnn'} S_{ic,nn'}^l A_{nn'} - I_{ic,n}^l V_n
\EA
\noindent Auxillary Equations:
\BA
C_{cn}^{s,k+1}-C_{cn}^{s,k} & \eq \frac{1}{\Delta t} \frac{k_c^f C_c^{l,k+1}-k_c^b C_c^{s,k}}{1+k_c^f\Delta t},\\
S_i^{m,k+1}-S_i^{m,k} &\eq \frac{k_{i\a}^b S_{i\a}^k}{1+k_{i\a}^b\Delta t} + \frac{k_{i\a}^f}{1+k_{i\a}^b \Delta t} \left(
\frac{N_\a}{D_\a}
\right) Q_i^{k+1},\\
S_{ic}^{s,k+1}-S_{ic}^{s,k} &\eq \frac{k_{i\a}^b S_{i\a}^k}{1+k_{i\a}^b\Delta t} + \frac{k_{i\a}^f}{1+k_{i\a}^b \Delta t} \left(
\frac{N_\a}{D_\a}
\right) Q_i^{k+1}.
\EA
where
...
\EN
\BA
\dfrac{\p R_{j}}{\p\ln C_{j'}}&\eq\\
\dfrac{\p R_{j}}{\p\ln C_{c'}}&\eq \delta_{jc'}\\
\dfrac{\p R_{j}}{\p\ln S_{i'c'}}&\eq
\EA
\BA
\dfrac{\p R_{c}}{\p\ln C_{j'}}&\eq\\
\dfrac{\p R_{c}}{\p\ln C_{c'}}&\eq\\
\dfrac{\p R_{c}}{\p\ln C_{i'c'}}&\eq
\EA
\BA
\dfrac{\p R_{ic}}{\p\ln C_{j'}}&\eq\\
\dfrac{\p R_{ic}}{\p\ln C_{c'}}&\eq\\
\dfrac{\p R_{ic}}{\p\ln S_{i'c'}}&\eq
\EA
