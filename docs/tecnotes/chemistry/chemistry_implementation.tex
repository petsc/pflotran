...
\EQ
\frac{\p}{\p t} \varphi s_l^{} C_j^l + \bnabla\cdot C_j^l \bv_j^l \eq -\sum_i \nu_{ji}I_i^l -\sum_i\nu_{ji}I_i^\a - \sum_m \nu_{jm}I_m,
\EN
and for secondary species $\A_i^\a$ for other fluid phases including $\a \!=\! l$
\EQ
\frac{\p}{\p t} \varphi s_l^{} C_i^\a + \bnabla\cdot C_i^\a \bv_i^\a \eq I_i^\a.
\EN
In these equations $\bv_k$ denotes the average velocity of the $k$th species including contributions from both advection and diffusion.
...
&+ \bnabla\cdot\left[ \big(C_j^l \bv_j^l + \sum_i \nu_{ji} C_i^l \bv_i^l\big) + \sum_{i,\,\a\ne l} \nu_{ji} C_i^\a \bv_i^\a \right] \eq -\sum_m \nu_{jm}I_m.
\EA
...
\EQ
\rho \bv_\a \eq \sum_k\rho_k^\a \bv_k^\a.
\EN
...
&\qquad + \bnabla\cdot\left[ \big(\rho_j^l \bv_j^l + W_j \sum_i \nu_{ji} W_i^{-1} \rho_i^l \bv_i^l\big) + W_j\sum_{i,\,\a\ne l} \nu_{ji} W_i^{-1}\rho_i^\a \bv_i^\a \right] \nonumber\\
&\qquad\qquad\eq -W_j\sum_m \nu_{jm}I_m.
\EA
Summing this equation over $j$ leads directly to Eqn.\eqref{totmassbal}.

Given a reference velocity $\bomega_\a$ for phase $\a$, the average species fluid velocity $\bv_k^\a$ can be separated into contributions from advection and diffusion according to the relation
\EQ
C_k^\a \bv_k^\a \eq C_k^\a \bomega_\a + \bJ_k^\a,
\EN
where the diffusive flux $\bJ_k^\a$ is defined as
\EQ
\bJ_k^\a \eq C_k^\a \big(\bv_k^\a - \bomega_\a\big),
\EN
with
\EQ
\bomega_\a \eq \sum_k \omega_k^\a \bv_k^\a,
\EN
and
\EQ
\sum_k\omega_k^\a \eq 1.
\EN
The diffusive flux satisfies the relation
\EQ
\sum_k\frac{\omega_k^\a}{C_k^\a}\bJ_k^\a \eq 0.
\EN
...
\midrule
Barycentric & $\dfrac{1}{\rho_\a}\sum_k\rho_k^\a\bv_k^\a$ & $\chi_k^\a\!=\!\rho_k^\a/\rho_\a$\\
Molecular & $\dfrac{1}{\eta_\a}\sum_k C_k^\a \bv_k^\a$ & $x_k^\a\!=\!C_k^\a/\eta_\a$\\
Volumetric (Fickian) & $\sum_k C_k^\a \bar{V}_k^\a \bv_k^\a$ & $\phi_k^\a\!=\!C_k^\a \bar{V}_k^\a$\\
Hittorf & $\bv_1^\a$ & 1, $k$=1; 0 otherwise\\
\bottomrule[1.5pt]
\end{tabular}
\end{table}
