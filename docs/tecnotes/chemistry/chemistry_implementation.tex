...
\EQ
\frac{\p}{\p t} \varphi s_l\Psi_j +\bnabla\cdot\bOmega_j \eq - \sum_{ic} \nu_{ji}^c I_{ic}^l - \sum_{ic} \nu_{ji}^c I_{ic}^s - \sum_{im} \nu_{ji}^m I_i^m - \sum_m\nu_{jm}I_m
\EN
\noindent Kinetic Sorption Equations---Colloids:\\
\noindent
mobile:
\EQ
\frac{\p}{\p t} \varphi s_l^{} S_{ic}^l + \bnabla\cdot\big(\bq_c S_{ic}^l\big) \eq -\widetilde I_c^{} + I_{ic}^l,
\EN
\EQ
\frac{\p}{\p t} \varphi s_l^{} S_{c}^l + \bnabla\cdot\big(\bq_c S_{c}^l\big) \eq -\widetilde I_c^{} - \sum_i I_{ic}^l,
\EN
immobile:
\EQ
\frac{\p S_{ic}^s}{\p t} \eq \widetilde I_c^{} + I_{ic}^s,
\EN
\EQ
\frac{\p S_{c}^s}{\p t} \eq \widetilde I_c^{} - \sum_i I_{ic}^s,
\EN
kinetic rate law:
\BA
I_{ic}^{l,\,s} &\eq k_{ic}^f Q_{ic}^{} S_{c}^{l,\,s} - k_{ic}^b S_{ic}^{l,\,s},\\
&\eq k_{ic}^b \big(K_{ic}^{} Q_{ic}^{} S_{c}^{l,\,s} - S_{ic}^{l,\,s}\big),
\EA
\EQ
...
Summing:
\EQ
\frac{\p}{\p t} \Big[ \varphi s_l^{} S_c^l + S_c^s + \sum_i \big(\varphi s_l^{} S_{ic}^l + S_{ic}^s\big)\Big] + \bnabla\cdot \Big[\bq_c \big( S_c^l + \sum_i S_{ic}^l\big)\Big] \eq 0,
\EN
or, in terms of $\omega_c^{l,s}$:
\EQ
\frac{\p}{\p t} \Big( \varphi s_l^{} \omega_c^l + \omega_c^s \Big) + \bnabla\cdot\bq_c \omega_c^l \eq 0,
\EN
...
\EQ
\omega_c^{l,s} \eq S_c^{l,s}+ \sum_iS_{ic}^{l,s}\eq \Gamma_c^{} C_c^{l,s},
\EN
\EQ
I_{ic}^{l,\,s} \eq k_{ic}^f Q_{ic}^{} \Big(\Gamma_c^{} C_{c}^{l,\,s}-\sum_iS_{ic}^{l,s}\big) - k_{ic}^b S_{ic}^{l,\,s},
\EN
\EQ
I_c \eq k_c^f C_c^l - k_c^b C_c^s,
\EN
...
\BA
\frac{\p\omega_c^s}{\p t} &\eq \frac{\p S_c^s}{\p t} + \sum_{i=1}^{N_c^{\rm srf}} \frac{\p S_{ic}^s}{\p t},\\
...
\begin{subequations}
\BA
S_{ic}^{l,s} &\eq K_{ic}^{} Q_{ic}^{} S_c^{l,s},\\
C_c^l &\eq K_{ls}^{} C_c^s.
\EA
\EQ
S_c^{l,s} \eq \frac{\omega_c^{l,s}}{1+\sum_i K_{ic} Q_{ic}},
\EN
\EQ
S_{ic}^{l,s} \eq \frac{\omega_c^{l,s} K_{ic} Q_{ic}}{1+\sum_i K_{ic} Q_{ic}},
\EN
\EQ
\omega_c^{l,s} \eq S_c^{l,s} + \sum_i S_{ic}^{l,s}.
\EN
\end{subequations}
\noindent Filtration Equilibrium:
\EQ
K_{ls}^{} \eq \frac{C_c^l}{C_c^s},
\EN
\EQ
\frac{S_c^l}{S_c^s} \eq \frac{S_{ic}^l}{S_{ic}^s} \eq \frac{\omega_c^l}{\omega_c^s} \eq \frac{C_c^l}{C_c^s} \eq K_{ls}^{}.
\EN
\EQ
\omega_c \eq \big(1+\varphi s_l K_{ls}\big) \omega_c^s 
\eq \left(\frac{1+\varphi s_l K_{ls}}{K_{ls}}\right) \omega_c^l.
%\big(1+\sum_i K_{ic}^{} Q_{ic}^{}\big) S_c^s.
\EN
\begin{subequations}
\BA
S_c^s &\eq \frac{\omega_c}{\big(1+\varphi s_l K_{ls}\big)\big(1+\sum_i K_{ic} Q_{ic}\big)},\\
S_{ic}^s &\eq \frac{\omega_c K_{ic} Q_{ic}}{\big(1+\varphi s_l K_{ls}\big)\big(1+\sum_{i'} K_{i'c} Q_{i'c}\big)},\\
S_c^l &\eq \frac{\omega_c K_{ls}}{\big(1+\varphi s_l K_{ls}\big)\big(1+\sum_i K_{ic} Q_{ic}\big)},\\
S_{ic}^l &\eq \frac{\omega_c K_{ls} K_{ic} Q_{ic}}{\big(1+\varphi s_l K_{ls}\big)\big(1+\sum_{i'} K_{i'c} Q_{i'c}\big)}.
\EA
\end{subequations}

In the absence of mineral surface complexation reactions it follows that
\EQ
K_j^l \eq K_{ls}^{} K_j^s.
\EN
Thus if $K_j^l\gg1$,
\EQ
R_j\eq 1+\frac{K_j^s}{K_j^l} \eq 1+K_{ls}^{-1}.
\EN
...
\midrule
Independent Variables: & $\{C_j, \, C_c^l, \, S_{ic}^l\}$ \\
Dependent Variables: & $\{C_i^{}, \, C_c^{s}, \, S_c^{l,s}, \, S_{ic}^s, \, S_m^{},\,S_{im}^{}\}$ \\
\midrule
\multicolumn{2}{c}{Sorption Equilibrium Formulation -- Kinetic Filtration}\\
\midrule
Independent Variables: & $\{C_j, \, C_c^l\}$ \\
Dependent Variables: & $\{C_i^{}, \, C_c^s, \, S_c^{l,s}, \, S_{ic}^{l,s}, \, S_m^{},\,S_{im}^{}\}$ \\
\bottomrule
\end{tabular}
\end{center}
...
+ \varphi_n s_{ln}^{} \sum_{ic}\nu_{ji}^{c} \big(S_{icn}^{l,k+1}-S_{icn}^{l,k}\big)\right. \nonumber\\
&\qquad\left. + \sum_{ic} \nu_{ji}^{c}\big(S_{icn}^{s,k+1}-S_{icn}^{s,k}\big) 
+ \sum_{im} \nu_{ji}^{m}\big(S_{in}^{m,k+1}-S_{in}^{m,k}\big) \right]\frac{V_n}{\Delta t}\nonumber \\
&\qquad\qquad + \sum_{n'} \Big[\Omega_{jnn'}^{k+1} + q_{cnn'} \sum_{ic}\nu_{ji}^{} S_{ic,nn'}^{l,k+1} \Big] A_{nn'}^{} + V_n^{}\sum_m\nu_{jm}^{}I_{mn}^{k+1},
\EA
\noindent Mobile Colloids:
\EQ
R_{cn} \eq \Big[\varphi_n^{} s_{ln}^{} \big(C_{cn}^{l,k+1}-C_{cn}^{l,k}\big) 
+ C_{cn}^{s,k+1}-C_{cn}^{s,k} \Big]\frac{V_n}{\Delta t} + \sum_{n'} q_{cnn'} C_{cnn'}^{l,k+1} A_{nn'}^{},
\EN
\noindent Mobile Colloid Sorbed Species:
\EQ
R_{ic,\,n} \eq \Big[\varphi s_l^{} \big(S_{ic,n}^{l,k+1}-S_{ic,n}^{l,k}\big) 
\Big]\frac{V_n}{\Delta t} + \sum_{n'} q_{cnn'}^{} S_{ic,nn'}^l A_{nn'}^{} + \Big(\widetilde I_{cn}^{} - I_{ic,n}^l\Big) V_n^{}.
\EN
\end{subequations}
...
S_{ic}^{s,k+1}-S_{ic}^{s,k} &\eq \frac{k_{ic}^b S_{ic}^{s,k}}{1+k_{ic}^b\Delta t} 
...
S_c^{l,s} &\eq \omega_c^{l,s} - \sum_i S_{ic}^{l,s},\\
...
\begin{subequations}
\EQ
N_c \eq\omega_c^{s,k+1} - \displaystyle\sum_{i} \dfrac{S_{ic}^{s,k}}{1+k_{ic}^b\Delta t},
\EN
\EQ
...
\left(\begin{array}{cc} 
\dfrac{\p R_{j}}{\p\ln C_{l}} & \dfrac{\p R_{j}}{\p\ln C_c^{l}}\\
\dfrac{\p R_{c}}{\p\ln C_{l}} & \dfrac{\p R_{c}}{\p\ln C_{c}^{l}}\\
\end{array}\right)
\left[
\begin{array}{c}
\delta\ln C_j\\
...
\BA
%R_j:C_j'
\left(\dfrac{\p R_{jn}}{\p C_{j'n'}}\right)_{C_c^l}^{} &\eq\delta_{nn'}\left[
\varphi_n^{} s_{ln}^{}\frac{\p\Psi_{jn}^{k+1}}{\p C_{j'n}} 
+ \varphi_n^{} s_{ln}^{}\sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{l,k+1}}{\p C_{j'n}} 
+ \sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{s,k+1}}{\p C_{j'n}}\right. \nonumber\\
&\left. \quad + \sum_{im} \nu_{ji}^{m} \frac{\p S_{i}^{m,k+1}}{\p C_{j'n}}
\right] \frac{V_n}{\Delta t}
+\sum_{n''}\left[\frac{\p\Omega_{jnn''}}{\p C_{j'n'}}+q_{cnn''}\sum_{ic}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{l,k+1}}{\p C_{j'n'}}\right] A_{nn''}^{}\nonumber \\
&\qquad+\delta_{nn'}^{} V_n^{}\sum_m\nu_{jm}^{}\frac{\p I_{mn}}{\p C_{j'n}},\\
%R_j:C_c'
\left(\dfrac{\p R_{jn}}{\p C_{cn'}^l}\right)_{C_j^{}}^{} &\eq \delta_{nn'}\left[ 
\varphi_n^{} s_{ln}^{}\sum_{i} \nu_{ji}^{c} \frac{\p S_{ic}^{l,k+1}}{\p C_{cn}^l} 
+ \sum_{i} \nu_{ji}^{c} \frac{\p S_{ic}^{s,k+1}}{\p C_{cn}^l}
\right] \frac{V_n}{\Delta t} \nonumber\\
&\qquad\qquad+\sum_{n''} q_{cnn''}^{}\sum_{i}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{l,k+1}}{\p C_{cn'}^l} A_{nn''}^{},\\
\left(\dfrac{\p R_{cn}}{\p C_{jn'}^{}}\right)_{C_j^{}}^{} &\eq 0,\\
\left(\dfrac{\p R_{cn}}{\p C_{cn'}^l}\right)_{C_j^{}}^{} &\eq \delta_{nn'} \varphi_n s_{ln} \Big(1+\frac{\p C_{cn}^s}{\p C_{cn}^l}\Big)\frac{V_n}{\Delta t}+\sum_{n'} q_{cnn'}^{}\frac{\p C_{cnn'}^l}{\p C_{cn}^l} A_{nn'}^{}.
\EA
\end{subequations}
...
\BA
%R_j:C_j'
\left(\dfrac{\p R_{jn}}{\p C_{j'n'}}\right)_{C_c^l,\,S_{ic}^l}^{} &\eq\delta_{nn'}\left[
\varphi_n^{} s_{ln}^{}\frac{\p\Psi_{jn}^{k+1}}{\p C_{j'n}} 
+ \sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{s,k+1}}{\p C_{j'n}} + \sum_{im} \nu_{ji}^{m} \frac{\p S_{i}^{m,k+1}}{\p C_{j'n}}
\right] \frac{V_n}{\Delta t}\nonumber\\
&+\sum_{n''}
...
+\delta_{nn'}^{} V_n^{}\sum_m\nu_{jm}^{}\frac{\p I_{mn}}{\p C_{j'n}},\\
%R_j:C_c'
\left(\dfrac{\p R_{jn}}{\p C_{cn'}^l}\right)_{C_j^{},\,S_{ic}^l}^{} &\eq \delta_{nn'}\left[ 
\sum_{i} \nu_{ji}^{c} \frac{\p S_{ic}^{s,k+1}}{\p C_{cn}^l}
\right] \frac{V_n}{\Delta t} \\
%R_j:S_i'c'
\left(\dfrac{\p R_{jn}}{\p S_{icn'}^l}\right)_{C_j^{},\,C_c^l}^{} &\eq\delta_{nn'}\nu_{ji}^c 
%\left[
\varphi_n^{} s_{ln}^{} 
%+\frac{\p S_{ic}^{s,k+1}}{\p S_{icn}^l}\right] 
\frac{V_n}{\Delta t} 
+\sum_{n''}q_{cnn''}\sum_{ic}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{l,k+1}}{\p S_{icn'}^l} A_{nn''}^{},
\EA
\end{subequations}
\begin{subequations}
\BA
%R_c:C_j'
\left(\dfrac{\p R_{cn}}{\p C_{j'n'}}\right)_{C_c^l,\,S_{ic}^l}^{} & \eq 0,\\
%R_c:C_c'
\left(\dfrac{\p R_{cn}}{\p C_{c'n'}^l}\right)_{C_j,\,S_{ic}^l}^{} &\eq\varphi_n^{} s_{ln}^{}\delta_{nn'}^{}\delta_{cc'}^{}
\left(1+\frac{\p C_{cn}^s}{\p C_{cn}^l}\right) \frac{V_n}{\Delta t} 
+ \delta_{cc'}\sum_{n''} q_{cnn''} \frac{\p C_{cnn''}^l}{\p C_{cn'}^l} A_{nn''},\\
%R_c:S_i'c'
\left(\dfrac{\p R_{cn}}{\p C_{i'c'n'}^l}\right)_{C_j,\,C_c^l}^{} &\eq 0,
\EA
\end{subequations}
\begin{subequations}
\BA
%R_ic:C_j'
\left(\dfrac{\p R_{icn}}{\p C_{j'n'}}\right)_{C_c^l,\,S_{ic}^l}^{} &\eq \varphi_n^{} s_{ln}^{}\delta_{nn'}\frac{\p S_{icn}^l}{\p C_{j'n}} \frac{V_n}{\Delta t} + \sum_{n''} q_{cnn''} \frac{\p S_{ic,nn''}^l}{\p C_{j'n'}^{}} A_{nn''}-\delta_{nn'} V_n\frac{\p I_{icn}^l}{\p C_{j'n}},\\
%R_ic:C_c'
\left(\dfrac{\p R_{icn}}{\p C_{c'}^l}\right)_{C_j,\,S_{ic}^l}^{} &\eq \varphi_n^{} s_{ln}^{}\delta_{nn'}^{}\delta_{cc'}^{} \frac{\p S_{icn}^l}{\p C_{cn}}\frac{V_n}{\Delta t} + \delta_{cc'}^{}\sum_{n''} q_{cnn''} \frac{\p S_{ic,nn''}^l}{\p C_{cn'}^l} A_{nn''}\nonumber\\
&\hspace{5cm} +\delta_{nn'}\delta_{cc'} V_n\left(\frac{\p\widetilde I_{cn}}{\p C_c^l}-\frac{\p I_{icn}^l}{\p C_{cn}}\right),\\
%R_ic:S_i'c'
\left(\dfrac{\p R_{icn}}{\p S_{i'c'}^l}\right)_{C_j,\,C_c^l}^{} &\eq \varphi_n^{} s_{ln}^{}\delta_{nn'}\delta_{ii'}\delta_{cc'} \frac{V_n}{\Delta t} + \delta_{cc'}\sum_{n''} q_{cnn''} \frac{\p S_{ic,nn''}^l}{\p S_{icn'}^l} A_{nn''}\nonumber\\
&\hspace{5cm}-\delta_{nn'}\delta_{cc'} V_n \frac{\p I_{icn}^l}{\p S_{i'cn}}.
\EA
\end{subequations}
...
\begin{subequations}
\BA
\frac{\p S_{c}^{s}}{\p\ln C_{j}} &\eq -\frac{N_c}{\Delta t D_c^2} \sum_i \frac{k_{ic}^f}{1+k_{ic}^b\Delta t} \frac{\p Q_{ic}}{\p\ln C_j},\\
%\EN
%\EQ
...
%\EN
%\EQ
\frac{\p S_{ic}^s}{\p\ln C_{c}^l} &\eq C_c^l\frac{S_{ic}^s}{C_c^s}\frac{k_c^f \Delta t}{1+k_c^f\Delta t},\\
%\EN
%\EQ
\frac{\p I_{icn}^l}{\p\ln C_{j'n}} &\eq k_{ic}^f \ldots,\\
%\frac{\p S_{ic,n}^l}{\p C_{cn'}^l} &\eq ,\\
\frac{\p I_{icn}^l}{\p S_{i'cn}} &\eq -k_{ic}^b,\\
%\EN
%\EQ
...
\begin{subequations}
%\EQ
%\frac{\p C_{c}^s}{\p C_{c}^l} \eq K_{ls}^{-1},
%\EN
\BA
\frac{\p S_{c}^{l,s}}{\p C_{j}} &\eq \\
%\EN
%\EQ
\frac{\p S_{ic}^{l,s}}{\p\ln C_{j}} &\eq S_{ic}^{l,s}\left[
\nu_{ji}^c - \frac{1}{\omega_{l,s}}\sum_{i'}\nu_{ji'}^c S_{i'c}^{l,s}
\right],\\
%\EN
%\EQ
...
%\EN
%\EQ
\frac{\p S_{ic}^{l}}{\p\ln C_{c'}^{l}} &\eq \delta_{cc'} S_{ic}^{l},\\
%\EN
%\EQ
\frac{\p S_{ic}^s}{\p\ln C_{c'}^l} &\eq \delta_{cc'} C_c^l \frac{S_{ic}^{s}}{C_c^s}\frac{k_c^f \Delta t}{1+k_c^f\Delta t}.
\EA
\end{subequations}
