...
\EN
where $\bu$ represents the displacement vector. The distance between two points $ds$ is transformed to (Einstein summation implied)
\EQ
{ds'}^2 \eq ds^2 + 2 u_{ik} dx_i dx_k,
\EN
where the strain tensor is defined as
\EQ
u_{ik} \eq \frac{1}{2}\left(\frac{\p u_i}{\p x_k} + \frac{\p u_k}{\p x_i}+ \frac{\p u_l}{\p x_i} \frac{\p u_l}{\p x_k}\right).
\EN
...
\EQ
dV' \eq dV \big(1 + u_{ii}\big) \eq dV \big(1+ \sum_i u^{(i)}\big).
\EN

\subsection{Stress Tensor}

The force acting on a body is related to the stress tensor $\sigma_{ik}$ by the divergence 
\EQ
F_i \eq \p_k\sigma_{ik},
\EN
...
\EQ
G \eq \E- TS + \sigma_{ik} u_{ik}.
\EN
\EQ
dG \eq -S dT - u_{ik} d\sigma_{ik}.
\EN
\EQ
...
\BA
F &\eq F_0 + \frac{1}{2} \lambda u_{ii}^2 + \mu u_{ik}^2 + \ldots,\\
&\eq \mu \big(u_{ik} - \frac{1}{3} \delta_{ik} u_{ll}\big)^2 + \frac{1}{2} K u_{ll}^2,
\EA
with Lam\'e coefficients $\lambda$ and $\mu$, where $K$ (bulk modulus, modulus of hydrostatic compression) is related to $\lambda$ and $\mu$ (shear modulus, modulus of rigidity) by
\EQ
K \eq \lambda + \frac{2}{3} \mu.
\EN
\EQ
\sigma_{ik} \eq \left(\frac{\p F}{\p u_{ik}}\right)_T \eq K u_{ll} \delta_{ik} + 2 \mu \big(u_{ik}-\frac{1}{3} \delta_{ik} u_{ll}\big).
\EN
Conversely
\EQ
u_{ik} \eq \frac{1}{9K} \delta_{ik} \sigma_{ll} + \frac{1}{2\mu} \big(\sigma_{ik} - \frac{1}{3} \delta_{ik} \sigma_{ll}\big),
\EN
and
\EQ
u_{ll} \eq \frac{\sigma_{ll}}{3K}.
\EN
