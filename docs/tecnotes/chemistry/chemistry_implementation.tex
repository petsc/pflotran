...
\section{Colloid Type Structure}
...
\EN
with internal energy $U_\a$ and enthalpy $H_\a$ of fluid phase $\a$, rock heat capacity $C_p$ and thermal conductivity $\kappa$.
...
using
\EQ
W \eq \frac{M}{N} \eq \frac{1}{N}\sum_i n_i W_i \eq \sum_i x_i W_i.
\EN
...
\eq \frac{x_w W_w}{w_w},\\
&\eq \frac{W_w\big(1+\sum_{l\ne w} m_l W_l\big)}{1+W_w\sum_{l\ne w} m_l}.
\EA
and
\EQ
