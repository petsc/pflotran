...
\EN
The equilibrium sorbed concentration $q_{j\a}^{\rm eq}$ is defined as
\EQ\label{qeq}
...
\EQ
q_{j\a}^{\rm eq}\eq f_\a K_j^d \Psi_j,
\EN
with
\EQ
\sum_\a f_\a \eq 1.
...
\EQ
\frac{\p q_{j\a}}{\p t}\eq k_\a\big(f_\a K_j^d \Psi_j-q_{j\a}\big).
\EN
Note that in the $K_d$ model in the limit $k_\a\rightarrow\infty$,
\EQ
q_{j\a}\eq f_\a K_j^d \Psi_j.
\EN
It then follows that
\EQ
q_j\eq \sum_\a f_\a K_j^d\Psi_j \eq K_j^d\Psi_j.
\EN
...
\begin{subequations}
\EQ
K_1 \eq \dfrac{S_1 a_{\rm H^+}^2}{S_{\rm SOH} a_{\rm UO_2^{2+}} a_{\rm H_2O}},
\EN
and
\EQ
K_2 \eq \dfrac{S_2 a_{\rm H^+}^2}{S_{\rm SOH} a_{\rm UO_2^{2+}} a_{\rm CO_2} a_{\rm H_2O}},
\EN
\end{subequations}
where subscripts 1 and 2 refer to surface complexes $\text{\sohuotwocothree}$ and $\text{\sohuotwocothree}$, respectively. Noting that
\EQ
S_{\rm SOH} \eq \frac{\omega_\a}{1+K_1Q_1+K_2Q_2},
\EN
where
\begin{subequations}
\EQ
...
\begin{subequations}
\EQ
S_1 \eq \dfrac{\omega_\a K_1Q_1}{1+K_1Q_1+K_2Q_2},
\EN
and
\EQ
S_2\eq \dfrac{\omega_\a K_2Q_2}{1+K_1Q_1+K_2Q_2}.
\EN
\end{subequations}
Assuming $K_1Q_1+K_2Q_2 \ll 1$, and conditions of constant pH and CO$_2$ then
\begin{subequations}
\EQ
S_1 \eq \omega_\a K_1' C_{\rm UO_2^{2+}},
\EN
and
\EQ
S_2 \eq \omega_\a K_2' C_{\rm UO_2^{2+}},
\EN
\end{subequations}
where
\begin{subequations}
...
\EQ
K_j^d\eq \frac{1}{\varphi\Psi_j}\sum_i\nu_{ji} S_i,
\EN
 it follows that for UO$_2^{2+}$
\begin{subequations}
\BA
K_{\rm UO_2^{2+}}^d &\eq \frac{\omega_\a \big(K_1' + K_2'\big) C_{\rm UO_2^2+}}{\varphi\Psi_{\rm UO_2^{2+}}},\\
& \eq \frac{\omega_\a \big(K_1' + K_2'\big)}{\varphi \big(1+\sum_i\nu_{ji} C_i/C_{\rm UO_2^{2+}}\big)}.
\EA
\end{subequations}
...
\EQ\label{qkin}
q_{j\a n}^{t+\Delta t} \eq \frac{q_{j\a n}^t+k_\a \Delta t \, q_{j\a n}^{{\rm eq},\,t+\Delta t}}{1+k_\a\Delta t}.
\EN
