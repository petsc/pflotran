...
The multirate formulation is considered only for surface complexation reactions, but ion exchange could also be implemented if desired. For surface complexation reactions of the form
\EQ
...
Combining these two equations leads to an alternative form in which the reaction rate does not appear explicitly
\EQ
...
\EN
where the distribution coefficients $K_j$ for the stationary solid phase including contributions from minerals and colloids, and $K_j^f$ for colloids present in the aqueous phase are defined by
\EQ
...
\subsection{Colloid Type Structure}
...
\subsection{Dual Continuum Connected Matrix (DCCM)}
...
\subsection{Multiple Continuum Disconnected Matrix (DCDM)}
...
\addcontentsline{toc}{section}{Appendix: Block Tridiagonal System of Equations}
...
\EN
%}
\caption{Node configuration for primary ($\a$) and secondary ($\b$) continua.}\label{fnodestruct}
\end{figure}
...
\subsection{Fundamental Governing Equations}

For a multiphase system with chemical reactions involving aqueous-aqueous and aqueous-gas, -oil, -napl etc., with aqueous phase as reference phase, the chemical reactions taking place in the system can be expressed in the canonical form
\EQ
...
&+ \bnabla\cdot\left[ \big(C_j^l \bar\bv_j^l + \sum_i \nu_{ji} C_i^l \bar\bv_i^l\big) + \sum_{i,\,\a\ne l} \nu_{ji} C_i^\a \bar\bv_i^\a \right] \eq -\sum_m \nu_{jm}I_m.
\EA
...
\EQ
\rho \bv_\a \eq \sum_k\rho_k^\a \bar\bv_k^\a.
\EN
...
&\qquad + \bnabla\cdot\left[ \big(\rho_j^l \bar\bv_j^l + W_j \sum_i \nu_{ji} W_i^{-1} \rho_i^l \bar\bv_i^l\big) + W_j\sum_{i,\,\a\ne l} \nu_{ji} W_i^{-1}\rho_i^\a \bar\bv_i^\a \right] \nonumber\\
...
Several different choices can be made for $\bomega_\a$ and the weight coefficients $\omega_k^\a$ leading to different forms for the fluid flow velocity and diffusive flux as listed in Table~\ref{trefframe}. The molar volume $\bar V_k^\a\!=\!\p V/\p n_k^\a$ satisfies the constraint
\EQ
\sum_k n_k^{\a} \bar V_k^\a \eq 1.
\EN
...
\begin{tabular}{lcc}
\toprule[1.5pt]
Reference Frame & Mean Flow Velocity $\bomega_\a$ & $\omega_k^\a$\\
\midrule
Barycentric & $\bv_\a = \dfrac{1}{\rho_\a}\sum_k\rho_k^\a\bar\bv_k^\a$ & $\chi_k^\a\!=\!\rho_k^\a/\rho_\a$\\
Molecular & $\bu_\a = \dfrac{1}{\eta_\a}\sum_k C_k^\a \bar\bv_k^\a$ & $x_k^\a\!=\!C_k^\a/\eta_\a$\\
Volumetric (Fickian) & $\bw_\a = \sum_k C_k^\a \bar{V}_k^\a \bar\bv_k^\a$ & $\phi_k^\a\!=\!C_k^\a \bar{V}_k^\a$\\
Hittorf & $\bv_\a^\prime = \bar\bv_1^\a$ & 1, $k$=1; 0 otherwise\\
\bottomrule[1.5pt]
\end{tabular}
\end{table}

\subsubsection{Transformation of Diffusive Flux Among Reference Velocities}
...
\paragraph{Molecular to Barycentric Transformation.}
...
Introducing
\EQ
~_{\bv}\widetilde\bJ_k^\a \eq W_k ~_{\bv}\bJ_k^\a,
\EN
and multiplying through by $W_k$, Eqn.\eqref{trans1} becomes
\EQ\label{trans2}
W_k ~_{\bu}\bJ_k^\a \eq ~_{\bv}\widetilde\bJ_k^\a - x_k^\a W_k \sum_i W_i^{-1} ~_{\bv}\widetilde\bJ_i^\a.
\EN
Kee et al. (2003) relate the molecular and diffusive fluxes to Fick's law according to the relations
\EQ
...
\BA
W_k ~_{\bu}\bJ_k^\a &\eq -W_k\eta_\a D_\a \bnabla \left(\frac{W_k^{-1} \chi_k^\a}{\sum_i W_i^{-1}\chi_i^\a}\right),\\
...
\EN
\EQ
W_\a \eq \sum_k W_k x_k^\a \eq \frac{1}{\sum W_k^{-1} \chi_k^\a},
\EN
and
\EQ
...
\subsection{Supercritical CO$_2$ Equilibria}
...
f_\c \eq \phi_\c P_\c,
\EN
with fugacity coefficient $\phi_\c$. 

Introducing the equilibrium constant $K_\c$ defined as
\EQ
K_\c \eq -\frac{1}{RT}\big(\mu_{\rm CO_2}^{g\ominus} - \mu_{\rm CO_2}^{l\ominus}\big),
\EN
yields the mass action equation
\EQ\label{massactco2}
K_\c \eq \frac{f_\c}{a_\c} \eq \frac{\phi_\c P_\c}{a_\c} \eq \frac{\phi_\c y_\c^g P_g}{a_\c},
\EN
where the mole fraction of $\c$ in the gas phase, $y_\c$, is assumed to be given by
\EQ
y_\c^g \eq \frac{P_\c^g}{P_g} \eq \frac{P_g-P_\w^{\rm sat}(T)}{P_g},
\EN
with total gas pressure $P_g$ equal to
\EQ\label{totalp}
P_g \eq P_\c^g + P_\w^{\rm sat}(T),
\EN
and where $P_\w^{\rm sat}(T)$ denotes the saturation pressure of pure water. Of the three quantities: $P_g$, $P_\c^g$, and $T$, two may be specified and the other computed from Eqn.\eqref{totalp}.
...
\EA
where an effective equilibrium constant $\widetilde K_\c$ is defined as
\EQ
...
Weight $w_i$ and mole $x_i$ fractions are related by the expression
\begin{subequations}
\EQ
...
\EQ
{ds'}^2 \eq ds^2 + 2 \sum_{ik} u_{ik} dx_i dx_k,
\EN
where the strain tensor is defined as
\EQ
u_{ik} \eq \frac{1}{2}\left(\frac{\p u_i}{\p x_k} + \frac{\p u_k}{\p x_i}+ \sum_l \frac{\p u_l}{\p x_i} \frac{\p u_l}{\p x_k}\right).
\EN
The strain tensor is symmetric $(u_{ik} \!=\!u_{ki})$ implying that it may be diagonalized
\EQ
u_{ik} \eq u^{(i)} \delta_{ik}.
\EN
A volume element $dV$ to first order transforms as
\EQ
dV' \eq dV \big(1 + \sum_i u_{ii}\big) \eq dV \big(1+ \sum_i u^{(i)}\big).
\EN
...
\paragraph{Energy}
\EQ
d\E \eq T dS + \sum_{ik} \sigma_{ik} du_{ik}.
\EN
\EQ
\sigma_{ik} \eq \left(\frac{\p\E}{\p u_{ik}}\right)_S.
\EN
...
\EN
\EQ
dF \eq -S dT + \sum_{ik} \sigma_{ik} du_{ik}.
\EN
\EQ
\sigma_{ik} \eq \left(\frac{\p F}{\p u_{ik}}\right)_T.
\EN
\paragraph{Gibbs Free Energy}
\EQ
G \eq \E- TS + \sum_{ik} \sigma_{ik} u_{ik}.
\EN
\EQ
dG \eq -S dT - \sum_{ik} u_{ik} d\sigma_{ik}.
\EN
\EQ
u_{ik} \eq -\left(\frac{\p G}{\p\sigma_{ik}}\right)_T.
\EN
...
\BA
F &\eq F_0 + \frac{1}{2} \sum_{i} \lambda u_{ii}^2 + \mu  \sum_{ik} u_{ik}^2 + \ldots,\\
&\eq \mu \big(u_{ik} - \frac{1}{3} \delta_{ik} \sum_{l} u_{ll}\big)^2 + \frac{1}{2} K  \sum_{l} u_{ll}^2,
\EA
...
\EQ
\sigma_{ik} \eq \left(\frac{\p F}{\p u_{ik}}\right)_T \eq K \delta_{ik} \sum_{l} u_{ll} + 2 \mu \big(u_{ik}-\frac{1}{3} \delta_{ik} \sum_{l} u_{ll}\big).
\EN
Conversely
\EQ
u_{ik} \eq \frac{1}{9K} \delta_{ik} \sum_{l} \s_{ll} + \frac{1}{2\mu} \big(\s_{ik} - \frac{1}{3} \delta_{ik} \sum_{l} \s_{ll}\big),
\EN
and
\EQ
u_{ll} \eq \frac{1}{3K}\sum_{l} \s_{ll}.
\EN

According to Euler's theorem
\EQ
F \eq \frac{1}{2} \sum_{ik} \s_{ik} u_{ik}.
\EN

From the quadratic form of the elastic energy
\EQ
u_{ik} \eq \frac{\p F}{\p\s_{ik}}.
\EN
...
\EN
\EQ
u_{xx} \eq u_{yy} \eq -\frac{1}{3}\left(\frac{1}{2\mu}-\frac{1}{3K}\right)p,
\EN
\EQ
u_{zz} \eq \frac{1}{3}\left(\frac{1}{3K}+\frac{1}{\mu}\right)p.
\EN
\EQ
u_{zz} \eq \frac{p}{E},
\EN
...
\EQ
\s \eq -\frac{\s_{xx}}{\s_{zz}},
\EN
where
\EQ
\s \eq \frac{1}{2} \left(\frac{3K-2\mu}{3K+\mu}\right).
\EN

\EQ
F \eq \frac{E}{2(1+\s)} \left[u_{ik}^2 + \frac{\s}{1-2\s} \sum_l u_{ll}^2\right],
\EN
\EQ
\s_{ik} \eq \frac{E}{1+\s}\left(u_{ik} + \frac{\s}{1-2\s}\delta_{ik} \sum_l u_{ll} \right),
\EN
\EQ
u_{ik} \eq \frac{1}{E} \left[(1+\s)\s_{ik} - \s\delta_{ik} \sum_l \s_{ll}\right],
\EN
\begin{subequations}
\EQ
\s_{xx} \eq \frac{E}{(1+\s)(1-2\s)} \left[\big(1-\s\big) u_{xx} + \s \big(u_{yy}+u_{zz}\big)\right],
\EN
\EQ
\s_{yy} \eq \frac{E}{(1+\s)(1-2\s)} \left[\big(1-\s\big) u_{yy} + \s \big(u_{xx}+u_{zz}\big)\right],
\EN
\EQ
\s_{zz} \eq \frac{E}{(1+\s)(1-2\s)} \left[\big(1-\s\big) u_{zz} + \s \big(u_{xx}+u_{yy}\big)\right],
\EN
\EQ
\s_{xy}\eq\frac{E}{1+\s} u_{xy}, \ \s_{xz}\eq\frac{E}{1+\s} u_{xz}, \ \s_{yz}\eq\frac{E}{1+\s} u_{yz}.
\EN
\end{subequations}
...
\EQ
F(T,\, u_{ik}) \eq F_0(T)-K\a(T-T_0) \sum_l u_{ll} + \mu \big(u_{ik}-\frac{1}{3}\delta_{ik}\sum_l u_{ll}\big)^2+\frac{1}{2}K \sum_l u_{ll}^2,
\EN
\EQ
\s_{ik} \eq -K \a (T-T_0) \delta_{ik} + K \delta_{ik} \sum_l u_{ll} + 2\mu \big(u_{ik} - \frac{1}{3} \delta_{ik} \sum_l u_{ll}\big).
\EN
...
\EQ
\sum_k \frac{\p\sigma_{ik}}{\p x_k} + \rho g_i - K\a \frac{\p T}{\p x_k}\eq 0.
\EN
...
\EQ
\frac{\p^2 u_i}{\p t^2} \eq \sum_k \frac{\p\sigma_{ik}}{\p x_k} + \rho g_i - K\a \frac{\p T}{\p x_k}.
\EN
For an isotropic medium this equation becomes using vector notation
\EQ
\rho\frac{\p^2\bu}{\p t^2} \eq \frac{E}{2(1+\sigma)} \nabla^2 \bu + \frac{E}{2(1+\sigma)(1-2\sigma)} \bnabla \big(\bnabla\cdot\bu\big) + \rho\bg - \frac{E\a}{3(1-2\s)} \bnabla T.
\EN
\EQ
K \eq \frac{E}{3(1-2\s)}.
\EN
