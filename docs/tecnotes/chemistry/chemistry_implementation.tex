...
$\circ$Colloids,\\ 
$\bullet$Surface Complexation, \\
$\bullet$Ion Exchange, \\
Biogeochemistry with Monode Kinetics, \\
$\bullet$Precipitation/Dissolution, \\
Solid Solutions, \\
Species-Dependent Diffusion, \\
Pitzer Model, \\
$\circ$Multiple Interacting Continua, \\
Hydration-Dehydration Reactions, \\
Electrical Conductivity, \\
$\circ$Multiphase Systems, \\
Isotopic Exchange, \\
\ldots}
...
\EN
\EQ
C_i^g \eq \frac{P_i}{RT} \eq \frac{K_i}{RT}\prod \left(\gamma_jC_j\right)^{\nu_{ji}^g}
\EN
...
\item[Total Sorbed Concentration $\Psi_j^0 + \sum_i\nu_{ji}^{\rm srf}(S_i^{\rm srf})_0 + \sum_i\nu_{ji}^{\rm ex}(S_i^{\rm ex})_0$:]
\EQ
\Psi_j^{\rm tot} \eq \Psi_j + \sum_i\nu_{ji}^{\rm srf}S_i^{\rm srf} + \sum_i\nu_{ji}^{\rm ex}S_i^{\rm ex} \eq \Psi_j^0 + \sum_i\nu_{ji}^{\rm srf}(S_i^{\rm srf})_0 + \sum_i\nu_{ji}^{\rm ex}(S_i^{\rm ex})_0.
\EN
...
\EN
\EQ
\ln K_m +\sum_j\nu_{jm}\ln\left(\gamma_j C_j\right)\eq 0.
\EN
\end{subequations}
...
\begin{subequations}
\EQ
P_i \eq K_i \prod_j\left(\gamma_j C_j\right)^{\nu_{ji}^g},
\EN
\EQ
\ln P_i \eq \ln K_i+ \sum_j \nu_{ji}^g\ln\left(\gamma_j C_j\right).
\EN
\end{subequations}
...
\EN
with equilibrium constant $K_i$, where the molality $m_l$ is related to molarity $C_l$ by the approximate expression
\EQ
C_l \eq \rho_w m_l,
\EN
...
\EQ\label{f}
f(\I)\eq \frac{1}{2} \sum_j z_j^2 m_j + \frac{1}{2} \sum_i z_i^2 m_i(\I) -\I.
\EN
The ionic strength is obtained through Newton-Raphson iteration to give
\EQ
\I_{k+1} \eq \I_k-\dfrac{f(\I_k)}{\dfrac{df(\I_k)}{d\I}},
\EN
