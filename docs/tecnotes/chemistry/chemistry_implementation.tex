...
An approximate expression for the activity of water is based on RaoultÕs law (Garrels and Christ, 1965, p. 65-66): 
\EQ
...
\EQ
A_m \eq 1-\left(K_m Q_m\right)^{1/\sigma_m},
\EN
the rate may be expressed alternatively as
\EQ
\widehat I_m \eq -s_m^{} \sum_l \P_{ml}^{} k_{ml}^{} 
\dfrac{A_m}{1+\dfrac{k_{ml}^{}}{r_m^{\rm lim}} \big(1-A_m\big)}.
\EN
...
\EN
In the numerical implementation for sorption on mineral surfaces it is customary to lag the site concentration $\omega_\a$ one time step behind giving the equation
\EQ\label{sfree1}
S_\a^{k+1} \eq \omega_\a^{k} - \sum_{i'} S_{i'\a}^{k+1}.
\EN
\end{subequations}
However, this approximation may not be adequate for colloids.
...
\EN
Substituting this result into Eqn.\eqref{sfree} and solving for $S_\a^{k+1}$ yields the result
\EQ
S_\a^{k+1} \eq \frac{
\omega_\a^{k+1} - \displaystyle\sum_{i} \dfrac{S_{{i}\a}^{k}}{1+k_{i\a}^b\Delta t}
}
{1+\displaystyle\sum_{i}\dfrac{k_{i\a}^f\Delta t}{1+k_{i\a}^b\Delta t} Q_{i}^{k+1}} \eq \frac{N_\a}{D_\a},
\EN
where
\EQ
N_\a \eq\omega_\a^{k+1} - \displaystyle\sum_{i} \dfrac{S_{i\a}^{k}}{1+k_{i\a}^b\Delta t},
\EN
and
\EQ
...
\frac{
\omega_\a^{k+1} - \displaystyle\sum_{i'} S_{i'\a}^{k}
}
...
It is necessary to distinguish between two distinct colloid states: colloids suspended in solution, and colloids attached to solid surfaces which are not moving. The two states are described by the concentrations $C_c^\m$ and $C_c^\im$. 
...
\A_c^\m \arrows \A_c^\im,
\EN
where $\A_c^\m$ and $\A_c^\im$ refer to suspended mobile colloids in the fluid phase and stationary colloids, respectively.
The colloid concentration $C_c^\m$ is equal to the number of moles of colloids of type $c$ in solution, $N_c^{\m}$, divided by the aqueous volume $V_{\rm aq}$
\EQ
C_c^\m\eq\frac{N_c^{\m}}{V_{\rm aq}}.
\EN
Whereas the stationary colloid concentration $C_c^\im$ is defined as
\EQ
C_c^\im\eq\frac{N_c^\im}{V},
\EN
...
\EQ
\sum_j\nu_{jc_{\m,\,\im}}\A_j\arrows\A_{c_{\m,\,\im}},
\EN
describing precipitation and dissolution (formation and degradation) of colloid $\A_c^{\m,\,\im}$ for mobile and immobile colloids, surface complexation
\EQ
...
In the case of surface complexation reactions, two different surface site densities may be defined corresponding to suspended and filtered colloids. The site concentrations for suspended and filtered colloids are equal to the number of sorption sites $N_c^{\rm srf}$ associated with colloid type $\A_c^{\m,\,\im}$ divided by the pore or bulk volume
\EQ
\omega_c^\m\eq\frac{N_c^{\m,\,{\rm srf}}}{V_p},
\EN
and
\EQ
\omega_c^\im \eq \frac{N_c^{\im,\,{\rm srf}}}{V}.
\EN
For suspended colloids, the site concentration is related to the suspended colloid concentration $C_c^\m$ according to the relation
\BA
\omega_c^\m &\eq\frac{N_c^{\m,\,{\rm srf}}}{A_c^\m}\frac{A_c^\m}{M_c^\m}\frac{M_c^\m}{N_c^{\m}}\frac{N_c^{\m}}{V_p},\\
&\eq \eta_c^{} A_c^M W_c^{} C_c^{\m},\\
&\eq\Gamma_c^{} C_c^\m,
\EA
...
\EQ
\Gamma_c\eq\frac{N_c^{\m,\,{\rm srf}}}{N_c^{\m}}\eq \frac{N_c^{\im,\,{\rm srf}}}{N_c^\im}\eq \eta_c^{} A_c^M W_c^{}.
\EN
For stationary colloids, the site concentration is given in terms of the trapped or immobile colloid concentration $C_c^\im$ as
\BA
\omega_c^\im &\eq \frac{N_c^{\im,\,{\rm srf}}}{A_c^s}\frac{A_c^s}{M_c^s}\frac{M_c^\im}{N_c^\im}\frac{N_c^\im}{V},\\
&\eq \eta_c^{} A_c^M W_c^{} C_c^\im,\\
&\eq\Gamma_c^{} C_c^\im.
\EA
...
\BA
\omega_c^{\m,\,\rm ex} &\eq \frac{N_c^{\m,\,\rm ex}}{V_p},\\
&\eq \frac{N_c^{\m,\,\rm ex}}{M_c^\m} \frac{M_c^\m}{N_c^{\m}}\frac{N_c^{\m}}{V_p},\\
&\eq \Q_c W_c C_c^\m,
\EA
...
\EQ
\frac{\p}{\p t}\varphi s_l^{} C_c^\m + \bnabla\cdot\bF_c^\m \eq -I_c^{\m,\,\im} - I_c^\m,
\EN
for mobile (liquid phase) colloids with concentration $C_c^\m$ and flux $\bF_c^\m$, and
\EQ
\frac{\p C_c^\im}{\p t} \eq I_c^{\m,\,\im} + I_c^\im,
\EN
for immobile colloids with concentration $C_c^\im$ attached to the stationary solid phase. 

Adding these two equations leads to an alternative form in which the reaction rates do not appear explicitly
\EQ
\frac{\p}{\p t}\left(\varphi s_l^{} C_c^\m + C_c^\im\right) + \bnabla\cdot\bF_c^\m \eq 0.
\EN

Transport equations for surface complexes associated with mobile and immobile colloids obey the conservation equations   
\EQ
\frac{\p}{\p t}\varphi s_l^{} S_{ic}^\m + \bnabla\cdot\bq_c S_{ic}^\m \eq I_{ic}^\m,
\EN
\EQ
\frac{\p S_{ic}^\im}{\p t} \eq I_{ic}^\im,
\EN
with colloid Darcy velocity $\bq_c$ and neglecting colloid diffusion. Free (empty) sites obey the equations
\EQ
\frac{\p}{\p t}\varphi s_l^{} S_{c}^\m + \bnabla\cdot\bq_c S_{c}^\m \eq -\sum_i I_{ic}^\m,
\EN
\EQ
\frac{\p S_{c}^\im}{\p t} \eq -\sum_i I_{ic}^\im.
\EN
Aqueous phase primary species transport equations have the form
\BA
\frac{\p}{\p t}\varphi s_l^{}\Psi_j &+ \bnabla\bdot\bOmega_j \eq 
-\sum_m\nu_{jm} I_m -\sum_{im}\nu_{ji}^m I_i^m \nonumber\\
&-\sum_c \nu_{jc}^{} \big(I_c^\m+I_c^\im\big) 
- \sum_{ic}^{}\nu_{ji}^c\big(I_{ic}^\m+I_{ic}^\im\big) 
...
\BA\label{pricolloid}
\frac{\p}{\p t}\bigg(\varphi s_l^{}\Psi_j + \sum_{im} \nu_{ji}^m S_{im} &+ \sum_{ic} \nu_{ji}^c \big(\varphi s_l^{} S_{ic}^\m+S_{ic}^\im\big)\bigg) + \bnabla\bdot\bigg(\bOmega_j + \sum_{ic}\nu_{ji}^c\bq_c S_{ic}^\m\bigg) \eq \nonumber\\
&-\sum_m\nu_{jm} I_m - \sum_{c}\nu_{jc}^{} \big(I_{c}^\m+I_{c}^\im\big) + \sum_r\nu_{jr}^{\kin}I_r^{\rm kin}.
\EA
...
&-\frac{1}{1+K_j^\im + K_j^\m}\left[\sum_m\nu_{jm} I_m + \sum_{c}\nu_{jc}^{} \big(I_{c}^\m+I_{c}^\im\big) - \sum_r\nu_{jr}^{\kin}I_r^{\rm kin}\right],
\EA
where the retardation factor $R_j$ is defined as
\EQ
R_j\eq 1+\dfrac{K_j^\im}{1+K_j^\m},
\EN
where the distribution coefficients, $K_j^\im$ for the stationary solid phase including contributions from minerals and colloids, and $K_j^\m$ for colloids present in the aqueous phase are defined by
\EQ
K_j^\im \eq \frac{1}{\varphi s_l^{}\Psi_j} \left[\sum_{im}\nu_{ji}^m S_{im} + \sum_{ic}\nu_{ji}^c S_{ic}^\im\right],
\EN
and
\EQ
K_j^\m \eq \frac{1}{\Psi_j} \sum_{ic}\nu_{ji}^c S_{ic}^\m.
\EN
Note that if $K_j^\m \!\geq\! K_j^\im \!\gg\! 1$, $R_j\!\leq\! 2$, and there is little or no retardation. However, there still remains a reduction in kinetic homogeneous and heterogeneous rates. Finally, it should be noted that if colloids are to compete with stationary solid phase, they must be sufficiently concentrated to provide an equivalent surface site density (or cation exchange capacity).
...
\section{PFLOTRAN Colloid Implementation}

Implementation of colloids in PFLOTRAN accounts for competing sorption in the form of surface complexation reactions between both immobile and mobile colloids and minerals.

\subsection{Summary of Equations}

\subsubsection{Colloid Reactions:}
%\renewcommand{\tabcolsep}{-8pt}
\EQ
\begin{array}{rll}
\A_c^\m &\!\!\!\!\!\arrows \A_c^\im, & \text{(Filtration: $I_c$)},\\
>\!\A_{ic}^\m &\!\!\!\!\!\arrows\! >\!\A_{ic}^\im, & \text{(Mobile-Immobile Colloid Exchange: $I_c$)},\\
>\!X_{c}^\m &\!\!\!\!\!\arrows\! >\!X_{c}^\im, & \text{(Mobile-Immobile Free Site Exchange: $I_c$)},\\
>\!X_c^{\m,\,\im} + \displaystyle\sum_i\nu_{ji}^c\A_j^{} &\!\!\!\!\!\arrows\! >\!A_{ic}^{\m,\,\im}, & \text{(Sorption: $I_{ic}^{\m,\,\im}$)},
\end{array}
\EN
for mobile $(\m)$ and immobile $(\im)$ colloids where $\m$ and $\im$ refer to liquid and solid phases, respectively.

\noindent Sorption site concentration is related to mobile and immobile colloid concentrations and mineral volume fractions according to the expressions:
\BA
\omega_{c}^{\m,\,\im} &\eq \eta_c A_c W_c C_c^{\m,\,\im},\\
&\eq \Gamma_c^{} C_c^{\m,\,\im},
\EA
where
\EQ
\Gamma_c \eq \eta_c A_c W_c,
\EN
for colloids, and
\EQ
...
\BA
\omega_c &\eq \varphi s_l^{} \omega_c^\m + \omega_c^\im, \\
&\eq \varphi s_l^{} \Big(S_c^\m+ \sum_{i=1}^{N_c^{\rm srf}} S_{ic}^{\m}\Big) + S_c^\im + \sum_{i=1}^{N_c^{\rm srf}} S_{ic}^{\im},\\
&\eq \varphi s_l^{} S_c^\m + S_c^\im 
+ \sum_{i=1}^{N_c^{\rm srf}} \big(\varphi s_l^{} S_{ic}^{\m}+S_{ic}^{\im}\big),\\
\omega_m &\eq S_m + \sum_{i=1}^{N_m^{\rm srf}} S_{i}^{m}.
\EA

\subsubsection{Colloid Filtration}
\EQ
\frac{\p}{\p t} \varphi s_l^{} C_c^\m + \bnabla\cdot\bq_c^{} C_c^\m \eq -I_c,
\EN
\EQ
\frac{\p C_c^\im}{\p t} \eq I_c,
\EN
\EQ
I_c \eq k_c^f C_c^\m - k_c^b C_c^\im,
\EN
$\m=$ liq, $\im=$ solid. Alternatively, the mobile colloid transport equation can be written as
\EQ
\frac{\p}{\p t} \Big[\varphi s_l C_c^\m + C_c^\im\Big] + \bnabla\cdot\bq_c^{} C_c^\m \eq 0,
\EN
implying the site conservation equation:
\EQ
\frac{\p}{\p t} \Big[\varphi s_l \omega_c^\m + \omega_c^\im\Big] + \bnabla\cdot\bq_c^{} \omega_c^\m \eq 0,
\EN
coupled to the immobile colloid concentration field
\EQ
\frac{\p \omega_c^\im}{\p t} \eq \Gamma_c I_c.
\EN

\subsubsection{Collolid-Facilitated Transport Equations}
%\EQ
...
%\EN
\paragraph{Primary Species:}
\EQ
\frac{\p}{\p t} \varphi s_l\Psi_j +\bnabla\cdot\bOmega_j \eq - \sum_{ic} \nu_{ji}^c I_{ic}^\m - \sum_{ic} \nu_{ji}^c I_{ic}^\im - \sum_{im} \nu_{ji}^m I_i^m - \sum_m\nu_{jm}I_m
\EN
\noindent Kinetic Sorption Equations---Colloids:\\
\noindent
mobile:
\EQ
\frac{\p}{\p t} \varphi s_l^{} S_{ic}^\m + \bnabla\cdot\big(\bq_c S_{ic}^\m\big) \eq -\widetilde I_c^{} + I_{ic}^\m,
\EN
\EQ
\frac{\p}{\p t} \varphi s_l^{} S_{c}^\m + \bnabla\cdot\big(\bq_c S_{c}^\m\big) \eq -\widetilde I_c^{} - \sum_i I_{ic}^\m,
\EN
immobile:
\EQ
\frac{\p S_{ic}^\im}{\p t} \eq \widetilde I_c^{} + I_{ic}^\im,
\EN
\EQ
\frac{\p S_{c}^\im}{\p t} \eq \widetilde I_c^{} - \sum_i I_{ic}^\im,
\EN
kinetic rate law:
\BA
I_{ic}^{\m,\,\im} &\eq k_{ic}^f Q_{ic}^{} S_{c}^{\m,\,\im} - k_{ic}^b S_{ic}^{\m,\,\im},\\
&\eq k_{ic}^b \big(K_{ic}^{} Q_{ic}^{} S_{c}^{\m,\,\im} - S_{ic}^{\m,\,\im}\big),
\EA
\EQ
...
\EN
\BA
I_i^m &\eq k_{im}^f Q_{ic}^{} S_{m} - k_{im}^b S_{i}^{m},\\
&\eq k_{im}^b \big(K_{im}^{} Q_{ic}^{} S_{m} - S_{i}^{m}\big).
\EA

\subsubsection{Site Conservation}
\begin{subequations}
\EQ
\frac{\p}{\p t} \Big( \varphi s_l^{} S_{ic}^\m + S_{ic}^\im\Big) + \bnabla\cdot\bq_c S_{ic}^\m \eq I_{ic}^\m + I_{ic}^\im,
\EN
\EQ
\frac{\p}{\p t} \Big( \varphi s_l^{} S_c^\m + S_c^\im\Big) + \bnabla\cdot\bq_c S_c^\m \eq -\sum_i\big(I_{ic}^\m + I_{ic}^\im\big).
\EN
\end{subequations}
Summing:
\EQ
\frac{\p}{\p t} \Big[ \varphi s_l^{} S_c^\m + S_c^\im + \sum_i \big(\varphi s_l^{} S_{ic}^\m + S_{ic}^\im\big)\Big] + \bnabla\cdot \Big[\bq_c \big( S_c^\m + \sum_i S_{ic}^\m\big)\Big] \eq 0,
\EN
or, in terms of $\omega_c^{\m,\,\im}$:
\EQ
\frac{\p}{\p t} \Big( \varphi s_l^{} \omega_c^\m + \omega_c^\im \Big) + \bnabla\cdot\bq_c \omega_c^\m \eq 0,
\EN
yields the site conservation equation.

\subsubsection{Independent Set of Governing Equations}

Alternatively (note that the rate $\widetilde I_c$ cancels): 
% local equilibrium:
\BA
\frac{\p}{\p t} &\left[\varphi s_l^{}\Psi_j^{} + \varphi s_l^{}\sum_{ic} \nu_{ji}^{c} S_{ic}^\m + \sum_{ic} \nu_{ji}^{c} S_{ic}^\im + \sum_{im} \nu_{ji}^{m} S_i^{m} \right] \nonumber\\
& \qquad\qquad\qquad + \bnabla\cdot\left[\bOmega_j^{} + \sum_{ic}\bq_c^{}\nu_{ji}^{c} S_{ic}^\m \right] \eq -\sum_m\nu_{jm}^{}I_m^{}.
\EA
\EQ
\frac{\p}{\p t} \varphi s_l^{} S_{ic}^\m + \bnabla\cdot\big(\bq_c S_{ic}^\m\big) \eq -\widetilde I_c^{} + I_{ic}^\m,
\EN
...
\frac{\p S_{ic}^\im}{\p t} \eq \widetilde I_c^{} + I_{ic}^\im.
\EN
\noindent Auxiliary Relations:
\EQ
\omega_c^{\m,\im} \eq S_c^{\m,\,\im}+ \sum_iS_{ic}^{\m,\,\im}\eq \Gamma_c^{} C_c^{\m,\,\im},
\EN
\EQ
I_{ic}^{\m,\,\im} \eq k_{ic}^f Q_{ic}^{} \Big(\Gamma_c^{} C_{c}^{\m,\,\im}-\sum_iS_{ic}^{\m,\,\im}\big) - k_{ic}^b S_{ic}^{\m,\,\im},
\EN
\EQ
I_c \eq k_c^f C_c^\m - k_c^b C_c^\im,
\EN
\EQ
\Gamma_c I_c \eq \widetilde I_c.
\EN

\subsubsection{Relation Between Mobile-Immobile Colloid Exchange Rates $\widetilde I_c$ and $I_c$:}
\BA
\frac{\p\omega_c^\im}{\p t} &\eq \frac{\p S_c^\im}{\p t} + \sum_{i=1}^{N_c^{\rm srf}} \frac{\p S_{ic}^\im}{\p t},\\
&\eq \widetilde I_c + \sum_{i=1}^{N_c^{\rm srf}}\widetilde I_c,\\
&\eq \Big(N_c^{\rm srf}+1\Big) \widetilde I_c,\\
&\eq \Gamma_c I_c.
\EA
\EQ
\widetilde I_c \eq \left(\frac{\Gamma_c}{N_c^{\rm srf}+1}\right) I_c.
\EN

\subsection{Local Equilibrium}

The equilibrium state of the system consistent with kinetic rate law expressions follows.

\noindent
Minerals:
\EQ
S_{i}^{m} \eq K_{im}^{} Q_{ic}^{} S_{m}. 
\EN
\EQ
S_m \eq \frac{\omega_m}{1+\sum K_{im} Q_{im}},
\EN
\EQ
S_{im} \eq \frac{\omega_m K_{im} Q_{im}}{1+\sum K_{im} Q_{im}},
\EN
\EQ
\omega_m \eq S_m + \sum_i S_{im}.
\EN
\noindent Colloids:
\begin{subequations}
\BA
S_{ic}^{\m,\,\im} &\eq K_{ic}^{} Q_{ic}^{} S_c^{\m,\,\im},\\
C_c^\m &\eq K_{\m,\,\im}^{} C_c^\im.
\EA
\EQ
S_c^{\m,\,\im} \eq \frac{\omega_c^{\m,\,\im}}{1+\sum_i K_{ic} Q_{ic}},
\EN
\EQ
S_{ic}^{\m,\,\im} \eq \frac{\omega_c^{\m,\,\im} K_{ic} Q_{ic}}{1+\sum_i K_{ic} Q_{ic}},
\EN
\EQ
\omega_c^{\m,\,\im} \eq S_c^{\m,\,\im} + \sum_i S_{ic}^{\m,\,\im}.
\EN
\end{subequations}
\noindent Filtration Equilibrium:
\EQ
K_{\m,\,\im}^{} \eq \frac{C_c^\m}{C_c^\im},
\EN
\EQ
\frac{S_c^\m}{S_c^\im} \eq \frac{S_{ic}^\m}{S_{ic}^\im} \eq \frac{\omega_c^\m}{\omega_c^\im} \eq \frac{C_c^\m}{C_c^\im} \eq K_{\m,\,\im}^{}.
\EN
\EQ
\omega_c \eq \big(1+\varphi s_l K_{\m,\,\im}\big) \omega_c^\im 
\eq \left(\frac{1+\varphi s_l K_{\m,\,\im}}{K_{\m,\,\im}}\right) \omega_c^\m.
%\big(1+\sum_i K_{ic}^{} Q_{ic}^{}\big) S_c^\im.
\EN
\begin{subequations}
\BA
S_c^\im &\eq \frac{\omega_c}{\big(1+\varphi s_l K_{\m,\,\im}\big)\big(1+\sum_i K_{ic} Q_{ic}\big)},\\
S_{ic}^\im &\eq \frac{\omega_c K_{ic} Q_{ic}}{\big(1+\varphi s_l K_{ls}\big)\big(1+\sum_{i'} K_{i'c} Q_{i'c}\big)},\\
S_c^\m &\eq \frac{\omega_c K_{ls}}{\big(1+\varphi s_l K_{\m,\,\im}\big)\big(1+\sum_i K_{ic} Q_{ic}\big)},\\
S_{ic}^\m &\eq \frac{\omega_c K_{\m,\,\im} K_{ic} Q_{ic}}{\big(1+\varphi s_l K_{\m,\,\im}\big)\big(1+\sum_{i'} K_{i'c} Q_{i'c}\big)}.
\EA
\end{subequations}

In the absence of mineral surface complexation reactions it follows that
\EQ
K_j^\m \eq K_{\m,\,\im}^{} K_j^\im.
\EN
Thus if $K_j^\m\gg1$,
\EQ
R_j\eq 1+\frac{K_j^\im}{K_j^\m} \eq 1+K_{\m,\,\im}^{-1}.
\EN

\subsection{Numerical Implementation}

\subsubsection{Residual Function Independent and Dependent Variables} 

\begin{center}
\begin{tabular}{ll}
\toprule
\multicolumn{2}{c}{Kinetic Formulation}\\
\midrule
Independent Variables: & $\{C_j, \, C_c^\m, \, S_{ic}^\m\}$ \\
Dependent Variables: & $\{C_i^{}, \, C_c^{\im}, \, S_c^{\m,\,\im}, \, S_{ic}^\im, \, S_m^{},\,S_{im}^{}\}$ \\
\midrule
\multicolumn{2}{c}{Sorption Equilibrium Formulation -- Kinetic Filtration}\\
\midrule
Independent Variables: & $\{C_j, \, C_c^\m\}$ \\
Dependent Variables: & $\{C_i^{}, \, C_c^\im, \, S_c^{l,s}, \, S_{ic}^{l,s}, \, S_m^{},\,S_{im}^{}\}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Primary Species:
\begin{subequations}
\BA
R_{jn} &\eq \left[\varphi_n s_{ln} \big(\Psi_{jn}^{k+1}-\Psi_{jn}^k\big) 
+ \varphi_n s_{ln}^{} \sum_{ic}\nu_{ji}^{c} \big(S_{icn}^{\m,k+1}-S_{icn}^{\m,k}\big)\right. \nonumber\\
&\qquad\left. + \sum_{ic} \nu_{ji}^{c}\big(S_{icn}^{\im,k+1}-S_{icn}^{\im,k}\big) 
+ \sum_{im} \nu_{ji}^{m}\big(S_{in}^{m,k+1}-S_{in}^{m,k}\big) \right]\frac{V_n}{\Delta t}\nonumber \\
&\qquad\qquad + \sum_{n'} \Big[\Omega_{jnn'}^{k+1} + q_{cnn'} \sum_{ic}\nu_{ji}^{} S_{ic,nn'}^{\m,k+1} \Big] A_{nn'}^{} + V_n^{}\sum_m\nu_{jm}^{}I_{mn}^{k+1},
\EA
\noindent Mobile Colloids:
\EQ
R_{cn} \eq \Big[\varphi_n^{} s_{ln}^{} \big(C_{cn}^{\m,k+1}-C_{cn}^{\m,k}\big) 
+ C_{cn}^{\im,k+1}-C_{cn}^{\im,k} \Big]\frac{V_n}{\Delta t} + \sum_{n'} q_{cnn'} C_{cnn'}^{\m,k+1} A_{nn'}^{},
\EN
\noindent Mobile Colloid Sorbed Species:
\EQ
R_{ic,\,n} \eq \Big[\varphi s_l^{} \big(S_{ic,n}^{\m,k+1}-S_{ic,n}^{\m,k}\big) 
\Big]\frac{V_n}{\Delta t} + \sum_{n'} q_{cnn'}^{} S_{ic,nn'}^\m A_{nn'}^{} + \Big(\widetilde I_{cn}^{} - I_{ic,n}^\m\Big) V_n^{}.
\EN
\end{subequations}
\noindent Auxiliary Equations:
\begin{subequations}
\BA
C_{cn}^{\im,k+1}-C_{cn}^{\im,k} & \eq \Delta t \frac{k_c^f C_c^{\im,k+1}-k_c^b C_c^{\im,k}}{1+k_c^f\Delta t},\\
S_{ic}^{\im,k+1}-S_{ic}^{\im,k} &\eq \frac{k_{ic}^b S_{ic}^{\im,k}}{1+k_{ic}^b\Delta t} 
+ \frac{k_{ic}^f}{1+k_{ic}^b \Delta t} \left(
\frac{N_c}{D_c}
\right) Q_{ic}^{k+1},\\
S_i^{m,k+1}-S_i^{m,k} &\eq \frac{k_{im}^b S_{im}^k}{1+k_{im}^b\Delta t} 
+ \frac{k_{im}^f}{1+k_{im}^b \Delta t} \left(
\frac{N_m}{D_m}\right) Q_{im}^{k+1},\\
S_c^{\m,\,\im} &\eq \omega_c^{\m,\,\im} - \sum_i S_{ic}^{\m,\,\im},\\
S_m^{} &\eq \omega_m^{} - \sum_i S_{i}^{m},
\EA
\end{subequations}
where the quantities $N_c$ and $N_m$ are defined by
\begin{subequations}
\EQ
N_c \eq\omega_c^{\im,k+1} - \displaystyle\sum_{i} \dfrac{S_{ic}^{\im,k}}{1+k_{ic}^b\Delta t},
\EN
\EQ
...
and ($\a=c,\,m$)
\EQ
D_\a \eq 1+\displaystyle\sum_{i}\dfrac{k_{i\a}^f\Delta t}{1+k_{i\a}^b\Delta t} Q_{i\a}^{k+1}.
\EN
\noindent Interface Sorbed Concentration:
\EQ
S_{ic,nn'}^\m \eq \frac{d_n S_{icn}^\m + d_{n'} S_{icn'}^\m}{d_n + d_{n'}}.
\EN

\subsubsection{Jacobian Structure}

For a fully implicit formulation the nonlinear equations
\EQ
\bR(\bx) \eq \bzero,
\EN
are solved for the logarithm of the concentration using the Newton-Raphson equations
\EQ
\bJ^{(i)} \delta\ln\bx^{(i+1)} \eq -\bR^{(i)},
\EN
for the $i$th iteration. The solution is then updated according to the equation
\EQ
\bx^{(i+1)} \eq \bx^{(i)} {\rm e}^{\delta\ln\bx^{(i+1)}}.
\EN

\paragraph{Equilibrium Colloid/Mineral Sorption Reactions}
\EQ
\renewcommand{\arraystretch}{2.5}
\left(\begin{array}{cc} 
\dfrac{\p R_{j}}{\p\ln C_{l}} & \dfrac{\p R_{j}}{\p\ln C_c^{\m}}\\
\dfrac{\p R_{c}}{\p\ln C_{l}} & \dfrac{\p R_{c}}{\p\ln C_{c}^{\m}}\\
\end{array}\right)
\left[
\begin{array}{c}
\delta\ln C_j\\
...
\BA
%R_j:C_j'
\left(\dfrac{\p R_{jn}}{\p C_{j'n'}}\right)_{C_c^\m}^{} &\eq\delta_{nn'}\left[
\varphi_n^{} s_{ln}^{}\frac{\p\Psi_{jn}^{k+1}}{\p C_{j'n}} 
+ \varphi_n^{} s_{ln}^{}\sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{\m,k+1}}{\p C_{j'n}} 
+ \sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{\im,k+1}}{\p C_{j'n}}\right. \nonumber\\
&\left. \quad + \sum_{im} \nu_{ji}^{m} \frac{\p S_{i}^{m,k+1}}{\p C_{j'n}}
\right] \frac{V_n}{\Delta t}
+\sum_{n''}\left[\frac{\p\Omega_{jnn''}}{\p C_{j'n'}}+q_{cnn''}\sum_{ic}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{\m,k+1}}{\p C_{j'n'}}\right] A_{nn''}^{}\nonumber \\
&\qquad+\delta_{nn'}^{} V_n^{}\sum_m\nu_{jm}^{}\frac{\p I_{mn}}{\p C_{j'n}},\\
%R_j:C_c'
\left(\dfrac{\p R_{jn}}{\p C_{cn'}^\m}\right)_{C_j^{}}^{} &\eq \delta_{nn'}\left[ 
\varphi_n^{} s_{ln}^{}\sum_{i} \nu_{ji}^{c} \frac{\p S_{ic}^{\m,k+1}}{\p C_{cn}^\m} 
+ \sum_{i} \nu_{ji}^{c} \frac{\p S_{ic}^{\im,k+1}}{\p C_{cn}^\m}
\right] \frac{V_n}{\Delta t} \nonumber\\
&\qquad\qquad+\sum_{n''} q_{cnn''}^{}\sum_{i}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{\m,k+1}}{\p C_{cn'}^\m} A_{nn''}^{},\\
\left(\dfrac{\p R_{cn}}{\p C_{jn'}^{}}\right)_{C_j^{}}^{} &\eq 0,\\
\left(\dfrac{\p R_{cn}}{\p C_{cn'}^\m}\right)_{C_j^{}}^{} &\eq \delta_{nn'} \varphi_n s_{ln} \Big(1+\frac{\p C_{cn}^\im}{\p C_{cn}^\m}\Big)\frac{V_n}{\Delta t}+\sum_{n'} q_{cnn'}^{}\frac{\p C_{cnn'}^\m}{\p C_{cn}^\m} A_{nn'}^{}.
\EA
\end{subequations}

\paragraph{Kinetic Colloid/Mineral Sorption Reactions}
...
\BA
%R_j:C_j'
\left(\dfrac{\p R_{jn}}{\p C_{j'n'}}\right)_{C_c^\m,\,S_{ic}^\m}^{} &\eq\delta_{nn'}\left[
\varphi_n^{} s_{ln}^{}\frac{\p\Psi_{jn}^{k+1}}{\p C_{j'n}} 
+ \sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{\im,k+1}}{\p C_{j'n}} + \sum_{im} \nu_{ji}^{m} \frac{\p S_{i}^{m,k+1}}{\p C_{j'n}}
\right] \frac{V_n}{\Delta t}\nonumber\\
&+\sum_{n''}
\frac{\p\Omega_{jnn''}}{\p C_{j'n'}}
A_{nn''}^{}
+\delta_{nn'}^{} V_n^{}\sum_m\nu_{jm}^{}\frac{\p I_{mn}}{\p C_{j'n}},\\
%R_j:C_c'
\left(\dfrac{\p R_{jn}}{\p C_{cn'}^\m}\right)_{C_j^{},\,S_{ic}^\m}^{} &\eq \delta_{nn'}\left[ 
\sum_{i} \nu_{ji}^{c} \frac{\p S_{ic}^{\im,k+1}}{\p C_{cn}^\m}
\right] \frac{V_n}{\Delta t} \\
%R_j:S_i'c'
\left(\dfrac{\p R_{jn}}{\p S_{icn'}^\m}\right)_{C_j^{},\,C_c^\m}^{} &\eq\delta_{nn'}\nu_{ji}^c 
%\left[
\varphi_n^{} s_{ln}^{} 
%+\frac{\p S_{ic}^{s,k+1}}{\p S_{icn}^\m}\right] 
\frac{V_n}{\Delta t} 
+\sum_{n''}q_{cnn''}\sum_{ic}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{l,k+1}}{\p S_{icn'}^\m} A_{nn''}^{},
\EA
\end{subequations}
\begin{subequations}
\BA
%R_c:C_j'
\left(\dfrac{\p R_{cn}}{\p C_{j'n'}}\right)_{C_c^\m,\,S_{ic}^\m}^{} & \eq 0,\\
%R_c:C_c'
\left(\dfrac{\p R_{cn}}{\p C_{c'n'}^\m}\right)_{C_j,\,S_{ic}^\m}^{} &\eq\varphi_n^{} s_{ln}^{}\delta_{nn'}^{}\delta_{cc'}^{}
\left(1+\frac{\p C_{cn}^\im}{\p C_{cn}^\m}\right) \frac{V_n}{\Delta t} 
+ \delta_{cc'}\sum_{n''} q_{cnn''} \frac{\p C_{cnn''}^\m}{\p C_{cn'}^\m} A_{nn''},\\
%R_c:S_i'c'
\left(\dfrac{\p R_{cn}}{\p C_{i'c'n'}^\m}\right)_{C_j,\,C_c^\m}^{} &\eq 0,
\EA
\end{subequations}
\begin{subequations}
\BA
%R_ic:C_j'
\left(\dfrac{\p R_{icn}}{\p C_{j'n'}}\right)_{C_c^\m,\,S_{ic}^\m}^{} &\eq \varphi_n^{} s_{ln}^{}\delta_{nn'}\frac{\p S_{icn}^\m}{\p C_{j'n}} \frac{V_n}{\Delta t} + \sum_{n''} q_{cnn''} \frac{\p S_{ic,nn''}^\m}{\p C_{j'n'}^{}} A_{nn''}-\delta_{nn'} V_n\frac{\p I_{icn}^\m}{\p C_{j'n}},\\
%R_ic:C_c'
\left(\dfrac{\p R_{icn}}{\p C_{c'}^\m}\right)_{C_j,\,S_{ic}^\m}^{} &\eq \varphi_n^{} s_{ln}^{}\delta_{nn'}^{}\delta_{cc'}^{} \frac{\p S_{icn}^\m}{\p C_{cn}}\frac{V_n}{\Delta t} + \delta_{cc'}^{}\sum_{n''} q_{cnn''} \frac{\p S_{ic,nn''}^\m}{\p C_{cn'}^\m} A_{nn''}\nonumber\\
&\hspace{5cm} +\delta_{nn'}\delta_{cc'} V_n\left(\frac{\p\widetilde I_{cn}}{\p C_c^\m}-\frac{\p I_{icn}^\m}{\p C_{cn}}\right),\\
%R_ic:S_i'c'
\left(\dfrac{\p R_{icn}}{\p S_{i'c'}^\m}\right)_{C_j,\,C_c^\m}^{} &\eq \varphi_n^{} s_{ln}^{}\delta_{nn'}\delta_{ii'}\delta_{cc'} \frac{V_n}{\Delta t} + \delta_{cc'}\sum_{n''} q_{cnn''} \frac{\p S_{ic,nn''}^\m}{\p S_{icn'}^\m} A_{nn''}\nonumber\\
&\hspace{5cm}-\delta_{nn'}\delta_{cc'} V_n \frac{\p I_{icn}^\m}{\p S_{i'cn}}.
\EA
\end{subequations}

\noindent Auxiliary Derivatives:
\EQ
\frac{\p Q_{ic}}{\p \ln C_j} \eq \nu_{ji}^c Q_{ic}^{},
\EN

\noindent Auxiliary Derivatives (Kinetic Sorption):
\begin{subequations}
\BA
\frac{\p S_{c}^{\im}}{\p\ln C_{j}} &\eq -\frac{N_c}{\Delta t D_c^2} \sum_i \frac{k_{ic}^f}{1+k_{ic}^b\Delta t} \frac{\p Q_{ic}}{\p\ln C_j},\\
%\EN
%\EQ
%\frac{\p S_{ic}^{s}}{\p\ln C_{j}} &\eq\frac{k_{ic}^f}{1+k_{ic}^b \Delta t} \left(
%\frac{N_c}{D_c}\right) \nu_{ji}^c Q_{ic}^{k+1},\\
%\EN
%\EQ
\frac{\p S_{i}^m}{\p\ln C_{j}} &\eq \frac{k_{im}^f}{1+k_{im}^b \Delta t} \left(
\frac{N_m}{D_m}\right) \nu_{ji}^m Q_{im}^{k+1},\\
%\EN
%\EQ
\frac{\p S_{ic}^\im}{\p\ln C_{c}^\m} &\eq C_c^\m\frac{S_{ic}^\im}{C_c^\im}\frac{k_c^f \Delta t}{1+k_c^f\Delta t},\\
%\EN
%\EQ
\frac{\p I_{icn}^\m}{\p\ln C_{j'n}} &\eq k_{ic}^f \ldots,\\
%\frac{\p S_{ic,n}^\m}{\p C_{cn'}^\m} &\eq ,\\
\frac{\p I_{icn}^\m}{\p S_{i'cn}} &\eq -k_{ic}^b,\\
%\EN
%\EQ
\frac{\p C_{c}^\im}{\p C_{c}^\m} &\eq \frac{k_c^f \Delta t}{1+k_c^f\Delta t}.
\EA
\end{subequations}

\noindent Auxiliary Derivatives (Local Equilibrium Sorption):
\begin{subequations}
%\EQ
%\frac{\p C_{c}^\im}{\p C_{c}^\m} \eq K_{ls}^{-1},
%\EN
\BA
\frac{\p S_{c}^{\m,\,\im}}{\p C_{j}} &\eq \\
%\EN
%\EQ
\frac{\p S_{ic}^{\m,\,\im}}{\p\ln C_{j}} &\eq S_{ic}^{\m,\,\im}\left[
\nu_{ji}^c - \frac{1}{\omega_{\m,\,\im}}\sum_{i'}\nu_{ji'}^c S_{i'c}^{\m,\,\im}
\right],\\
%\EN
%\EQ
\frac{\p S_{i}^m}{\p\ln C_{j}} &\eq S_{i}^{m}\left[
\nu_{ji}^m - \frac{1}{\omega_{m}}\sum_{i'}\nu_{ji'}^m S_{i'}^{m}
\right],\\
%\EN
%\EQ
\frac{\p S_{ic}^{\im}}{\p\ln C_{c'}^{\im}} &\eq \delta_{cc'} S_{ic}^{\im},\\
%\EN
%\EQ
\frac{\p S_{ic}^\im}{\p\ln C_{c'}^\m} &\eq \delta_{cc'} C_c^\m \frac{S_{ic}^{\im}}{C_c^\im}\frac{k_c^f \Delta t}{1+k_c^f\Delta t}.
\EA
\end{subequations}
...
For the case of species-independent diffusion coefficients, because the homogeneous and heterogeneous reactions conserve charge, 
%expressed by Eqn.\eqref{chrg},
...
Multiplying the primary species transport equations 
%Eqn.\eqref{pri} 
by $z_j$ and summing over all primary species noting that $\sum_{jm}z_j\nu_{jm}I_m=\sum_mz_mI_m=0$, 
%from Eqn.\eqref{chrg}, 
gives
\EQ
...
\EA
after rearranging terms. Defining the quantities
\EQ
...
the primary species transport equations 
%Eqn.\eqref{pri} 
with flux given by Eqn.\eqref{totflux} are multiplied by the valence $z_j$ and summed over all primary species to yield
\EQ\label{chrgcon}
...
\section{Multi-Region Model}

For a multi-region continuum model with primary continuum $\a$ and secondary continua denoted by $\b$, and with multirate sorption and mineral precipitation and dissolution the transport equations read
\begin{subequations}
\EQ\label{acont}
\frac{\p}{\p t} \varphi_\a\Psi_j^\a + \bnabla\cdot\bOmega_j^\a \eq - \sum_\b\Gamma_{\a\b} \big(\Psi_j^\a - \Psi_j^\b\big) -\sum_{il}\nu_{ji} \frac{\p S_{il}^\a}{\p t} -\sum_m\nu_{jm}I_m^\a,
\EN
...
\EN
\end{subequations}
for secondary continua. Note that transport is assumed to take place only in the primary continuum $\a$ and not the secondary continua $\b$, each of which are considered to represented by a single compartment. These equations are augmented by the kinetic sorption equations for each continuum given by
\begin{subequations}
\EQ\label{bcont}
...
\BA
\frac{\p}{\p t} \left(\varphi_\a\Psi_j^\a + \sum_\b\varphi_\b\Psi_j^\b \right) + \bnabla\cdot\bOmega_j^\a \eq -\sum_{il}\nu_{ji}&\left(\frac{\p S_{il}^\a}{\p t} + \sum_\b\frac{\p S_{il}^\b}{\p t}\right)\nonumber\\
& -\sum_m\nu_{jm}\big(I_m^\a + \sum_\b I_m^\b\big).
\EA
As can be seen from the form of this equation it is not possible to eliminate the secondary continua variables.
...
\EQ\label{totmassbal}
\frac{\p}{\p t} \left(\varphi\sum_\a s_\a\rho_\a\right) + \bnabla\cdot\sum_\a \rho_\a \bv_\a \eq -\sum_m W_m I_m,
\EN
...
\EQ
\sum_\a\left\{\frac{\p}{\p t} \big(\varphi s_\a \rho_\a U_\a\big) + \bnabla\cdot\big(\bq_\a \rho_\a H_\a\big) \right\} + \frac{\p}{\p t} \big((1-\varphi)\rho_r C_p T \big) - \bnabla\cdot\big(\kappa\bnabla T\big) \eq Q,
\EN
as the sum of contributions from different fluid phases and rock,
with internal energy $U_\a$ and enthalpy $H_\a$ of fluid phase $\a$, and rock heat capacity $C_p$ and thermal conductivity $\kappa$. Note that
\EQ
...
\EN
with viscosity $\mu_\a$, formula weight $W_\a$, acceleration of gravity $g$, and height $z$.
...
\subsection{Air-Water System}

For the air-water two-phase system the Flash method is also applicable provided the air component does not vanish. To derive expressions for the coefficients $K_1$ and $K_2$ defined through the equations
\BA
x_v^g &\eq K_1 x_v^l,\\
x_a^g &\eq K_2 x_a^l,
\EA
the following relations may be used. The total pressure is related to the partial pressures for air and water vapor as
\EQ
P \eq P_a + P_v,
\EN
with the vapor pressure $P_v$ equal to the saturation pressure $P_{\rm sat}$
\EQ
P_v \eq P_{\rm sat}(T).
\EN
The partial pressure of air is related to the mole fraction of air in the aqueous phase through Henry's equation
\EQ
P_a \eq K_a^{} x_a^l,
\EN
with
...
It then follows that
\EQ
x_a^g \eq \frac{P_a}{P} \eq \frac{K_a}{P}x_a^l,
\EN
and hence
\EQ
K_2 \eq \frac{K_a}{P}.
...
and
\EQ
x_v^g\eq 1-x_a^g \eq 1-\frac{K_a}{P} x_a^l.
\EN
Hence
\EQ
K_1 \eq \frac{x_v^g}{x_v^l} \eq \frac{P_{\rm sat}}{P} \frac{1}{1-\dfrac{P}{K_a}\Big(1-\dfrac{P_{\rm sat}}{P}\Big)}.
\EN
Note that if $P_a=0$, then $P=P_{\rm sat}$, and $K_1 = 1$, and the Flash method is not applicable.
...
Solving Eqn.\eqref{massactco2a} for the $\c$ partial pressure gives
\BA
...
\subsection{Supercritical CO$_2$--H$_2$O Equilibrium (Duan \& Sun, 2003)}
...
\EN
%Of the three quantities: $P_g$, $P_\c^g$, and $T$, two may be specified and the other computed from Eqn.\eqref{totalp1}.
...
\EQ
{ds'}^2 \eq ds^2 + 2 \sum_{ik} \ee_{ik} dx_i dx_k,
\EN
where the strain tensor is defined as
\BA
\ee_{ik} &\eq \frac{1}{2}\left(\frac{\p u_i}{\p x_k} + \frac{\p u_k}{\p x_i} + \sum_l \frac{\p u_l}{\p x_i} \frac{\p u_l}{\p x_k}\right),\\
&~\simeq~ \frac{1}{2}\left(\frac{\p u_i}{\p x_k} + \frac{\p u_k}{\p x_i}\right).
\EA
The strain tensor is symmetric $(\ee_{ik} \!=\!\ee_{ki})$ implying that it may be diagonalized
\EQ
\ee_{ik} \eq u^{(i)} \delta_{ik}.
\EN
A volume element $dV$ to first order transforms as
\EQ
dV' \eq dV \big(1 + \sum_i \ee_{ii}\big) \eq dV \big(1+ \sum_i \ee^{(i)}\big),
\EN
with
\EQ
\sum_i\ee_{ii} \eq \sum_i\frac{\p u_i}{\p x_i} \eq \bnabla\cdot\bu.
\EN
...
\paragraph{Energy}
\EQ
d\E \eq T dS + \sum_{ik} \sigma_{ik} d\ee_{ik}.
\EN
\EQ
\sigma_{ik} \eq \left(\frac{\p\E}{\p \ee_{ik}}\right)_S.
\EN
...
\EN
\EQ
dF \eq -S dT + \sum_{ik} \sigma_{ik} d\ee_{ik}.
\EN
\EQ
\sigma_{ik} \eq \left(\frac{\p F}{\p \ee_{ik}}\right)_T.
\EN
\paragraph{Gibbs Free Energy}
\EQ
G \eq \E- TS + \sum_{ik} \sigma_{ik} \ee_{ik}.
\EN
\EQ
dG \eq -S dT - \sum_{ik} \ee_{ik} d\sigma_{ik}.
\EN
\EQ
\ee_{ik} \eq -\left(\frac{\p G}{\p\sigma_{ik}}\right)_T.
\EN
...
\BA
F &\eq F_0 + \frac{1}{2} \sum_{i} \lambda \ee_{ii}^2 + \mu  \sum_{ik} \ee_{ik}^2 + \ldots,\\
&\eq \mu \big(\ee_{ik} - \frac{1}{3} \delta_{ik} \sum_{l} \ee_{ll}\big)^2 + \frac{1}{2} K  \sum_{l} \ee_{ll}^2,
\EA
...
\BA
\sigma_{ik} &\eq \left(\frac{\p F}{\p \ee_{ik}}\right)_T,\\
&\eq K \delta_{ik} \sum_{l} \ee_{ll} + 2 \mu \big(\ee_{ik}-\frac{1}{3} \delta_{ik} \sum_{l} \ee_{ll}\big),\\
&\eq 2 \mu \ee_{ik} + \lambda \delta_{ik} \sum_l \ee_{ll},
\EA
with
\EQ
K\eq \lambda + \frac{2}{3} \mu.
...
Conversely
\EQ
\ee_{ik} \eq \frac{1}{9K} \delta_{ik} \sum_{l} \s_{ll} + \frac{1}{2\mu} \big(\s_{ik} - \frac{1}{3} \delta_{ik} \sum_{l} \s_{ll}\big).
\EN
It follows that
\EQ
\sum_{l}\ee_{ll} \eq \frac{1}{3K}\sum_{l}\s_{ll},
\EN
or
\EQ
K \eq \frac{1}{3}\frac{\sum_{l}\s_{ll}}{\sum_{l}\ee_{ll}}.
\EN

According to Euler's theorem
\EQ
F \eq \frac{1}{2} \sum_{ik} \s_{ik} \ee_{ik}.
\EN

From the quadratic form of the elastic energy
\EQ
\ee_{ik} \eq \frac{\p F}{\p\s_{ik}}.
\EN
...
implying 
\EQ
\ee_{ik} \eq -\frac{p}{3K} \delta_{ik},
\EN
and
\EQ
\sum_l \ee_{ll} \eq -\frac{p}{K}.
\EN

\subsection{Homogeneous Deformation}

\EQ
F \eq \frac{E}{2(1+\s)} \left[\ee_{ik}^2 + \frac{\s}{1-2\s} \sum_l \ee_{ll}^2\right],
\EN
\EQ
\s_{ik} \eq \frac{E}{1+\s}\left(\ee_{ik} + \frac{\s}{1-2\s}\delta_{ik} \sum_l \ee_{ll} \right),
\EN
\EQ
\ee_{ik} \eq \frac{1}{E} \left[(1+\s)\s_{ik} - \s\delta_{ik} \sum_l \s_{ll}\right],
\EN
\begin{subequations}
\EQ
\s_{xx} \eq \frac{E}{(1+\s)(1-2\s)} \left[\big(1-\s\big) \ee_{xx} + \s \big(\ee_{yy}+\ee_{zz}\big)\right],
\EN
\EQ
\s_{yy} \eq \frac{E}{(1+\s)(1-2\s)} \left[\big(1-\s\big) \ee_{yy} + \s \big(\ee_{xx}+\ee_{zz}\big)\right],
\EN
\EQ
\s_{zz} \eq \frac{E}{(1+\s)(1-2\s)} \left[\big(1-\s\big) \ee_{zz} + \s \big(\ee_{xx}+\ee_{yy}\big)\right],
\EN
\EQ
\s_{xy}\eq\frac{E}{1+\s} \ee_{xy}, \ \s_{xz}\eq\frac{E}{1+\s} \ee_{xz}, \ \s_{yz}\eq\frac{E}{1+\s} \ee_{yz}.
\EN
\end{subequations}
...
\EN
\EQ
\ee_{xx} \eq \ee_{yy} \eq -\frac{1}{3}\left(\frac{1}{2\mu}-\frac{1}{3K}\right)p,
\EN
\EQ
\ee_{zz} \eq \frac{1}{3}\left(\frac{1}{3K}+\frac{1}{\mu}\right)p.
\EN
\EQ
\ee_{zz} \eq \frac{p}{E},
\EN
...
\EQ
\ee_{ik} \eq \frac{1}{2G}\s_{ik} - \frac{2\nu}{E} \sum_l \s_{ll} + \frac{1}{H} p,
\EN
\EQ
\s_{ik} \eq 2\mu \ee_{ik} + \left[\lambda\sum_l\ee_{ll} - \a p\right] \delta_{ik}.
\EN
\EQ
\sum_l\s_{ll} \eq \frac{E}{1-2\nu} \sum_l\ee_{ll} -\frac{E}{1-2\nu}\frac{p}{H}.
\EN
\EQ
\a \eq \frac{2G(1+\nu)}{3H(1-2\nu)} \eq \frac{E}{3H(1-2\nu)}, \ \ \ G \eq \frac{E}{2(1+\nu)}.
\EN
...
\EQ
F(T,\, \ee_{ik}) \eq F_0(T)-K\b(T-T_0) \sum_l \ee_{ll} + \mu \big(\ee_{ik}-\frac{1}{3}\delta_{ik}\sum_l \ee_{ll}\big)^2+\frac{1}{2}K \sum_l \ee_{ll}^2,
\EN
\EQ
\s_{ik} \eq -K \b (T-T_0) \delta_{ik} + K \delta_{ik} \sum_l \ee_{ll} + 2\mu \big(\ee_{ik} - \frac{1}{3} \delta_{ik} \sum_l \ee_{ll}\big).
\EN
...
\EQ
\bq \eq -\frac{kk_r(s)}{\mu} \bnabla\big(p-\rho g z\big),
\EN
...
R_n^{(k+1)} \eq \frac{(\varphi s\rho)_n^{(k+1)} - (\varphi s\rho)_n^{(k)}}{\Delta t_{k}} V_n + \sum_{n'} A_{nn'}^{} F_{nn'}^{(k+1)} \eq 0,
\EN
...
\EQ
F_{nn'} \eq - \rho_{nn'} \left(\frac{kk_r}{\mu}\right)_{nn'}\left[\frac{P_{n'}-P_n-W\rho_{nn'} g z_{nn'}}{d_{n'}+d_n}\right],
\EN
...
%&\eq \frac{d_{n'}\rho_{n'}+d_{n}\rho_n}{d_{n'}+d_n}.
%\EA
%\end{comment}
The density $\rho_{nn'}$ at the interface is computed according to the weighted mean
\BA
\rho_{nn'} \eq \frac{d_{n'}\rho_{n}+d_{n}\rho_{n'}}{d_{n'}+d_n},
\EA
with the property that as $d_n\rightarrow 0$, $\rho_{nn'}\rightarrow \rho_n$.
The permeability is computed according to the harmonic mean
\EQ
k_{nn'} \eq \frac{(d_{n'}+d_n) k_{n'}k_{n}}{d_n k_{n'} + d_{n'} k_{n}}.
\EN
...
\EN
where $\omega_{nn'}=1$ if $q_{nn'}>0$ (node $n$ upstream), and $\omega_{nn'}=0$ if $q_{nn'}<0$ (node $n'$ upstream).
...
\EQ
F_{nb} \eq -\rho_{nb} \left(\frac{kk_r}{\mu}\right)_{nb} \left[\frac{P_{b}-P_n-W\rho_{nb} g z_{nb}}{d_{nb}}\right],
\EN
...
\section{Newton-Raphson Equations}

The Newton-Raphson equations have the form
\EQ
\bJ \bdelta X \eq -\bR,
\EN
with residual function $\bR$ and Jacobian matrix $\bJ$ with unknown $X$ equal to the pressure $p$. In integrated finite volume form
\EQ
R_n \eq \Delta_t M_n + \Delta f_{n}.
\EN

Writing for the accumulation term
\EQ
\Delta_t M_n \eq \frac{M_n^{k+1}-M_n^k}{\Delta t} V_n \eq \frac{(\varphi s\rho)_n^{(k+1)} - (\varphi s\rho)_n^{(k)}}{\Delta t} V_n.
\EN
with
\EQ
M_n^{k+1} \eq \frac{(\varphi s\rho)_n^{(k+1)}}{\Delta t} V_n.
\EN
The derivative of the accumulation term is given by
\BA
\delta M_n &\eq \frac{V_n}{\Delta t} \varphi (s_n \delta \rho_n + \rho_n \delta s_n),\\
&\eq \frac{V_n}{\Delta t} \varphi (s_n \rho_{np} + \rho_n s_{np}) \delta p_n.
\EA

The flux contribution $\Delta f_n$ has the form
\BA
\Delta f_{n} \eq \T_{n+1,n} \,\rho_{n+1,n} \,\lambda_{n+1,n} \,\Phi_{n+1} - \T_{n,n-1}\,\rho_{n,n-1} \,\lambda_{n,n-1}\Phi_n,
\EA
for the interface between nodes $n$ and $n-1$, with potential
\EQ
\Phi_{n+1} \eq P_{n+1}-P_n-W\rho_{n+1,n}\, g\, z_{n+1,n},
\EN
mobility
\EQ
\lambda_{n,n-1} \eq \omega_{n,n-1}\lambda_{n} + (1-\omega_{n,n-1})\lambda_{n-1},
\EN
with weighting factor $\omega_{n,n-1}$ ($\omega=1$ node $n$ upstream, $\omega=0$ node $n$ downstream), and transmissibility
\EQ
\T_{nn'} \eq A_{nn'} \frac{k_{n'}k_{n}}{d_n k_{n'} + d_{n'} k_{n}}.
\EN
The derivative of the flux has the form
\BA
\delta f_n &\eq \T_{n+1,n} \Big( \delta \rho_{n+1,n} \, \lambda_{n+1,n} \, \Phi_{n+1}
+ \rho_{n+1,n} \, \delta \lambda_{n+1,n} \, \Phi_{n+1}
+ \rho_{n+1,n} \, \lambda_{n+1,n} \, \delta \Phi_{n+1}\Big),
\EA
with
\BA
\delta \rho_{n+1,n} \eq \dfrac{d_{n}}{d_{n+1}+d_n} \rho_{n+1,p} \,\delta p_{n+1} +
\dfrac{d_{n+1}}{d_{n+1}+d_n}\rho_{np} \,\delta p_n,
\EA
\EQ
\delta\lambda_{n,n-1} \eq \omega_{n,n-1}\delta\lambda_{n} + (1-\omega_{n,n-1})\delta\lambda_{n-1},
\EN
and
\EQ
\delta\Phi_{n+1} \eq \delta p_{n+1} - \delta p_n - W \delta\rho_{n+1,n} \,g \,\Delta z_{n+1,n}.
\EN
