...
\EA
\end{subequations}
where $\widetilde\bR_{mM}^{\b}$ denotes the contribution to the residual function which does not contain the boundary concentration equal to the primary continuum concentration $\bPsi_m^\a$ at the $n$th node and $d_M\!=\!d_{\a\b}$. These equations form a tri-diagonal system of nonlinear equations which can be solved using the techniques outlined in the Appendix to this section.
...
\EA
\end{subequations}
with known quantities $\ba_m$ and $\bb_m$ defined as
\begin{subequations}
\BA
