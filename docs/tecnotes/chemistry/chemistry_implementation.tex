...
\noindent {\bf Residual Function Independent and Dependent Variables} 
...
\bJ \delta\bx \eq -\bR
\EN
\noindent Equilibrium Colloid/Mineral Sorption Reactions:
\EQ
\left(\begin{array}{cc} 
...
%R_j:C_c'
\left(\dfrac{\p R_{jn}}{\p C_{c'n'}^l}\right)_{C_j^{}}^{} &\eq \delta_{jc'}\delta_{nn'}\left[ 
\varphi_n^{} s_{ln}^{}\sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{l,k+1}}{\p C_{c'n}^l} 
+ \sum_{ic} \nu_{ji}^{c} \frac{\p S_{ic}^{s,k+1}}{\p C_{c'n}^l}
\right] \frac{V_n}{\Delta t} \nonumber\\
&\qquad\qquad+\sum_{n''} q_{cnn''}\sum_{ic}\nu_{ji}^{c} \frac{\p S_{ic,nn''}^{l,k+1}}{\p C_{c'n'}^l} A_{nn''}^{}.
\EA
\end{subequations}

\noindent Kinetic Colloid/Mineral Sorption Reactions:
