...
The multirate formulation is considered only for surface complexation reactions, but ion exchange could also be implemented if desired. Consider surface complexation reactions of the form
\EQ
\nu_\a >\!\!\chi_\a + \sum_j\nu_{ji} \A_j \arrows >\!\! \S_{i\a},
\EN
for the $i$th surface complex $>\!\!\S_{i\a}$ on site $\a$ and empty site $>\!\!\chi_\a$. For simplicity, in what follows it is assumed that $\nu_\a\!=\!1$. 

With each site $\a$ is associated a rate constant $k_\a$ and site concentration $\omega_\a$. These quantities are defined through a given distribution of sites $\P(\a)$, such that
\EQ
\int_0^\infty \P(k_\a)dk_\a \eq 1.
\EN
The fraction of sites $f_\a$ belonging to site $\a$ is determined from the relation
\EQ
f_\a \eq \int_{k_\a-\Delta k_\a/2}^{k_\a+\Delta k_\a/2} \P(k_\a)dk_\a \simeq \P(k_\a)\Delta k_\a,
\EN
...
The rate of reaction is described through a kinetic relation given by
\EQ\label{sorbed}
...
The solute transport equations become
\EQ
...
An alternative form of these equations is to introduce the total sorbed concentration for the $j$th primary species defined as
\EQ
q_{j\a}\eq\sum_i \nu_{ji}S_{i\a}.
\EN
Then the transport equations become
\EQ
\frac{\p}{\p t}\left(\varphi \Psi_j + \sum_{\a}q_{j\a}\right) + \bnabla\cdot\bOmega_j \eq  - \sum_m\nu_{jm}I_m.
\EN
The sorbed concentrations are obtained from the equations
\EQ
\frac{\p q_{j\a}}{\p t} \eq k_\a^{} \big(q_{j\a}^{\rm eq}-q_{j\a}\big),
\EN
where $q_{j\a}^{\rm eq}$ is defined as
\EQ\label{qeq}
q_{j\a}^{\rm eq} \eq \sum_i \nu_{ji}^{} S_{i\a}^{\rm eq}\eq \frac{\omega_\a}{1+\sum_l K_lQ_l} \sum_i \nu_{ji}K_i Q_i.
\EN
...
\EQ\label{sorbnum}
\frac{q_{j\a n}^{t+\Delta t}-q_{j\a n}^t}{\Delta t} \eq k_\a\big(q_{j\a n}^{{\rm eq},\,t+\Delta t}-q_{j\a n}^{t+\Delta t}\big),
\EN
for fully backward implicit time differencing. Solving for $q_{j\a}^{t+\Delta t}$ at the new time $t\!+\!\Delta t$ gives
\EQ\label{qkin}
q_{j\a n}^{t+\Delta t} \eq \frac{q_{j\a n}^t+k_\a \Delta t \, q_{j\a}^{{\rm eq},\,t+\Delta t}}{1+k_\a\Delta t}.
\EN
Thus the sorption rate equal to the right-hand side of Eqn.\eqref{sorbnum} becomes
\EQ
k_\a\big(q_{j\a n}^{{\rm eq},\,t+\Delta t}-q_{j\a n}^{t+\Delta t}\big) \eq k_\a\frac{q_{j\a n}^{{\rm eq},\,t+\Delta t}-q_{j\a n}^{t}}{1+k_\a\Delta t}.
\EN
This result can be substituted directly into the residual function for the finite difference form corresponding to the $j$th primary species equation 
\BA
R_{jn} \eq \frac{\big(\varphi_n \Psi_{jn}\big)_{t+\Delta t}-\big(\varphi_n \Psi_{jn}\big)_t}{\Delta t}V_n &+ \sum_\a \frac{q_{j\a n}^{t+\Delta t}-q_{j\a n}^t}{\Delta t}V_n \nonumber\\
&+ \sum_{n'} \Omega_{jnn'} A_{nn'} + \sum_m\nu_{jm}I_{mn}V_n,
\EA
with control volume $V_n$ and interfacial area $A_{nn'}$, becomes
\BA
R_{jn} \eq \frac{\big(\varphi_n \Psi_{jn}\big)_{t+\Delta t}-\big(\varphi_n \Psi_{jn}\big)_t}{\Delta t}V_n &+ \sum_\a \frac{k_\a}{1+k_\a\Delta t}\big(q_{j\a n}^{{\rm eq},\,t+\Delta t}-q_{j\a n}^{t}\big)V_n \nonumber\\
&+ \sum_{n'} \Omega_{jnn'} A_{nn'} + \sum_m\nu_{jm}I_{mn}V_n,
\EA
where $q_{j\a n}^{{\rm eq},\,t+\Delta t}$ is given explicitly as a function of the primary species concentrations in Eqn.\eqref{qeq}. Once the primary species transport equations are solved, the sorbed concentrations can be updated from Eqn.\eqref{qkin}.
...
\BA\label{pricolloid}
\frac{\p}{\p t}\bigg(\varphi\Psi_j + \sum_{im} \nu_{ji}^m S_{im} &+ \sum_{ic} \nu_{ji}^c \big(\varphi S_{ic}^f+S_{ic}^s\big)\bigg) + \bnabla\bdot\bigg(\bOmega_j +\bq_c \sum_{ic}\nu_{ji}^c S_{ic}^f\bigg) \eq \nonumber\\
