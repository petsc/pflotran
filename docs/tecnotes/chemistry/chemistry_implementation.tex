...
\EQ\label{sorbnum}
\frac{q_{j\a}^{t+\Delta t}-q_{j\a}^t}{\Delta t} \eq k_\a\big(q_{j\a}^{\rm eq}-q_{j\a}^{t+\Delta t}\big),
\EN
for a fully backward implicit time differencing. Solving for $q_{j\a}$ at the new time $t+\Delta t$ gives
\EQ\label{qkin}
q_{j\a}^{t+\Delta t} \eq \frac{q_{j\a}^t+k_\a \Delta t \, q_{j\a}^{\rm eq}}{1+k_\a\Delta t}.
\EN
Thus the sorption rate equal to the right-hand side of Eqn.\eqref{sorbnum} becomes
\EQ
k_\a\big(q_{j\a}^{\rm eq}-q_{j\a}^{t+\Delta t}\big) \eq k_\a\frac{q_{j\a}^{\rm eq}-q_{j\a}^{t}}{1+k_\a\Delta t}.
\EN
...
becomes
\EQ
R_{jn} \eq \varphi \frac{\Psi_{jn}^{t+\Delta t}-\Psi_{jn}^t}{\Delta t} + \sum_\a \frac{k_\a}{1+k_\a\Delta t}\big(q_{j\a}^{\rm eq}-q_{j\a}^{t}\big) + \sum_{n'} \Omega_{jnn'} A_{nn'} + \sum_m\nu_{jm}I_{mn},
\EN
