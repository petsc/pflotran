...
Irreducible liquid saturation described by the Brooks-Corey (19??) relation 
\EQ\label{seffbc}
...
\EN
\BA
\delta M_{P} &\eq \varphi (\delta\rho_l Z+\rho_l\delta Z),\nonumber\\
&\eq \varphi(\rho_{lP}^{}\delta P + \rho_l\delta Z).
\EA
...
\noindent $Z_\c$:
\EQ
M_{Z} \eq \varphi Z(s_l\rho_l+s_g\rho_g),
\EN
\BA
\delta M_{Z} &\eq \varphi Z(s_l^{}\delta\rho_l^{}+s_g^{}\delta\rho_g^{})+\varphi Z(\rho_g^{}-\rho_l^{}) \delta s_g + \varphi (s_l\rho_l^{}+s_g\rho_g^{}) \delta Z,\nonumber\\
&\eq \varphi Z(s_l\rho_{lP}^{}+s_g^{}\rho_{gP}^{})\delta P + \varphi Z(\rho_g^{}-\rho_l^{}) s_{gP}^{} \delta P \nonumber\\
&\qquad + \varphi Z(\rho_g^{}-\rho_l^{}) s_{gZ}^{} \delta Z + \varphi (s_l^{}\rho_l^{}+s_g^{}\rho_g^{}) \delta Z.
\EA
...
The flux in the $x,\,y,\,z$-directions for the pressure equation is defined as
\begin{subequations}
\EQ
...
The liquid flux at the $i-1/2$ face is given by
\EQ
...
\EN
The mobility $\lambda_l$ given by
\EQ
\lambda_l \eq \frac{k_{l}}{\mu_l},
\EN
where $k_l$ denotes the relative permeability in general a nonlinear function of saturation, and $\gamma_l$ is defined by
\EQ
...
where
\EQ
\Phi_{gi} \eq \Delta P_i^g - \gamma_g \Delta z_i,
\EN
with
\EQ
...
& \left. \qquad + \rho_{l,i-1/2} \, \lambda_{l,i-1/2} \, (\delta \Delta P_{i}^l -\delta \gamma_l \Delta z_i)\right],
...
where
\begin{align}
\delta \lambda_l &= \frac{d \lambda_l}{ds_g} \delta s_g,\\
...
\end{align}
The change in $\c$ flux in the liquid phase can be written in terms of the pressure equation flux and a diffusion term
\EQ
\delta \left(f_l X_c^l + f_D^l\right)_{i-1/2} \eq X_{cu}^l \delta f_{l,i-1/2} + f_{l,i-1/2} \delta X_{c,i-1/2}^l + \delta (f_D^l)_{i-1/2}.
\EN
where
\EQ
(f_D^l)_{i-1/2} \eq -\frac{(\varphi s_l \rho_l D_l)_{i-1/2}}{d_i+d_{i-1}} \big(X_{\c,i}^l-X_{\c,i-1}^l\big).
\EN
Upstream weighting is used for the $\c$ mole fraction $X_\c^l$.
...
\section{References}
\begin{description}
\item Duan Z., and Sun, R. (2003) An improved model calculating CO2 solubility in pure water and aqueous NaCl solutions from 273 to 533 K andfrom 0 to 2000 bar, Chemical Geology 193, 257--271.
...
\item Li D., and Duan, Z. (2007)The speciation equilibrium coupling with phase equilibrium in the H2OÐCO2ÐNaCl system from 0 to 250 $^\circ$C, from 0 to 1000 bar,and from 0 to 5 molality of NaCl, Chemical Geology 244, 730--751.
