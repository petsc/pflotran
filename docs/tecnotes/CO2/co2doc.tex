...
For an isothermal system the governing equations for the two-component system consisting of H$_2$O and CO$_2$ with independent variables $P$ and $Z$, have the following form for the pressure equation
\EQ
\frac{\p}{\p t} M + \bnabla\cdot\bf \eq Q,
\EN
and
\EQ
\frac{\p}{\p t} MZ + \bnabla\cdot(\bf_l X_c^l + \bf_g X_c^g + \bf_{Dc}^l + \bf_{Dc}^g) \eq Q Z,
\EN
for the total $c$ = CO$_2$ mole fraction $Z$. In these equations
\EQ
...
with
\EQ
\bf_l^{} \eq \bq_l^{} \rho_l^{} + \bf_{Dc}^l.
\EN
and
\EQ
\bf_g^{} \eq \bq_l^{} \rho_g^{} + \bf_{Dc}^g.
\EN

The Jacobian matrix elements are calculated for a 3D nine-point orthogonal stencil using the notation for the $(i,\,j,\,k)$th-grid block
\BA
R_n \eq \frac{\Delta_t M_n}{\Delta t}V_n &+ f_{i+1/2} A_{i+1/2}-f_{i-1/2} A_{i-1/2} \nonumber\\
...
& \left. \qquad + \rho_{l,i-1/2} \, \lambda_{l,i-1/2} \, (\delta \Delta P_{i}^l - \Delta z_i \delta \gamma_l)\right],
...
\begin{array}{llc}
0, & \text{two-phase:} & X_\c^{l,\rm eq} < Z < X_c^{g,\rm eq}\\
\delta Z, & \text{single phase:} & Z \leq X_\c^{l,\rm eq} \ \text{or} \ X_\c^{g,\rm eq} \leq Z
\end{array}
\right..
\EN
