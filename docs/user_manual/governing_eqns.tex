...
\EN
with scale factor $f_m$ corresponding to a different choice of unit cell. A different scale factor may be used for each mineral. The scaled reaction corresponds to 
\EQ
\sum_j \nu_{jm}' \A_j \arrows \M_m',
\EN
with $\M_m' \!=\! f_m\M_m$, $\nu_{jm}' \!=\! f_m\nu_{jm}$. In addition, to obtain identical results to the unscaled reaction, it is also necessary to scale the mineral molar volume $\overline V_m' \!=\! f_m\overline V_m$, formula weight $W_m'\!=\!f_m W_m$, equilibrium constant $K_m' \!=\! K_m^{f_m}$, and the kinetic rate constant with the inverse factor $k_m'\!=\!f_m^{-1}k_m$. The saturation index ${\rm SI}_m$ transforms according to
\EQ
{\rm SI}_m' \eq K_m' Q_m' \eq \big(K_m Q_m\big)^{f_m} \eq {\rm SI}_m^{f_m}.
\EN
Consequently, equilibrium is not affected as expected. 
However, a more general form for the reaction rate involving Tempkin's constant is needed to obtain the transformation of the reaction rate
\begin{subequations}
\BA
I_m' &\eq -k_m' A_m \big(1-(K_m'Q_m')^{1/\sigma'}\big),\\
&\eq -\frac{1}{f_m} k_m A_m \big(1-(K_m Q_m)^{f_m/\sigma'} \big),\\
&\eq \frac{1}{f_m} I_m.
\EA
\end{subequations}
Thus it is necessary to scale Tempkin's constant $\sigma'\!=\!\sigma/f_m$ [see Eqn.\eqref{Im}].
It follows that the mineral concentration $(C_m' \!=\!({\overline V}_m^{-1})^{'} \phi_m \!=\! f_m^{-1} C_m)$, differs in the two formulations.
However, the mineral volume fraction and solute concentrations are identical since ${\overline V}_m' I_m' \!=\! \overline V_m I_m$, and $\nu_{jm}' I_m' \!=\! \nu_{jm} I_m$.
%\end{comment}
