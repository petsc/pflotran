...
The output in the mass balance file refers to global mass conservation. This output is described in detail for the {\tt MPHASE} mode. The total number of moles of the $i$th component in phase $\a$ is given as the integral over the entire computational domain
\EQ
N_i^\a \eq \int_V \varphi s_\a \eta_\a x_i^\a \, dV.
\EN
From the governing equations the time rate of change of $N_i = \sum_\a N_i^\a$ is given by
\BA
\frac{dN_i}{dt} &\eq \sum_\a \frac{dN_i^\a}{dt} \\
&\eq -\sum_\a \int_{\p V} \bF_i^\a\cdot\bdS + Q_i,
\EA
where the integral on the right hand side is over flux flowing through the boundaries of the domain and the source/sink
\EQ
Q_i \eq \int_V q_i \, dV.
\EN
...
\EN
where $s_\a^0$ denotes the residual saturation. There are as many rows of cumulative and instantaneous fluxes as there are boundaries.
