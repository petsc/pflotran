...
The BOUNDARY\_CONDITION card couples a FLOW\_CONDITION and/or TRANSPORT\_CONDITION to a REGION in the problem domain.  The use of this card enables the use/reuse of flow and transport conditions and regions within multiple boundary and initial conditions and source/sinks in the input deck.
...
\subsubsection{Description}
The BREAKTHROUGH card specifies that data values normally printed to the standard output files be monitored and printed at a specific location (e.g. grid cell or coordinate) throughout the simulation for observation.  These data are printed locally from each processor, thus requiring no parallel I/O, to files named 'breakthrough\_N.tec', where N is the rank of the processor upon which the observation points reside.
\subsubsection{Example}
\shadowbox{
\begin{minipage}{6.5in}
\begin{deflist}{0000000000}
...
\begin{minipage}{6.5in}
\begin{deflist}{0000000000}
\item[REGION] Followed by region\_name, begins a region entry
\item[BLOCK] Defines a volumetric, planar, or point region through IJK indices: i1 i2 j1 j2 k1 k2
\item[COORDINATE] Defines a point region through coordinates in 3D space
\item[COORDINATES] Defines a volumetric, planar, or point region between two coordinates in space
\item[FILE] Defines an HDF5 file within which a dataset named region\_name contains a list of grid cells corresponding to a region.
\item[LIST] (to be implemented)
\item[FACE] (structured grids only) Defines the face of the grid cell to which boundary conditions are connected (i.e. WEST, EAST, NORTH, SOUTH, BOTTOM, TOP])
\item[END] Ends the region entry
\end{deflist}
\end{minipage}
}
