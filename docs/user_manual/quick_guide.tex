...
\EQ
k_{ml} (T) \eq k_{ml}^0 \exp\left[\frac{E_l}{R}\Big(\frac{1}{T_0}-\frac{1}{T}\Big)\right],
\EN
where $k_{ml}^0$ refers to the rate constant at the reference temperature $T_0$ taken as 25\degc, with $T$ in units of Kelvin, $E_l$ denotes the activation energy (kJ/mol),
and the quantity $\P_{ml}$ denotes the prefactor for the $l$th parallel reaction with the form
\EQ
\P_{ml} \eq \prod_i\big(\gamma_i m_i\big)^{\nu_{il}^m},
\EN
where the product index $i$ generally runs over both primary and secondary species.
