...
\subsection{Mode: {\tt 2PH}}

The {\tt 2PH} mode involves two phase liquid water-gas flow coupled to the reactive transport mode. Mass conservation equations have the form
\EQ
...
\bR(\bx) \eq \bzero,
\EN
for the residual function $\bR$ are solved using an iterative solver based on the Newton-Raphson equations
\EQ
...
\noindent{\bf Description:} this keyword defines a spatial grid and specifies the discretization scheme to be employed in the simulation.
...
\item[FILE] \ file name: name of file containing grid information (unstructured only)
\end{deflist}
...
\begin{tabularx}{\linewidth}{lX}
{\bf GRAVITY} <\# \# \#>: &Specifies gravity vector\\

{\bf ORIGIN} <\# \# \#>: &Coordinates of grid origin (default: 0. 0. 0.)\\

{\bf BOUNDS:} &Specifies bounds of structured cartesian grid (see examples below)\\
{\bf DXYZ:} &Specifies grid spacing of structured cartesian grid (see examples below)\\
{\bf INVERT\_Z:} &Inverts the z axis (positive Z is down)
\end{tabularx}
\end{comment}
...
\noindent{\bf Description:} determines the mode of operation: IMMIS (Immisible), MPH, MPHASE, FLASH2 (CO$_2$ + H$_2$O), Richards (vadose-saturated zones), THC (Thermal-Hydrologic-Chemical, in pro\-gress).
