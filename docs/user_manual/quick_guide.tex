...
K_{ji}^\a \eq \frac{\big(k_j^\a\big)^{z_i}}{\big(k_i^\a\big)^{z_j}} \eq \left(\frac{X_j^\a}{a_j}\right)^{z_i} \left(\frac{a_i}{X_i^\a}\right)^{z_j}.
\EN
...
\EQ
{K'}_{ji}^\a \eq \frac{\big({k'}_j^\a\big)^{1/z_j}}{\big({k'}_i^\a\big)^{1/z_i}} \eq \left(\frac{X_j^\a}{a_j}\right)^{1/z_j} \left(\frac{a_i}{X_i^\a}\right)^{1/z_i}.
\EN
...
\EN
for both equilibrium and kinetic reactions and is presumed constant (although it may change through mineral precipitation/dissolution reactions in a mineral specific description).
The equilibrium sorbed concentration $S_{j\a}^{\rm eq}$ referenced to the $j$th primary species is defined as
\EQ\label{qeq}
...
\item[TEMPERATURE] {\tt [dirichlet, neumann, hydrostatic, zero\_gradient]}: \ heat flux [W/m$^2$] can be specified with  {\tt{neumann}} type
