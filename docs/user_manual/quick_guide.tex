...
\begin{subequations}
\BA
&\Big[\varphi \Big(\sum_\a \big(s_\a\rho_\a U_\a\big)_n^{k+1} - \sum_\a \big(s_\a\rho_\a U_\a\big)_n^{k}\Big) + (1-\varphi)\Big(\big(\rho_r C_r T\big)_n^{k+1} - \big(\rho_r C_r T\big)_n^{k}\Big)\Big] \frac{v_n}{\Delta t} \nonumber\\
&\qquad + \sum_{n'} \Big[ \sum_\a\big(q_\a\rho_\a H_\a\big)_{nn'} + \frac{\kappa_{nn'}}{d_n+d_{n'}}\big(T_n-T_{n'}\big) \Big] A_{nn'} \nonumber\\
&\qquad + \frac{\kappa_{nM}}{d_n+d_{M}}\big(T_n-T_{M}\big) A_{nM} \eq 0,
\EA
...
\EA
\end{subequations}
for the $m$th matrix node with volume $v_m$. The matrix node designated by $M$ refers to the outer most node in contact with the fracture. 
...
\EA
\end{subequations}
The interfacial area $A_{nn'}$ between fracture control volumes is equal to $\Delta y \Delta z$,  $\Delta z \Delta x$, $\Delta x \Delta y$ for $x$, $y$, and $z$ directions, respectively. The fracture-matrix interfacial area $A_{nM}$ is equal to
\EQ
A_{nM} \eq 6 d^2.
\EN
...
specified under MATERIAL\_PROPERTY keyword.
