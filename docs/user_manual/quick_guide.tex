...
&\qquad + \sum_\b\frac{\kappa_{nM}^\b}{d_n+d_{M}}\big(T_n^\a-T_{M}^\b\big) A_{nM}^\b \eq 0,
\EA
for the $n$th fracture control volume, where $v_n$ denotes the fracture volume, and
\BA
\Big(\big(\rho_r C_r T\big)_m^{k+1} - \big(\rho_r C_r T\big)_m^{k}\Big) \frac{v_m}{\Delta t} &+ \sum_{m'} \frac{\kappa_{mm'}^\b}{d_m+d_{m'}}\big(T_m-T_{m'}\big) A_{mm'}^\b \nonumber\\
&+ \delta_{mM}^{}\frac{\kappa_{mM}^\b}{d_m+d_{M}}\big(T_m-T_{M}\big) A_{mM}^\b \eq 0,
\EA
\end{subequations}
...
\EA
\end{subequations}
The interfacial area $A_{nn'}$ between fracture control volumes is equal to $\Delta y \Delta z$,  $\Delta z \Delta x$, $\Delta x \Delta y$ for $x$, $y$, and $z$ directions, respectively. 
...
\begin{table}\centering
\caption{DCDM geometric parameters.}\label{tdcdm}
\vspace{3mm}
\begin{tabular}{lcc}
\toprule
