...
The current implementation of the {\tt THC} mode applies only to mass and energy conservation equations and a single tracer ($i=1$ in Eqn.\eqref{thc} below). Future generalizations will include multicomponent variable density fluids.
...
and
\EQ
\frac{\p}{\p t}\big(\varphi s\rho U + (1-\varphi) c_p T\big) + \bnabla\cdot\big(\rho\bq H -\kappa \bnabla T\big) = Q_e,
\EN
