...
\subsubsection{Brook-Corey Saturation Function} 

The Brook-Corey saturation function is a limiting form of the van Genuchten relation for $p_c/p_c^0 \gg 1$, with the form
\EQ
...
The Mualem relative permeability function based on the Brook-Corey saturation function is defined by 
\BA
...
A thermal conduction model employing a multiple continuum model has been added to MPHASE and THC. The formulation is based on Pruess and Narasimhan (1985). The DCDM (dual continuum discrete matrix) model following the classification given in Lichtner (2000) is implemented. In what follows the matrix porosity is assumed to be zero. Using the control volume configuration shown in Figure~\ref{fminc}, with fracture nodes designated by the subscript $n$ and matrix nodes by $m$, the integrated finite volume form of the heat transport equation is given by
\begin{subequations}
\BA
&\Big[\varphi \Big(\sum_\a \big(s_\a\rho_\a U_\a\big)_n^{k+1} - \sum_\a \big(s_\a\rho_\a U_\a\big)_n^{k}\Big) 
 + (1-\varphi)\Big(\big(\rho_r C_r T\big)_n^{k+1} - \big(\rho_r C_r T\big)_n^{k}\Big)\Big] \frac{v_n}{\Delta t} \nonumber\\
&+ \sum_{n'} \Big[ \sum_\a\big(q_\a\rho_\a H_\a\big)_{nn'} + \frac{\kappa_{nn'}}{d_n+d_{n'}}\big(T_n-T_{n'}\big) \Big] A_{nn'} \nonumber\\
&+ \frac{\kappa_{nM}}{d_n+d_{M}}\big(T_n-T_{M}\big) A_{nM} \eq 0,
\EA
for the $n$th fracture control volume, where $v_n$ denotes the fracture volume, and
\BA
\Big(\big(\rho_r C_r T\big)_m^{k+1} - \big(\rho_r C_r T\big)_m^{k}\Big) \frac{v_m}{\Delta t} &+ \sum_{m'} \frac{\kappa_{mm'}}{d_m+d_{m'}}\big(T_m-T_{m'}\big) A_{mm'} \nonumber\\
&+ \delta_{mM}^{}\frac{\kappa_{mM}}{d_m+d_{M}}\big(T_m-T_{M}\big) A_{mM} \eq 0,
\EA
\end{subequations}
for the $m$th matrix node. The matrix node designated by $M$ refers to the outer most node in contact with the fracture. The fracture volume $v_n$ is related to the REV volume $V_n$ by
\EQ
...
\begin{subequations}
\BA
\epsilon &\eq \frac{(d+2\delta)^3 - d^3}{(d+2\delta)^3} \eq 1-\left(\dfrac{1}{1+\dfrac{2\delta}{d}}\right)^3,\\
...
\caption{Control volumes in DCDM multiple continuum model with fracture aperture $2\delta$ and matrix block size $d$.}
\label{fminc}
\end{figure}

In terms of partial differential equations the heat conduction equations may be written as
\begin{subequations}
\EQ
\frac{\p}{\p t} \epsilon \Big[\varphi \sum_\a s_\a \rho_\a U_\a + (1-\varphi) \rho_r C_r T_f\Big] + \bnabla\cdot \Big(\sum_\a \bq_\a \rho_\a H_\a -\epsilon\kappa_f\bnabla T_f\Big) \eq -A_{fm} \kappa_{fm}\frac{\p T_m}{\p n},
\EN
and
\EQ
\frac{\p}{\p t} \rho_r C_r T_m + \frac{\p}{\p\xi} \Big(-\kappa_m\frac{\p T_m}{\p\xi}\Big) \eq 0,
\EN
for fracture and matrix temperatures $T_f$ and $T_m$, respectively, where $\xi$ represents the matrix coordinate assumed to be an effective 1D problem. The boundary condition 
\EQ
...
Porosity, permeability and mineral surface area may be updated optionally due to mineral precipitation and dissolution reactions according to the relations
\EQ
...
and
\EQ
a_m \eq a_m^0 \left(\frac{\varphi_m}{\varphi_m^0}\right)^n,
\EN
where the super/subscript 0 denotes initial values, with a typical value for $n$ of $2/3$ reflecting the surface to volume ratio. Note that this relation only applies to primary minerals that do not completely dissolve.
...
\verb|mc_heat|:& --enable multiple continuum formulation for heat\\ &(works only in THC and MPHASE modes)\\
...
\end{deflist}
%\item[(., /, END)]
\item[SECONDARY\_CONTINUUM] \ Need to compile with flag {\tt mc\_heat} 
\begin{deflist}{000}
\item[TYPE] \ {\tt name} (SLAB, NESTED\_CUBES, NESTED\_SPHERES)
\item[LENGTH] \ {\tt <float>} [m] \ length (for SLAB), side (for NESTED\_CUBES), radius (for NESTED\_SPHERES)]
\item[AREA] \ {\tt <float>} [m$^2$] \ cross-section area for SLAB type (not applicable for other types)
\item[NUM\_CELLS] \ {\tt <int>} \ Number of secondary continuum grid cells
\item[EPSILON] \ {\tt <float>} \ Volume fraction of the primary continuum (fracture) in the REV
\end{deflist}
\item[(., /, END)]
