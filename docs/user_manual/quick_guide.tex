...
\EQ
Q_w \eq \frac{q_M}{W} \delta(\br-\br_{ss}),
\EN
where $q_M$ denotes a mass rate in kg/s, $W$ denotes the formula weight of water, and $\br_{ss}$ denotes the location of the source/sink.
...
\EQ
\bq_\a \eq -\frac{kk_\a}{\mu_\a} \bnabla \big(P_\a-W_\a^{}\eta_\a g \bz\big),
\EN
where $k$ refers to the intrinsic permeability, $k_\a$ denotes the relative permeability, $\mu_\a$ denotes the fluid viscosity, $W_\a^{}$ denotes the formula weight, $g$ denotes the acceleration of gravity, and $z$ designates the vertical of the position vector. The quantities $\rho_r$, $c_r$, and $\kappa$ refer to the mass density, heat capacity, and thermal conductivity of the porous rock. The source/sink terms, $Q_i$ and $Q_e$, describe injection and extraction of mass and heat at wells, respectively. Several different well models are available. The simplest is a mass injection/production well given by
\EQ
Q_i \eq \sum_\a W_\a^{-1} q_\a^M X_i^\a \delta(\br-\br_{ss}),
\EN
where $q_\a^M$ refers to the mass rate in kg/s, and $W_\a$ denotes the formula weight given by
\EQ
W_\a \eq \sum_i W_i^{} X_i^\a.
\EN
The position vector $\br_{ss}$ refers to the location of the source/sink.
...
\subsubsection{Gas: $(p_g,\,T,\,X_\c^g)$ $\rightarrow$ Two-Phase: $(p_g,\,T,\,s_g^{})$}

$\bullet$ gas $\rightarrow$ 2-ph: $X_\c^g \leq 1-\dfrac{P_{\rm sat}(T)}{p_g}$, \ or equivalently: $X_\w^g \geq \dfrac{P_{\rm sat}(T)}{p_g}$

\subsubsection{Liquid: $(p_l,\,T,\,X_\c^l)$ $\rightarrow$ Two-phase: $(p_g,\,T,\,s_g^{})$}
...
\subsubsection{Two-Phase: $(p_g,\,T,\,s_g)$ $\rightarrow$ Liquid $(p_l,\,T,\,X_\c^l)$ or Gas $(p_g,\,T,\,X_\c^g)$}
...
\EQ
Q_j \eq \frac{q_M}{\rho}\Psi_j \delta(\br-\br_{ss}),
\EN
where $q_M$ denotes a mass rate in units of kg/s, $\rho$ denotes the fluid density in kg/m$^3$, and $\br_{ss}$ refers to the location of the source/sink. The quantity $S_j$ represents the sorbed concentration of the $j$th primary species considered in more detail in the next section.
