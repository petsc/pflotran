...
\EN
where $K_m$ denotes the equilibrium constant and the ion activity product is defined as
\EQ
...
\EQ
k_{ml} (T) \eq k_{ml}^0 \exp\left[\frac{1}{R}\Big(\frac{1}{T_0}-\frac{1}{T}\Big)\right],
\EN
where $k_{ml}^0$ refers to the rate constant at the reference temperature $T_0$, with $T$ in units of Kelvin and $R$ denotes the gas constant (8.317 J/mol/K). The sum over $l$ in Eqn.\eqref{Im} represents contributions from parallel reaction mechanisms such as pH dependence etc. where $\P_{ml}$ denotes a prefactor of the form
\EQ
